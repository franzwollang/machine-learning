...
- **Canonical arbiter deferred to persistence (M2) / DM gate (M4).** The intrinsic-vs-composite distinction is a cross-scale statement (SI S2.6.2): a real partition persists across ≥ 2 adjacent τ grid points; a uniform manifold's arc-partition does not. The heuristic purge and the null test therefore **land with M2** (Q-partition persistence across the scale grid, already the #28 secondary signal), or with the M4 S3.4 Dirichlet–multinomial evidence gate that supplies the non-degenerate likelihood-ratio null. SI S2.6.1 now documents this scope: single-scale `Q` is a proposal screen and the cleanup passes are provisional operational stand-ins for the persistence signal.
- **Remaining code work (blocked until M2/M4).** `clustering.py` still carries the provisional heuristics with dataset-motivated constants: boundary-refine `eta = 0.3` (`_refine_boundaries`), the tiny-satellite absorbers (`_absorb_tiny_clusters_into_dominant`, `_absorb_one_tiny_satellite`, `_absorb_full_graph_isolates`), and the `<= 3`-fragment collapse in `run_clustering`. Do **not** delete them until the persistence/DM arbiter can reproduce the circle=1 / swiss≤3 / hierarchy(6-leaf) regressions: deleting them now regresses circle→4 and swiss→4 (measured). The four previously-dead helpers are already removed.
