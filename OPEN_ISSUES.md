...
- **Single-cluster null as first-class candidate.** Uniform single-component manifolds (circle) currently rely on isolate absorption plus a final "collapse if <= 3 fragments" pass. The principled form: any proposed partition must beat the one-cluster null by a margin under the same criterion. **Design subtlety (discovered):** the whole-graph null cluster has *zero* boundary edges, so `BoundaryInter = 0` and `Q(null) = log(LocalIntra/eps)` is degenerately large — the raw partition-`Q` cannot be used to compare a multi-cluster partition against the null (the null trivially wins). The null test must therefore use the DM Bayes-factor margin (S3.4), or a null formulated so its boundary term is well-defined (e.g. comparing against the best single-cut rather than the empty boundary). Resolve this before wiring the null into `clustering.py`.
- **Replace residual cleanup passes with one rule.** The three dead helpers (`_merge_tiny_lifted_into_full_graph_neighbor`, `_coalesce_if_marginal_k_way_split`, `_maybe_rebalance_two_cluster_partition`) and the stale `_smoothed_pmi_similarity_matrix` wrapper are now removed (`KMeans` import dropped with them); regressions stay green. Still active in `clustering.py`: boundary-refine `eta = 0.3` (`_refine_boundaries`), the tiny-satellite absorbers (`_absorb_tiny_clusters_into_dominant`, `_absorb_one_tiny_satellite`, `_absorb_full_graph_isolates`), and the final `<= 3` collapse pass in `run_clustering`. Target: a single Q-improving pairwise merge applied to fixpoint plus the null test. If the existing regressions cannot pass under that single rule, the Q definition (likely the kernel term `K_v`) is what needs fixing — not more cleanup.
