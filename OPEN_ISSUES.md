...
- Rework direction:
  1. Primary signal: the compensated node-count / support trace — on a d-dimensional support, equilibrium node count scales as `N(tau) ~ V_supp / tau^{d/2}`, so knees/plateaus in `N(tau) * tau^{d/2}` (equivalently `V_C(tau)`) mark characteristic scales. **Implemented** (`stage1/characteristic_scale.py`; `controller.py` `selector="node_count"` / `"combined"`, default still `"load_band"`): compensated-count trace + coarse-edge-of-plateau knee detector with per-grid-point intrinsic-dim compensation; constants `theta_C=0.15`, `P_plat=2` (SI S2.5 / S14.3). Unit-tested on controlled plateau/monotone inputs.
...
- **Blocking finding (measured this turn):** the compensated node-count trace does *not* yet plateau cleanly on the reference scaffold, so `node_count`/`combined` currently fall back to `load_band` (verified: they return the same tau* as legacy). Two causes, both must close before the primary signal can beat legacy and the default can flip:
  1. the degree-proxy intrinsic-dim estimate is biased high (median `d≈3` on the `d=1` circle), corrupting the `tau^{d/2}` compensation — couples the M2 primary-signal rebuild to **#39** (fix/validate the intrinsic-dim estimator first);
  2. node count saturates against `max_nodes` at fine scales while the scaffold begins resolving noise / tube cross-sections, so effective `d` drifts with scale and `C(tau)` never flattens — needs a non-binding `max_nodes` (or an over-refinement guard) during scale search, and/or a scale-space-consistent `d` (couples to **#32**).
- Remaining: (a) unblock the primary signal by closing #39 (and the `max_nodes`/over-refinement effect), then make `node_count`/`combined` dominate load-band on tau* accuracy; (b) make it the default selector (migrated scenario by scenario) and remove `band_lo = 0.65` + the one-step-coarser patch; (c) tighten the scale-search tau* tolerance bands once (a)+(b) land; (d) rewrite SI S2.5/S2.5.1 to match.
