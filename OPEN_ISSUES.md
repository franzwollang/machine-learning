...
- **CORRECTED FINDING (cross-family validated): the persistence gate does NOT replace the stand-ins; they are load-bearing and deletion is BLOCKED.** A *full* ablation — monkeypatch `_refine_boundaries`, `_absorb_*`, **and** `_q_merge_any_improving` (which also disables the `<= 3` collapse and the `>= 4` re-merge) to identities — with `require_persistent_split=True` gives **circle → 37 leaves** (want 1) and **hierarchy → 12 leaves** (want 6). This *refutes* the earlier turn-7 ablation note ("gate alone yields circle→1"), which almost certainly left the `<= 3` collapse intact and so silently kept the very heuristic under test. GPT audit (agent `320a28ae`) reproduced circle→37 exactly and confirmed the diagnosis and every point below.
  ...
  - *Deletion scope (now actionable — DM gate module has landed on `main`):* the load-bearing stand-ins are `_refine_boundaries` (eta=0.3), `_absorb_tiny_clusters_into_dominant`, `_absorb_one_tiny_satellite`, `_absorb_full_graph_isolates`, and the `<= 3`-fragment collapse in `run_clustering` (the four previously-dead helpers are already removed). With (ii′) refuted, the sole remaining path to deletion is **(i) the Stage-2 S3.4 DM evidence gate**, whose module now exists and is tested (`evidence/dm_score.py`, `gate.py`, `star_matrix.py`; S3.4–S3.6, S10.4, merged in 7fab454) — the blocker is no longer "the gate does not exist" but the runtime wiring. Remaining work: (1) wire `EvidenceGate` into the Stage-1 clustering/recursion acceptance loop (score each proposed split/merge on its affected region during training, respecting the S3.6 cadence/hysteresis/edit-budget), (2) validate the gate reproduces correct cluster counts across ALL scenarios (circle, swiss, hierarchy, and — when available — nested spheres / linked tori) with the stand-ins ablated, then (3) make the gate default, re-scope the single-scale tests (chiefly `test_circle_clustering_produces_one_cluster` ==1 and `test_swiss_roll_stage1_diagnostics_at_tau_star` ≤3), and delete the stand-ins. This is acceptance-path — cross-family audit required.
