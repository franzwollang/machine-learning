...
- **CORRECTED FINDING (cross-family validated): the persistence gate does NOT replace the stand-ins; they are load-bearing and deletion is BLOCKED.** A *full* ablation — monkeypatch `_refine_boundaries`, `_absorb_*`, **and** `_q_merge_any_improving` (which also disables the `<= 3` collapse and the `>= 4` re-merge) to identities — with `require_persistent_split=True` gives **circle → 37 leaves** (want 1) and **hierarchy → 12 leaves** (want 6). This *refutes* the earlier turn-7 ablation note ("gate alone yields circle→1"), which almost certainly left the `<= 3` collapse intact and so silently kept the very heuristic under test. GPT audit (agent `320a28ae`) reproduced circle→37 exactly and confirmed the diagnosis and every point below.
  ...
  - *Hardening directions (ranked d>c>a>b, cross-family):* (i) most principled — require the Stage-2 S3.4 Dirichlet–multinomial evidence gate as the acceptance null (blocks on M4); (ii) require the persistence interval to be coarse-anchored / connected to the coarse end, rejecting an isolated fine-end block truncated by the grid; (iii) stopgap — longer minimal run or higher **mean** interval overlap (`min_persistence ≥ 3` cleanly fixes both datasets but contradicts the SI "minimal interval" framing and risks overfitting to two scenarios); (iv) NOT recommended — raising `theta_ovl` alone is brittle (0.60 → circle still 27 leaves; 0.65 → hierarchy degrades to 9).
  - *Blocked deletion scope:* the load-bearing stand-ins are `_refine_boundaries` (eta=0.3), `_absorb_tiny_clusters_into_dominant`, `_absorb_one_tiny_satellite`, `_absorb_full_graph_isolates`, and the `<= 3`-fragment collapse in `run_clustering` (the four previously-dead helpers are already removed). Only once a hardening direction lands and is validated across ALL scenarios (circle, swiss, hierarchy, and — when available — nested spheres / linked tori) should the gate be made default, the single-scale tests re-scoped (chiefly `test_circle_clustering_produces_one_cluster` ==1 and `test_swiss_roll_stage1_diagnostics_at_tau_star` ≤3), and the stand-ins deleted. SI S2.6.1/S2.6.2/S14.3 were corrected this turn to record the limitation (`P_persist` reclassified derived→operational).
