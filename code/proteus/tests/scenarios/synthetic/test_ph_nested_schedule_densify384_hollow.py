"""Nested schedule {1:3,2:6} × densify384 × hollow cliff pin (#41 / A4-T80).

A4-T74: schedule recovers both shells at densify128×hollow (``all_match=true``).
A4-T77: the same schedule at densify256×hollow also recovers
(``all_match=true`` on signal+primary+mild). A4-T73: densify512×hollow left
``any_all=false`` / ``any_b2=false``. This harness pins the densify cliff at
``max_nodes=384`` under signal vs primary/mild hollow, asking whether the
256 win still holds mid-ladder or regresses before 512.
...
class NestedScheduleDensify384HollowArm:
    name: str
...
@dataclass(frozen=True)
class NestedScheduleDensify384HollowBundle:
    dataset_seed: int
    stage1_seed: int
    max_nodes: int
    n_signal: int
    ...
    signal_arm: NestedScheduleDensify384HollowArm
    hollow_arms: tuple[NestedScheduleDensify384HollowArm, ...]
    any_all_match: bool
...
def nested_schedule_densify384_hollow_bundle() -> (
    NestedScheduleDensify384HollowBundle
):
    """Fit nested max_nodes=384; scheduled mult on signal vs hollow arms."""
    ...
    def _scheduled_arm(
        ...
        *,
    ...
    ) -> NestedScheduleDensify384HollowArm:
        scheduled = run_scheduled_mult_per_region_ph(
            pts,
            labs,
            sigma,
            mult_by_region=SCHEDULE,
            scenario=f"nested_schedule_densify384_hollow_{name}",
        ...
        return NestedScheduleDensify384HollowArm(
            name=name,
            mid_radius_frac=mid,
    ...
    hollow_arms: list[NestedScheduleDensify384HollowArm] = []
    ...
    for name, cfg in (("primary", PRIMARY_HOLLOW), ("mild", MILD_HOLLOW)):
        ...
        if not fallback:
            arm = NestedScheduleDensify384HollowArm(
                name=arm.name,
    ...
    return NestedScheduleDensify384HollowBundle(
...
def test_nested_schedule_densify384_hollow_harness_lands(
    nested_schedule_densify384_hollow_bundle,
) -> None:
    """Nested schedule×densify384×hollow lands; SI defaults untouched."""
    bundle = nested_schedule_densify384_hollow_bundle
...
@pytest.mark.scenario
@pytest.mark.synthetic
def test_nested_schedule_densify384_hollow_documents_gap(
    nested_schedule_densify384_hollow_bundle,
) -> None:
    """Document schedule×densify384 cliff vs T77@256 win / T73@512 fail.

    Soft: signal/hollow scheduled all-match shows densify256 win transfers
    to 384. Otherwise document mid-densify regression before 512.
    Never flip awaiting.
    """
    bundle = nested_schedule_densify384_hollow_bundle
    if bundle.signal_recovers or bundle.any_all_match:
        assert FILTRATION_MULTIPLIER == 1.5
        assert (
