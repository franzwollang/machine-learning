"""Nested schedule {1:3,2:6} × densify{294,318} × hollow cliff fine-pin (#41 / A4-T107).

Cliff map (prior): 296full / 300mild / 304mild / 306mild / 308full / 310full / 312fail.
This harness fine-pins remaining gaps at ``max_nodes∈{294,318}``
(between 288full→296full and past 316fail) under signal vs primary/mild hollow.
...
MAX_NODES_GRID: tuple[int, ...] = (294, 318)
DATASET_SEED: int = 21
STAGE1_SEED: int = 77
...
@dataclass(frozen=True)
class NestedScheduleDensify294318HollowBundle:
    dataset_seed: int
    stage1_seed: int
    ...
    full_recover_nodes: tuple[int, ...]
    mild_only_nodes: tuple[int, ...]
    hard_fail_nodes: tuple[int, ...]
    cliff_onset_outside_known_recover: bool
    table: str
    ...
    def _scheduled_arm(
        ...
        *,
        ...
        scheduled = run_scheduled_mult_per_region_ph(
            ...
            scenario=f"nested_schedule_densify{max_nodes}_cliff294_318_hollow_{name}",
...
def nested_schedule_densify294_318_hollow_bundle() -> (
    NestedScheduleDensify294318HollowBundle
):
    """Fit nested schedule×hollow at densify 294 and 318."""
    ...
    hard_nodes: list[int] = []
    for r in rungs:
        mild = (not r.signal_recovers) and any(
            a.name == "mild" and a.all_match is True for a in r.hollow_arms
        )
        if r.signal_recovers:
            full_nodes.append(int(r.max_nodes))
        elif mild or r.any_all_match:
            mild_nodes.append(int(r.max_nodes))
        else:
            hard_nodes.append(int(r.max_nodes))
    # Cliff pin: neither rung recovers between 288full→296full gap and past 316fail.
    ...
    return NestedScheduleDensify294318HollowBundle(
        ...
        full_recover_nodes=tuple(full_nodes),
        mild_only_nodes=tuple(mild_nodes),
        hard_fail_nodes=tuple(hard_nodes),
        cliff_onset_outside_known_recover=cliff,
        table="\n".join(table_lines),
    )
...
def test_nested_schedule_densify294_318_hollow_harness_lands(
    nested_schedule_densify294_318_hollow_bundle,
) -> None:
    """Nested schedule×densify294/318×hollow lands; SI defaults untouched."""
    bundle = nested_schedule_densify294_318_hollow_bundle
...
@pytest.mark.scenario
@pytest.mark.synthetic
def test_nested_schedule_densify294_318_hollow_documents_gap(
    nested_schedule_densify294_318_hollow_bundle,
) -> None:
    """Document densify294/318 vs cliff map 288full/296full/298full/300mild/…/316fail.

    Soft: any rung recovering all-match fills the non-mono cliff map.
    Otherwise document hard fail / mild-only at these pins.
    Never flip awaiting.
    """
    bundle = nested_schedule_densify294_318_hollow_bundle
    if bundle.any_signal_recovers or bundle.any_all_match:
        ...
        assert bundle.full_recover_nodes == ()
        assert bundle.mild_only_nodes == ()
        assert bundle.hard_fail_nodes == tuple(MAX_NODES_GRID)
        assert bundle.cliff_onset_outside_known_recover is True
        for r in bundle.rungs:
