"""Nested schedule {1:3,2:6} × densify{302,314} × hollow cliff fine-pin (#41 / A4-T101).

Cliff map (prior): 296full / 300mild / 304mild / 306mild / 308full / 310full / 312fail.
This harness fine-pins remaining gaps at ``max_nodes∈{302,314}``
(between 300mild→304mild and 312fail→beyond) under signal vs primary/mild hollow.
...
MAX_NODES_GRID: tuple[int, ...] = (302, 314)
DATASET_SEED: int = 21
STAGE1_SEED: int = 77
...
@dataclass(frozen=True)
class NestedScheduleDensify302314HollowBundle:
    dataset_seed: int
    stage1_seed: int
    ...
    cliff_onset_between_304_and_312: bool
    table: str
    ...
    def _scheduled_arm(
        ...
        *,
        ...
        scheduled = run_scheduled_mult_per_region_ph(
            ...
            scenario=f"nested_schedule_densify{max_nodes}_cliff302_314_hollow_{name}",
...
def nested_schedule_densify302_314_hollow_bundle() -> (
    NestedScheduleDensify302314HollowBundle
):
    """Fit nested schedule×hollow at densify 302 and 314."""
    ...
    # Cliff pin: neither rung recovers between 304 mild and 308/312 neighbors.
    ...
    return NestedScheduleDensify302314HollowBundle(
        ...
        cliff_onset_between_304_and_312=cliff,
        table="\n".join(table_lines),
    )
...
def test_nested_schedule_densify302_314_hollow_harness_lands(
    nested_schedule_densify302_314_hollow_bundle,
) -> None:
    """Nested schedule×densify302/314×hollow lands; SI defaults untouched."""
    bundle = nested_schedule_densify302_314_hollow_bundle
...
@pytest.mark.scenario
@pytest.mark.synthetic
def test_nested_schedule_densify302_314_hollow_documents_gap(
    nested_schedule_densify302_314_hollow_bundle,
) -> None:
    """Document densify302/314 vs cliff map 300mild/304mild/306mild/308full/312fail.
    ...
    if bundle.any_signal_recovers or bundle.any_all_match:
        ...
        assert bundle.cliff_onset_between_304_and_312 is True
        for r in bundle.rungs:
