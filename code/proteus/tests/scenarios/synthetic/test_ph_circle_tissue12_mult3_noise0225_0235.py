"""Circle tissue=0.12 mult3 residual × noise{0.225,0.235} edge pin (#41 / A4-T105).
...
and pins the collapse edge with ``noise∈{0.22,0.225,0.23,0.235,0.24,0.25}`` — proposal-path only.
...
NOISE_GRID: tuple[float, ...] = (0.22, 0.225, 0.23, 0.235, 0.24, 0.25)
EXPECTED_B1: int = 1
PIN_MULT: float = 4.0
PIN_FRAC: float = 3.0
...
class CircleTissue12Mult3Noise02250235Row:
    noise: float
...
@dataclass(frozen=True)
class CircleTissue12Mult3Noise02250235Bundle:
    dataset_seed: int
    stage1_seed: int
    ...
    rows: tuple[CircleTissue12Mult3Noise02250235Row, ...]
...
def circle_tissue12_mult3_noise0225_0235_bundle() -> CircleTissue12Mult3Noise02250235Bundle:
    """Cal/mult3 neighborhood × noise{0.22..0.25} fine-pin incl 0.225/0.235 at tissue=0.12."""
    rows: list[CircleTissue12Mult3Noise02250235Row] = []
    ...
    for noise in NOISE_GRID:
        ...
        for mult in MULT_ARMS:
            for frac in FRAC_GRID:
                ...
                rows.append(
                    CircleTissue12Mult3Noise02250235Row(
                        noise=float(noise),
                        filtration_mult=float(mult),
                        lifetime_frac=float(frac),
                        n_signal=n_signal,
    ...
    surv_seq = [
        residual_022, residual_0225, residual_023,
        residual_0235, residual_024, residual_025,
    ]
    ...
    return CircleTissue12Mult3Noise02250235Bundle(
...
def test_circle_tissue12_mult3_noise0225_0235_harness_lands(
    circle_tissue12_mult3_noise0225_0235_bundle,
) -> None:
    """Tissue0.12 mult3×noise{0.225,0.235} edge lands; SI defaults untouched."""
    bundle = circle_tissue12_mult3_noise0225_0235_bundle
...
@pytest.mark.scenario
@pytest.mark.synthetic
def test_circle_tissue12_mult3_noise0225_0235_documents_gap(
    circle_tissue12_mult3_noise0225_0235_bundle,
) -> None:
    """Document mult3 residual edge vs noise{0.225,0.235}; never flip awaiting.
