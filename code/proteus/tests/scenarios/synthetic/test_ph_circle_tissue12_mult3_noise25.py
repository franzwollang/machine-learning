"""Circle tissue=0.12 mult3 residual × noise≥0.25 collapse (#41 / A4-T96).

A4-T93: tissue0.12 cal4 pin survived ≤0.20 and died@0.22; mult3 alt stayed
clean at all three noises incl. 0.22 (20/27 cells). This harness freezes the
same ``mult∈{3,4,5} × frac∈{2.5,3,3.5}`` neighborhood at tissue=0.12 and
extends the noise ladder to ``{0.22,0.25,0.30}``, asking whether the mult3
residual collapses above 0.22 — proposal-path only.
...
# Bridge T93 residual@0.22 into higher-noise collapse probe.
...
class CircleTissue12Mult3Noise25Row:
    noise: float
...
@dataclass(frozen=True)
class CircleTissue12Mult3Noise25Bundle:
    dataset_seed: int
    stage1_seed: int
    ...
    rows: tuple[CircleTissue12Mult3Noise25Row, ...]
...
def circle_tissue12_mult3_noise25_bundle() -> CircleTissue12Mult3Noise25Bundle:
    """Cal/mult3 neighborhood × noise≥0.22 ladder at tissue=0.12."""
    rows: list[CircleTissue12Mult3Noise25Row] = []
    ...
    return CircleTissue12Mult3Noise25Bundle(
...
def test_circle_tissue12_mult3_noise25_harness_lands(
    circle_tissue12_mult3_noise25_bundle,
) -> None:
    """Tissue0.12 mult3×noise≥0.22 lands; SI defaults untouched."""
    bundle = circle_tissue12_mult3_noise25_bundle
...
@pytest.mark.scenario
@pytest.mark.synthetic
def test_circle_tissue12_mult3_noise25_documents_gap(
    circle_tissue12_mult3_noise25_bundle,
) -> None:
    """Document mult3 residual vs noise≥0.25; never flip awaiting.

    Soft: any pin/alt clean is proposal-path only.
    Otherwise document collapse above the T93@0.22 residual.
    """
    bundle = circle_tissue12_mult3_noise25_bundle
