"""Nested schedule {1:3,2:6} × densify128 × hollow (#41 / A4-T74-followon).

A4-T33: schedule recovers both shells at ``max_nodes=128``. A4-T73: the same
schedule at densify512 × hollow left ``any_all=false`` / ``any_b2=false``.
This harness reconfirms schedule recovery at the densify128 baseline under
signal vs primary/mild hollow, asking whether hollow preserves the known
``(1,0,1)`` schedule win (contrast to the densify512 regression).
...
class NestedScheduleDensify128HollowArm:
    name: str
...
@dataclass(frozen=True)
class NestedScheduleDensify128HollowBundle:
    dataset_seed: int
    stage1_seed: int
    max_nodes: int
    n_signal: int
    ...
    signal_arm: NestedScheduleDensify128HollowArm
    hollow_arms: tuple[NestedScheduleDensify128HollowArm, ...]
    any_all_match: bool
...
def nested_schedule_densify128_hollow_bundle() -> (
    NestedScheduleDensify128HollowBundle
):
    """Fit nested max_nodes=128; scheduled mult on signal vs hollow arms."""
    ...
    def _scheduled_arm(
        ...
        *,
    ...
    ) -> NestedScheduleDensify128HollowArm:
        scheduled = run_scheduled_mult_per_region_ph(
            pts,
            labs,
            sigma,
            mult_by_region=SCHEDULE,
            scenario=f"nested_schedule_densify128_hollow_{name}",
        ...
        return NestedScheduleDensify128HollowArm(
            name=name,
            mid_radius_frac=mid,
    ...
    hollow_arms: list[NestedScheduleDensify128HollowArm] = []
    ...
    for name, cfg in (("primary", PRIMARY_HOLLOW), ("mild", MILD_HOLLOW)):
        ...
        if not fallback:
            arm = NestedScheduleDensify128HollowArm(
                name=arm.name,
    ...
    return NestedScheduleDensify128HollowBundle(
...
def test_nested_schedule_densify128_hollow_harness_lands(
    nested_schedule_densify128_hollow_bundle,
) -> None:
    """Nested schedule×densify128×hollow lands; SI defaults untouched."""
    bundle = nested_schedule_densify128_hollow_bundle
...
@pytest.mark.scenario
@pytest.mark.synthetic
def test_nested_schedule_densify128_hollow_documents_gap(
    nested_schedule_densify128_hollow_bundle,
) -> None:
    """Document schedule×densify128×hollow vs T73 densify512 regression.

    Soft: signal (or hollow-preserved) scheduled all-match reconfirms the
    densify128 schedule win. Otherwise document non-recovery at 128+hollow.
    Never flip awaiting.
    """
    bundle = nested_schedule_densify128_hollow_bundle
