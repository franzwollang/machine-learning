"""Nested schedule {1:3,2:6} × densify{300,308} × hollow cliff fine-pin (#41 / A4-T92).

A4-T89: densify296 full recover (signal+primary+mild); densify312 hard fail.
This harness fine-pins the cliff between 296 win and 312 fail at
``max_nodes∈{300,308}`` under signal vs primary/mild hollow.
...
MAX_NODES_GRID: tuple[int, ...] = (300, 308)
DATASET_SEED: int = 21
STAGE1_SEED: int = 77
...
@dataclass(frozen=True)
class NestedScheduleDensify300308HollowBundle:
    dataset_seed: int
    stage1_seed: int
    ...
    def _scheduled_arm(
        ...
        *,
        ...
        scheduled = run_scheduled_mult_per_region_ph(
            ...
            scenario=f"nested_schedule_densify{max_nodes}_ultrafine_hollow_{name}",
...
def nested_schedule_densify300_308_hollow_bundle() -> (
    NestedScheduleDensify300308HollowBundle
):
    """Fit nested schedule×hollow at densify 300 and 308."""
    ...
    return NestedScheduleDensify300308HollowBundle(
...
def test_nested_schedule_densify300_308_hollow_harness_lands(
    nested_schedule_densify300_308_hollow_bundle,
) -> None:
    """Nested schedule×densify300/308×hollow lands; SI defaults untouched."""
    bundle = nested_schedule_densify300_308_hollow_bundle
...
@pytest.mark.scenario
@pytest.mark.synthetic
def test_nested_schedule_densify300_308_hollow_documents_gap(
    nested_schedule_densify300_308_hollow_bundle,
) -> None:
    """Document densify300/308 cliff fine-pin vs T89@296 win / 312 fail.

    Soft: any rung recovering all-match shows cliff onset after that densify.
    Otherwise document cliff onset already by 300/308.
    Never flip awaiting.
    """
    bundle = nested_schedule_densify300_308_hollow_bundle
