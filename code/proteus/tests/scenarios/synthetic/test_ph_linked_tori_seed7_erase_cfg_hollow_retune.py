"""Seed7 erase × hollow mid/h0 retune after MST dirty reintro (#41 / A4-T97).

A4-T94: erase×mst/soft_capacity reintroduced dirty-b2 on the previously clean
baseline erase (ungated baseline ``b2=0``) and never unlocked clean
``(1,2,1)``. This harness freezes ungated hollow (no MST/soft capacity) and
retunes ``mid_radius_frac × h0`` under both baseline erase flags
(``gabriel=False, min_end=0.5``) and the T91/T94 dirty-reintro flags
(``gabriel∧H, min_end=0.25``), asking whether mid/h0 retune recovers clean
void Betti after the MST dirty-reintro finding.
...
DATASET_SEED: int = 2
STAGE1_SEED: int = 7
# Expanded mid×h0 retune around erase band (T79/T82 mid=0.35).
MID_GRID: tuple[float, ...] = (0.25, 0.35, 0.45, 0.55)
H0_GRID: tuple[float, ...] = (0.4, 0.5, 0.6, 0.8)
# Flag modes: (name, gabriel, min_end, conj)
FLAG_MODES: tuple[tuple[str, bool, float, bool], ...] = (
    ("baseline", False, 0.5, False),
    ("dirty_reintro", True, 0.25, True),
)
...
class Seed7EraseHollowRetuneRow:
    mode: str
...
@dataclass(frozen=True)
class Seed7EraseHollowRetuneBundle:
    dataset_seed: int
    stage1_seed: int
    max_nodes: int
    n_signal: int
    ...
    mid_grid: tuple[float, ...]
    h0_grid: tuple[float, ...]
    flag_modes: tuple[tuple[str, bool, float, bool], ...]
    soft_fracs: tuple[float, ...]
    soft_mults: tuple[float, ...]
    rows: tuple[Seed7EraseHollowRetuneRow, ...]
    any_full_match: bool
    any_clean_b2: bool
    any_dirty_b2: bool
    max_b1: int
    max_b2: int
    best_cfg: tuple[str, float, float] | None
    baseline_mode_max_b2: int
    dirty_mode_max_b2: int
    table: str


@pytest.fixture(scope="module")
def linked_tori_seed7_erase_cfg_hollow_retune_bundle() -> (
    Seed7EraseHollowRetuneBundle
):
    """Fit seed7 denser256; erase mid×h0 retune under baseline/dirty flags."""
    ...
    signal_fixed = run_per_region_ph(
        ...
        signal_labs,
        sigma_star,
        scenario="linked_tori_seed7_erase_hollow_retune_signal_fixed",
        include_labels=[0, 1],
    ...
    rows: list[Seed7EraseHollowRetuneRow] = []
    any_full = False
    any_clean = False
    any_dirty = False
    max_b1 = 0
    max_b2 = 0
    baseline_mode_max_b2 = 0
    dirty_mode_max_b2 = 0
    best_cfg: tuple[str, float, float] | None = None
    best_score = -1
    table_lines = [
        "mode\tmid\th0\tn_after\taxis\tkey\tregion\tbetti\tclean\tdirty"
    ]

    for mode_name, gab, min_end, conj in FLAG_MODES:
        for mid in MID_GRID:
            for h0 in H0_GRID:
                ...
                fixed = run_per_region_ph(
                    ...
                    sigma_star,
                    scenario=(
                        f"seed7_erase_retune_{mode_name}_"
                        f"m{mid:g}_h{h0:g}_fixed"
                    ),
                    include_labels=[0, 1],
                    reading="fixed_threshold",
                    max_dim=2,
                ...
                for betti in fixed_betti.values():
                    ...
                    if betti == EXPECTED_TORI:
                        ...
                        any_full = True
                    table_lines.append(
                        f"{mode_name}\t{mid:g}\t{h0:g}\t{n_after}\tfixed\tSI\t"
                ...
                for cell in soft_grid.rows:
                    ...
                    if is_dirty:
                        ...
                        any_dirty = True
                    table_lines.append(
                        f"{mode_name}\t{mid:g}\t{h0:g}\t{n_after}\tsoft\t"
                ...
                if score > best_score:
                    best_score = score
                    best_cfg = (mode_name, float(mid), float(h0))

                max_b1 = max(max_b1, row_max_b1)
                max_b2 = max(max_b2, row_max_b2)
                if mode_name == "baseline":
                    baseline_mode_max_b2 = max(
                        baseline_mode_max_b2, row_max_b2
                    )
                else:
                    dirty_mode_max_b2 = max(dirty_mode_max_b2, row_max_b2)

                rows.append(
                    Seed7EraseHollowRetuneRow(
                        mode=mode_name,
                        mid_radius_frac=float(mid),
                        h0=float(h0),
                        gabriel_fallback=bool(gab),
    ...
    return Seed7EraseHollowRetuneBundle(
        ...
        max_b2=max_b2,
        best_cfg=best_cfg,
        baseline_mode_max_b2=int(baseline_mode_max_b2),
        dirty_mode_max_b2=int(dirty_mode_max_b2),
        table="\n".join(table_lines),
    )
...
def test_linked_tori_seed7_erase_cfg_hollow_retune_harness_lands(
    linked_tori_seed7_erase_cfg_hollow_retune_bundle,
) -> None:
    """Seed7 erase×hollow mid/h0 retune lands; SI defaults untouched."""
    bundle = linked_tori_seed7_erase_cfg_hollow_retune_bundle
    ...
    assert bundle.mid_grid == MID_GRID
    assert bundle.h0_grid == H0_GRID
    assert bundle.flag_modes == FLAG_MODES
    assert bundle.soft_fracs == SOFT_FRAC_GRID
    assert bundle.soft_mults == SOFT_MULT_ARMS
    assert len(bundle.rows) == (
        len(FLAG_MODES) * len(MID_GRID) * len(H0_GRID)
    )
    assert bundle.signal_fixed_dirty_b2 is True
    assert bundle.best_cfg is not None
    header = bundle.table.splitlines()[0]
    assert "mode" in header and "mid" in header and "h0" in header


@pytest.mark.scenario
@pytest.mark.synthetic
def test_linked_tori_seed7_erase_cfg_hollow_retune_documents_gap(
    linked_tori_seed7_erase_cfg_hollow_retune_bundle,
) -> None:
    """Document mid/h0 retune vs clean (1,2,1); never flip awaiting.

    Soft: any full ``(1,2,1)`` or clean ``b2`` is proposal-path evidence.
    Otherwise keep documenting mid/h0 retune ≠ clean recover after MST dirty.
    """
    bundle = linked_tori_seed7_erase_cfg_hollow_retune_bundle
    if bundle.any_full_match or bundle.any_clean_b2:
        ...
        assert bundle.baseline_mode_max_b2 >= 0
        assert bundle.dirty_mode_max_b2 >= 0
