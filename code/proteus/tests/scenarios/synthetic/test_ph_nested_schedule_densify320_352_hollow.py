"""Nested schedule {1:3,2:6} × densify{320,352} × hollow cliff fine-pin (#41 / A4-T83).

A4-T77: schedule recovers both shells at densify256×hollow (``all_match=true``).
A4-T80: densify384×hollow left ``any_all=false`` / ``any_b2=false`` — cliff sits
between 256 and 384. This harness fine-pins that densify cliff at mid-ladder
``max_nodes∈{320,352}`` under signal vs primary/mild hollow.
...
MAX_NODES_GRID: tuple[int, ...] = (320, 352)
DATASET_SEED: int = 21
STAGE1_SEED: int = 77
...
@dataclass(frozen=True)
class NestedScheduleDensify320352HollowBundle:
    dataset_seed: int
    stage1_seed: int
    ...
    cliff_between_256_and_384: bool
    table: str
    ...
    def _scheduled_arm(
        ...
        *,
        ...
        scheduled = run_scheduled_mult_per_region_ph(
            ...
            scenario=f"nested_schedule_densify{max_nodes}_hollow_{name}",
...
def nested_schedule_densify320_352_hollow_bundle() -> (
    NestedScheduleDensify320352HollowBundle
):
    """Fit nested schedule×hollow at densify 320 and 352."""
    ...
    # Cliff fine-pin: neither mid rung recovers while T77@256 did / T80@384 failed.
    ...
    return NestedScheduleDensify320352HollowBundle(
        ...
        cliff_between_256_and_384=cliff,
        table="\n".join(table_lines),
    )
...
def test_nested_schedule_densify320_352_hollow_harness_lands(
    nested_schedule_densify320_352_hollow_bundle,
) -> None:
    """Nested schedule×densify320/352×hollow lands; SI defaults untouched."""
    bundle = nested_schedule_densify320_352_hollow_bundle
...
@pytest.mark.scenario
@pytest.mark.synthetic
def test_nested_schedule_densify320_352_hollow_documents_gap(
    nested_schedule_densify320_352_hollow_bundle,
) -> None:
    """Document densify320/352 cliff fine-pin vs T77@256 / T80@384.

    Soft: any rung recovering all-match shows cliff onset after that densify.
    Otherwise document mid-ladder regression already by 320/352.
    Never flip awaiting.
    """
    bundle = nested_schedule_densify320_352_hollow_bundle
    if bundle.any_signal_recovers or bundle.any_all_match:
        ...
        assert bundle.cliff_between_256_and_384 is True
        for r in bundle.rungs:
