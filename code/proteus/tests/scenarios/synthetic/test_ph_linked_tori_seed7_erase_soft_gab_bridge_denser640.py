"""Seed7 erase × soft×gabriel∧H×bridge denser640 (#41 / A4-T112).

A4-T109: soft×gabriel∧H×bridge denser512 signal was void-absent
with ``max_b2=0`` (signal ``(1,1,0)/(1,3,0)``). This harness lifts the
same compose to denser ``max_nodes=640`` — asking whether further densify
unlocks void ``(1,2,1)`` / dirty b2 under baseline erase + gabriel∧H ×
bridge/soft mass (densify≠void unlock continues).
...
class Seed7EraseSoftGabBridgeDenserRow:
...
@dataclass(frozen=True)
class Seed7EraseSoftGabBridgeDenserBundle:
    dataset_seed: int
    stage1_seed: int
    max_nodes: int
    n_signal: int
    ...
    rows: tuple[Seed7EraseSoftGabBridgeDenserRow, ...]
    any_full_match: bool
    any_clean_b2: bool
    any_dirty_b2: bool
    max_b1: int
...
def linked_tori_seed7_erase_cfg_soft_gab_bridge_denser640_bundle() -> (
    Seed7EraseSoftGabBridgeDenserBundle
):
    """Fit seed7 denser640; erase × soft×gabriel∧H×bridge denser640."""
    ...
    signal_fixed = run_per_region_ph(
        ...
        signal_labs,
        sigma_star,
        scenario="linked_tori_seed7_erase_soft_gab_bridge_denser640_signal_fixed",
        include_labels=[0, 1],
    ...
    rows: list[Seed7EraseSoftGabBridgeDenserRow] = []
    any_full = False
    any_clean = False
    any_dirty = False
    max_b1 = 0
    ...
    for (mid, h0, gab, min_end, conj), is_baseline in hollow_cfgs:
        for bridge, soft, soft_frac, method in CAPACITY_MODES:
            ...
            fixed = run_per_region_ph(
                ...
                sigma_star,
                scenario=(
                    f"seed7_erase_softgab_d640_b{int(is_baseline)}_"
            ...
            rows.append(
                Seed7EraseSoftGabBridgeDenserRow(
                    mid_radius_frac=float(mid),
                    h0=float(h0),
                    gabriel_fallback=bool(gab),
                    min_end_count=float(min_end),
    ...
    return Seed7EraseSoftGabBridgeDenserBundle(
...
def test_linked_tori_seed7_erase_cfg_soft_gab_bridge_denser640_harness_lands(
    linked_tori_seed7_erase_cfg_soft_gab_bridge_denser640_bundle,
) -> None:
    """Seed7 soft×gabriel∧H×bridge denser640 lands; SI defaults untouched."""
    bundle = linked_tori_seed7_erase_cfg_soft_gab_bridge_denser640_bundle
    ...
    # denser640 signal is typically clean-partial (1,2,0) — not denser256 dirty-b2.
...
@pytest.mark.scenario
@pytest.mark.synthetic
def test_linked_tori_seed7_erase_cfg_soft_gab_bridge_denser640_documents_gap(
    linked_tori_seed7_erase_cfg_soft_gab_bridge_denser640_bundle,
) -> None:
    """Document soft×gabriel∧H×bridge denser640 vs clean (1,2,1); never flip awaiting.
    ...
    (signal may stay void-absent like denser384/512, or reintroduce dirty/clean b2).
    """
    bundle = linked_tori_seed7_erase_cfg_soft_gab_bridge_denser640_bundle
