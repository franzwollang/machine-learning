"""Seed7 erase × soft×gabriel∧H×bridge denser512 (#41 / A4-T109).

A4-T106: soft×gabriel∧H×bridge denser384 signal was void-absent
``(1,2,0)`` with ``max_b2=0``. This harness lifts the same compose to
denser ``max_nodes=512`` — asking whether further densify unlocks void
``(1,2,1)`` / dirty b2 under baseline erase + gabriel∧H × bridge/soft mass.
...
def linked_tori_seed7_erase_cfg_soft_gab_bridge_denser512_bundle() -> (
    Seed7EraseSoftGabBridgeDenserBundle
):
    """Fit seed7 denser512; erase × soft×gabriel∧H×bridge denser512."""
    ...
    signal_fixed = run_per_region_ph(
        ...
        signal_labs,
        sigma_star,
        scenario="linked_tori_seed7_erase_soft_gab_bridge_denser512_signal_fixed",
        include_labels=[0, 1],
    ...
    for (mid, h0, gab, min_end, conj), is_baseline in hollow_cfgs:
        for bridge, soft, soft_frac, method in CAPACITY_MODES:
            ...
            fixed = run_per_region_ph(
                ...
                sigma_star,
                scenario=(
                    f"seed7_erase_softgab_d512_b{int(is_baseline)}_"
...
def test_linked_tori_seed7_erase_cfg_soft_gab_bridge_denser512_harness_lands(
    linked_tori_seed7_erase_cfg_soft_gab_bridge_denser512_bundle,
) -> None:
    """Seed7 soft×gabriel∧H×bridge denser512 lands; SI defaults untouched."""
    bundle = linked_tori_seed7_erase_cfg_soft_gab_bridge_denser512_bundle
    ...
    # denser512 signal is typically clean-partial (1,2,0) — not denser256 dirty-b2.
...
@pytest.mark.scenario
@pytest.mark.synthetic
def test_linked_tori_seed7_erase_cfg_soft_gab_bridge_denser512_documents_gap(
    linked_tori_seed7_erase_cfg_soft_gab_bridge_denser512_bundle,
) -> None:
    """Document soft×gabriel∧H×bridge denser512 vs clean (1,2,1); never flip awaiting.
    ...
    (signal may stay clean-partial ``(1,2,0)`` like denser384, or reintroduce dirty/clean b2).
    """
    bundle = linked_tori_seed7_erase_cfg_soft_gab_bridge_denser512_bundle
