"""Nested schedule {1:3,2:6} × densify{288,304} × hollow cliff fine-pin (#41 / A4-T86).

A4-T77: schedule recovers both shells at densify256×hollow (``all_match=true``).
A4-T83: densify320/352×hollow already ``any_all=false`` / ``any_b2=false``.
This harness fine-pins cliff onset between T77@256 win and T83@320 fail at
``max_nodes∈{288,304}`` under signal vs primary/mild hollow.
...
MAX_NODES_GRID: tuple[int, ...] = (288, 304)
DATASET_SEED: int = 21
STAGE1_SEED: int = 77
...
@dataclass(frozen=True)
class NestedScheduleDensify288304HollowBundle:
    dataset_seed: int
    stage1_seed: int
    ...
    def _scheduled_arm(
        ...
        *,
        ...
        scheduled = run_scheduled_mult_per_region_ph(
            ...
            scenario=f"nested_schedule_densify{max_nodes}_onset_hollow_{name}",
...
def nested_schedule_densify288_304_hollow_bundle() -> (
    NestedScheduleDensify288304HollowBundle
):
    """Fit nested schedule×hollow at densify 288 and 304."""
    ...
    return NestedScheduleDensify288304HollowBundle(
...
def test_nested_schedule_densify288_304_hollow_harness_lands(
    nested_schedule_densify288_304_hollow_bundle,
) -> None:
    """Nested schedule×densify288/304×hollow lands; SI defaults untouched."""
    bundle = nested_schedule_densify288_304_hollow_bundle
...
@pytest.mark.scenario
@pytest.mark.synthetic
def test_nested_schedule_densify288_304_hollow_documents_gap(
    nested_schedule_densify288_304_hollow_bundle,
) -> None:
    """Document densify288/304 cliff onset vs T77@256 / T83@320.

    Soft: any rung recovering all-match shows cliff onset after that densify.
    Otherwise document cliff onset already by 288/304.
    Never flip awaiting.
    """
    bundle = nested_schedule_densify288_304_hollow_bundle
