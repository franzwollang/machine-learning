...
def test_linked_tori_betti_numbers():
    ...
    ``topology_from_accepted_regions(..., reading='lifetime')`` on accepted-region
    node positions (not the lifted-graph flag complex) with
    ``sigma_star = sqrt(tau_star)``. Do not flip by weakening thresholds — fitted
    circle probe still fails at SI ``1.5 sigma_star`` (``test_ph_fitted_circle_probe``).
    """
