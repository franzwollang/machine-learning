"""Circle tissue=0.08 cal4 neighborhood × high noise collapse (#41 / A4-T84).

A4-T81: cal4×frac3 clean pin survived noise∈{0,0.02,0.05} and the neighborhood
expanded under noise≥0.02. This harness freezes the same proposal-path
neighborhood ``mult∈{3,4,5}`` × ``frac∈{2.5,3.0,3.5}`` and crosses a higher
noise ladder ``{0.08, 0.10, 0.12}`` at fixed tissue=0.08, asking where the
clean pin collapses.
...
# Higher noise than T81's {0,0.02,0.05} — probe collapse boundary.
NOISE_GRID: tuple[float, ...] = (0.08, 0.10, 0.12)
EXPECTED_B1: int = 1
PIN_MULT: float = 4.0
PIN_FRAC: float = 3.0
...
class CircleTissue08Cal4NoiseHighRow:
    noise: float
...
@dataclass(frozen=True)
class CircleTissue08Cal4NoiseHighBundle:
    dataset_seed: int
    stage1_seed: int
    ...
    rows: tuple[CircleTissue08Cal4NoiseHighRow, ...]
...
def circle_tissue08_cal4_noise_high_bundle() -> CircleTissue08Cal4NoiseHighBundle:
    """Cal4 neighborhood × high-noise ladder at tissue=0.08."""
    rows: list[CircleTissue08Cal4NoiseHighRow] = []
    ...
    for noise in NOISE_GRID:
        ...
        for mult in MULT_ARMS:
            for frac in FRAC_GRID:
                ...
                rows.append(
                    CircleTissue08Cal4NoiseHighRow(
                        noise=float(noise),
                        filtration_mult=float(mult),
                        lifetime_frac=float(frac),
                        n_signal=n_signal,
    ...
    return CircleTissue08Cal4NoiseHighBundle(
...
def test_circle_tissue08_cal4_noise_high_harness_lands(
    circle_tissue08_cal4_noise_high_bundle,
) -> None:
    """Tissue0.08 cal4×high-noise lands; SI defaults untouched."""
    bundle = circle_tissue08_cal4_noise_high_bundle
...
@pytest.mark.scenario
@pytest.mark.synthetic
def test_circle_tissue08_cal4_noise_high_documents_gap(
    circle_tissue08_cal4_noise_high_bundle,
) -> None:
    """Document cal4×frac3 clean pin vs high noise; never flip awaiting.

    Soft: any pin/neighborhood clean at noise≥0.08 is proposal-path only.
    Otherwise document collapse relative to T81 mild-noise survival.
    """
    bundle = circle_tissue08_cal4_noise_high_bundle
