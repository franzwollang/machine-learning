"""Nested schedule {1:3,2:6} × densify256 × hollow (#41 / A4-T77-followon).

A4-T74: schedule recovers both shells at densify128×hollow (``all_match=true``).
A4-T73: the same schedule at densify512×hollow left ``any_all=false`` /
``any_b2=false``. This harness probes the densify cliff at ``max_nodes=256``
under signal vs primary/mild hollow, asking whether the densify128 schedule
win transfers mid-ladder or regresses before 512.
...
class NestedScheduleDensify256HollowArm:
    name: str
...
@dataclass(frozen=True)
class NestedScheduleDensify256HollowBundle:
    dataset_seed: int
    stage1_seed: int
    max_nodes: int
    n_signal: int
    ...
    signal_arm: NestedScheduleDensify256HollowArm
    hollow_arms: tuple[NestedScheduleDensify256HollowArm, ...]
    any_all_match: bool
...
def nested_schedule_densify256_hollow_bundle() -> (
    NestedScheduleDensify256HollowBundle
):
    ...
    def _scheduled_arm(
        ...
        *,
    ...
    ) -> NestedScheduleDensify256HollowArm:
        scheduled = run_scheduled_mult_per_region_ph(
            pts,
            labs,
            sigma,
            mult_by_region=SCHEDULE,
            scenario=f"nested_schedule_densify256_hollow_{name}",
        ...
        return NestedScheduleDensify256HollowArm(
            name=name,
            mid_radius_frac=mid,
    ...
    hollow_arms: list[NestedScheduleDensify256HollowArm] = []
    ...
    for name, cfg in (("primary", PRIMARY_HOLLOW), ("mild", MILD_HOLLOW)):
        ...
        if not fallback:
            arm = NestedScheduleDensify256HollowArm(
                name=arm.name,
    ...
    return NestedScheduleDensify256HollowBundle(
...
def test_nested_schedule_densify256_hollow_harness_lands(
    nested_schedule_densify256_hollow_bundle,
) -> None:
    """Nested schedule×densify256×hollow lands; SI defaults untouched."""
    bundle = nested_schedule_densify256_hollow_bundle
...
@pytest.mark.scenario
@pytest.mark.synthetic
def test_nested_schedule_densify256_hollow_documents_gap(
    nested_schedule_densify256_hollow_bundle,
) -> None:
    """Document schedule×densify256×hollow cliff vs T74@128 / T73@512.

    Soft: signal/hollow scheduled all-match shows densify128 win transfers
    to 256. Otherwise document mid-densify regression before 512.
    Never flip awaiting.
    """
    bundle = nested_schedule_densify256_hollow_bundle
