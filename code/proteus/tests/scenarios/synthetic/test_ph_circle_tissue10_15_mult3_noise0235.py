"""Circle mult3 residual × tissue∈{0.10,0.15} @ noise=0.235 (#41 / A4-T111).
...
``mult∈{3,4,5} × frac∈{2.5,3,3.5}`` neighborhood at noise=0.235 and asks
...
class CircleTissue1015Mult3Noise0235Row:
...
@dataclass(frozen=True)
class CircleTissue1015Mult3Noise0235Bundle:
    dataset_seed: int
    stage1_seed: int
    noise: float
    ...
    rows: tuple[CircleTissue1015Mult3Noise0235Row, ...]
...
def circle_tissue10_15_mult3_noise0235_bundle() -> (
    CircleTissue1015Mult3Noise0235Bundle
):
    """Cal/mult3 neighborhood × tissue{0.10,0.15} at noise=0.235."""
    rows: list[CircleTissue1015Mult3Noise0235Row] = []
    ...
    for tissue in TISSUE_GRID:
        ...
        for mult in MULT_ARMS:
            for frac in FRAC_GRID:
                ...
                rows.append(
                    CircleTissue1015Mult3Noise0235Row(
                        tissue_fraction=float(tissue),
                        filtration_mult=float(mult),
                        lifetime_frac=float(frac),
                        n_signal=n_signal,
    ...
    return CircleTissue1015Mult3Noise0235Bundle(
...
def test_circle_tissue10_15_mult3_noise0235_harness_lands(
    circle_tissue10_15_mult3_noise0235_bundle,
) -> None:
    """Tissue{0.10,0.15} mult3×noise0.235 lands; SI defaults untouched."""
    bundle = circle_tissue10_15_mult3_noise0235_bundle
...
@pytest.mark.scenario
@pytest.mark.synthetic
def test_circle_tissue10_15_mult3_noise0235_documents_gap(
    circle_tissue10_15_mult3_noise0235_bundle,
) -> None:
    """Document mult3 residual vs tissue{0.10,0.15}@0.235; never flip awaiting.

    Soft: any pin/alt clean is proposal-path only.
    Otherwise document collapse away from the T102/T108 tissue0.12@0.235 residual.
    """
    bundle = circle_tissue10_15_mult3_noise0235_bundle
