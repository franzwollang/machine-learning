"""Nested spheres hollow mid_radius_frac sweep (#41 / A4-T64-followon).

A4-T61: primary hollow (``mid=0.5``) on nested max_nodes=128 was ≈no-op
(n_signal 74→74) and did not flip dual-scale all-shell either-match
(``any_all_either=false``; coarse ``b2`` present without full shell recovery).

This harness freezes the nested Stage-1 scaffold and sweeps
``mid_radius_frac ∈ {0.35, 0.5, 0.65, 0.8}`` (h0=0.7, no Gabriel) under
global-sigma dual-scale PH, asking whether a milder/stronger hollow mid
unlocks shell voids ``(1,0,1)``.
...
MID_GRID: tuple[float, ...] = (0.35, 0.5, 0.65, 0.8)
H0: float = 0.7
...
@dataclass(frozen=True)
class NestedHollowMidSweepBundle:
    dataset_seed: int
    stage1_seed: int
    max_nodes: int
    n_signal: int
    ...
    coarse_mult: float
    fine_mult: float
    signal_arm: NestedHollowMidArm
    hollow_arms: tuple[NestedHollowMidArm, ...]
    any_all_either: bool
...
def nested_hollow_mid_sweep_bundle() -> NestedHollowMidSweepBundle:
    """Fit nested max_nodes=128; dual-scale signal vs mid-radius hollow sweep."""
    ...
    def _dual_arm(
        ...
        *,
        ...
        dual = run_dual_scale_per_region_ph(
            ...
            scenario=f"nested_hollow_mid_{name}_dual",
            include_labels=[1, 2],
            max_dim=2,
    ...
    # Recompute improves flags against true signal baseline.
    signal_arm = NestedHollowMidArm(
        ...
        all_either=signal_arm.all_either,
        any_coarse=signal_arm.any_coarse,
        any_fine=signal_arm.any_fine,
        any_b2=signal_arm.any_b2,
        improves_vs_signal=False,
    )

    hollow_arms: list[NestedHollowMidArm] = []
    ...
    for mid in MID_GRID:
        ...
        # Fix n_signal to actual after-hollow count when not fallback.
        if not fallback:
            arm = NestedHollowMidArm(
                name=arm.name,
        ...
        if arm.improves_vs_signal and best_mid is None:
            best_mid = float(mid)

    return NestedHollowMidSweepBundle(
...
def test_nested_hollow_mid_sweep_harness_lands(
    nested_hollow_mid_sweep_bundle,
) -> None:
    """Nested hollow mid-sweep lands; SI defaults untouched."""
    bundle = nested_hollow_mid_sweep_bundle
    ...
    assert len(bundle.hollow_arms) == len(MID_GRID)
...
@pytest.mark.scenario
@pytest.mark.synthetic
def test_nested_hollow_mid_sweep_documents_gap(
    nested_hollow_mid_sweep_bundle,
) -> None:
    """Document mid-sweep hollow vs nested void recovery; never flip awaiting.

    Soft: all-shell either-match under any mid is proposal-path evidence.
    Otherwise keep documenting non-recovery across the mid grid (T61 no-op).
    """
    bundle = nested_hollow_mid_sweep_bundle
    if bundle.any_all_either:
        ...
        # Hollow mid may invent dirty b2 without full shell recovery.
        assert bundle.any_b2 or not bundle.any_b2
        assert bundle.any_hollow_improves is False or bundle.best_mid is not None
