"""Nested densify max_nodes=256 × hollow dual-scale (#41 / A4-T67-followon).

A4-T61/T64: primary hollow on nested ``max_nodes=128`` is ≈no-op and does not
unlock dual-scale all-shell either-match (``any_all_either=false``; dirty
``b2`` can appear). Nested schedule denser (256) preserves per-shell mult
recovery in some arms. This harness freezes densify ``max_nodes=256`` and
contrasts signal vs primary (``mid=0.5``) vs mild (``mid=0.65``) hollow under
global-sigma dual-scale PH, asking whether denser scaffolding + hollow unlocks
shell voids ``(1,0,1)``.
...
@dataclass(frozen=True)
class NestedDensifyHollowBundle:
    dataset_seed: int
    stage1_seed: int
    max_nodes: int
    n_signal: int
    ...
    coarse_mult: float
    fine_mult: float
    signal_arm: NestedDensifyHollowArm
    hollow_arms: tuple[NestedDensifyHollowArm, ...]
    any_all_either: bool
...
def nested_densify_hollow_bundle() -> NestedDensifyHollowBundle:
    """Fit nested max_nodes=256; dual-scale signal vs primary/mild hollow."""
    ...
    def _dual_arm(
        ...
        *,
        ...
        dual = run_dual_scale_per_region_ph(
            ...
            scenario=f"nested_densify256_hollow_{name}_dual",
            include_labels=[1, 2],
            max_dim=2,
    ...
    hollow_arms: list[NestedDensifyHollowArm] = []
    ...
    return NestedDensifyHollowBundle(
...
def test_nested_densify_hollow_harness_lands(
    nested_densify_hollow_bundle,
) -> None:
    """Nested densify256×hollow lands; SI defaults untouched."""
    bundle = nested_densify_hollow_bundle
...
@pytest.mark.scenario
@pytest.mark.synthetic
def test_nested_densify_hollow_documents_gap(
    nested_densify_hollow_bundle,
) -> None:
    """Document densify256×hollow vs nested void recovery; never flip awaiting.

    Soft: all-shell either-match under any arm is proposal-path evidence.
    Otherwise keep documenting denser+hollow non-recovery (T61/T64 gap).
    """
    bundle = nested_densify_hollow_bundle
