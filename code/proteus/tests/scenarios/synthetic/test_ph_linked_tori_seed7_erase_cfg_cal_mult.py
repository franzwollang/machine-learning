"""Seed7 erase-cfg (mid0.35×h0≥0.5) × cal-mult (#41 / A4-T82).

A4-T79: seed7 densify256 hollow mid×h0×lifetime left ``any_clean_b2=false``;
erase cfgs ``mid=0.35 × h0∈{0.5,0.7}`` zeroed dirty-b2 (``max_b2=0``) without
unlocking ``(1,2,1)``. This harness freezes those erase cfgs and crosses a
lifetime×cal-mult grid (SI + cal ladder), asking whether cal-mult after erase
unlocks clean ``b2`` or only preserves the erased void gap.
...
# T79 erase band: mid=0.35 with h0≥0.5 wiped dirty-b2.
ERASE_CFGS: tuple[tuple[float, float], ...] = ((0.35, 0.5), (0.35, 0.7))
FRAC_GRID: tuple[float, ...] = (0.5, 2.0, 4.0, 8.0)
MULT_ARMS: tuple[float, ...] = (1.5, 2.0, 3.0, 4.0, 6.0)
...
class Seed7EraseCfgCalMultRow:
    ...
    max_b1: int
    max_b2: int
    full_cells: tuple[tuple[int, float, float, tuple[int, ...]], ...]
    clean_cells: tuple[tuple[int, float, float, tuple[int, ...]], ...]
    dirty_cells: tuple[tuple[int, float, float, tuple[int, ...]], ...]


@dataclass(frozen=True)
class Seed7EraseCfgCalMultBundle:
    dataset_seed: int
    stage1_seed: int
    max_nodes: int
    n_signal: int
    ...
    fracs: tuple[float, ...]
    mults: tuple[float, ...]
    rows: tuple[Seed7EraseCfgCalMultRow, ...]
    any_full_match: bool
    any_clean_b2: bool
    any_dirty_b2: bool
    max_b1: int
    ...
    full_cells: tuple[tuple[float, float, int, float, float, tuple[int, ...]], ...]
    clean_cells: tuple[tuple[float, float, int, float, float, tuple[int, ...]], ...]
    erase_fixed_max_b2: int


@pytest.fixture(scope="module")
def linked_tori_seed7_erase_cfg_cal_mult_bundle() -> Seed7EraseCfgCalMultBundle:
    """Fit seed7 denser256; erase hollow cfgs × lifetime×cal-mult."""
    ...
    signal_fixed = run_per_region_ph(
        ...
        signal_labs,
        sigma,
        scenario="linked_tori_seed7_erase_cal_signal_fixed",
        include_labels=[0, 1],
    ...
    rows: list[Seed7EraseCfgCalMultRow] = []
    all_full: list[tuple[float, float, int, float, float, tuple[int, ...]]] = []
    all_clean: list[tuple[float, float, int, float, float, tuple[int, ...]]] = []
    any_full = False
    any_clean = False
    any_dirty = False
    max_b1 = 0
    ...
    for mid, h0 in ERASE_CFGS:
        ...
        fixed = run_per_region_ph(
            ...
            hollow_labs,
            sigma,
            scenario=f"seed7_erase_mid{mid}_h0{h0}_fixed",
            include_labels=[0, 1],
            reading="fixed_threshold",
            max_dim=2,
            filtration_mult=FILTRATION_MULTIPLIER,
        ...
        grid = sweep_lifetime_mult_grid_per_region(
            ...
            fracs=FRAC_GRID,
            mults=MULT_ARMS,
            scenario=f"seed7_erase_mid{mid}_h0{h0}_life_cal",
            include_labels=[0, 1],
            max_dim=2,
            expected_betti=EXPECTED_TORI,
            b1_target=2,
        ...
        row_full_cells: list[tuple[int, float, float, tuple[int, ...]]] = []
        row_clean_cells: list[tuple[int, float, float, tuple[int, ...]]] = []
        row_dirty_cells: list[tuple[int, float, float, tuple[int, ...]]] = []
        row_full = False
        row_clean = False
        row_dirty = False
        row_max_b1 = 0
        ...
        for cell in grid.rows:
            ...
            if betti == EXPECTED_TORI:
                row_full = True
                any_full = True
                tup = (rid, frac, mult, betti)
                row_full_cells.append(tup)
                all_full.append(
                    (float(mid), float(h0), rid, frac, mult, betti)
                )
            if _is_clean_b2(betti):
                row_clean = True
                any_clean = True
                tup = (rid, frac, mult, betti)
                row_clean_cells.append(tup)
                all_clean.append(
                    (float(mid), float(h0), rid, frac, mult, betti)
                )
            if _is_dirty_b2(betti):
                row_dirty = True
                any_dirty = True
                row_dirty_cells.append((rid, frac, mult, betti))
    ...
    return Seed7EraseCfgCalMultBundle(
...
def test_linked_tori_seed7_erase_cfg_cal_mult_harness_lands(
    linked_tori_seed7_erase_cfg_cal_mult_bundle,
) -> None:
    """Seed7 erase-cfg×cal-mult lands; SI defaults untouched."""
    bundle = linked_tori_seed7_erase_cfg_cal_mult_bundle
...
@pytest.mark.scenario
@pytest.mark.synthetic
def test_linked_tori_seed7_erase_cfg_cal_mult_documents_gap(
    linked_tori_seed7_erase_cfg_cal_mult_bundle,
) -> None:
    """Document erase-cfg×cal-mult vs clean (1,2,1); never flip awaiting.

    Soft: any full ``(1,2,1)`` or clean ``b2`` is proposal-path evidence.
    Otherwise keep documenting erase≠recover under cal-mult.
    """
    bundle = linked_tori_seed7_erase_cfg_cal_mult_bundle
    if bundle.any_full_match or bundle.any_clean_b2:
        ...
        assert bundle.signal_fixed_dirty_b2 is True
        # Erase cfgs may keep max_b2=0 (T79) or reintroduce dirty under cal.
