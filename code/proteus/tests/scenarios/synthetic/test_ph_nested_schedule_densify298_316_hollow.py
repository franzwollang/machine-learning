"""Nested schedule {1:3,2:6} × densify{298,316} × hollow cliff fine-pin (#41 / A4-T104).

Cliff map (prior): 296full / 300mild / 304mild / 306mild / 308full / 310full / 312fail.
This harness fine-pins remaining gaps at ``max_nodes∈{298,316}``
(between 296full→300mild and 314fail→beyond) under signal vs primary/mild hollow.
...
MAX_NODES_GRID: tuple[int, ...] = (298, 316)
DATASET_SEED: int = 21
STAGE1_SEED: int = 77
...
@dataclass(frozen=True)
class NestedScheduleDensify298316HollowBundle:
    dataset_seed: int
    stage1_seed: int
    ...
    cliff_onset_between_296_and_300: bool
    table: str
    ...
    def _scheduled_arm(
        ...
        *,
        ...
        scheduled = run_scheduled_mult_per_region_ph(
            ...
            scenario=f"nested_schedule_densify{max_nodes}_cliff298_316_hollow_{name}",
...
def nested_schedule_densify298_316_hollow_bundle() -> (
    NestedScheduleDensify298316HollowBundle
):
    """Fit nested schedule×hollow at densify 298 and 316."""
    ...
    # Cliff pin: neither rung recovers between 296full and 300mild / past 314fail.
    ...
    return NestedScheduleDensify298316HollowBundle(
        ...
        cliff_onset_between_296_and_300=cliff,
        table="\n".join(table_lines),
    )
...
def test_nested_schedule_densify298_316_hollow_harness_lands(
    nested_schedule_densify298_316_hollow_bundle,
) -> None:
    """Nested schedule×densify298/316×hollow lands; SI defaults untouched."""
    bundle = nested_schedule_densify298_316_hollow_bundle
...
@pytest.mark.scenario
@pytest.mark.synthetic
def test_nested_schedule_densify298_316_hollow_documents_gap(
    nested_schedule_densify298_316_hollow_bundle,
) -> None:
    """Document densify298/316 vs cliff map 296full/300mild/302mild/304mild/312fail/314fail.
    ...
    if bundle.any_signal_recovers or bundle.any_all_match:
        ...
        assert bundle.cliff_onset_between_296_and_300 is True
        for r in bundle.rungs:
