"""Circle tissue=0.08 cal4 neighborhood × extra-high noise collapse (#41 / A4-T90).

A4-T87: cal4×frac3 pin survived noise through 0.20 (pin 3/3; neighborhood 26/27).
This harness freezes the same proposal-path neighborhood
``mult∈{3,4,5}`` × ``frac∈{2.5,3.0,3.5}`` and crosses an extra-high noise ladder
``{0.22, 0.25, 0.30}`` at fixed tissue=0.08, asking where the clean pin collapses.
...
# Higher noise than T87's {0.15,0.18,0.20} — probe collapse beyond 0.20.
NOISE_GRID: tuple[float, ...] = (0.22, 0.25, 0.30)
EXPECTED_B1: int = 1
PIN_MULT: float = 4.0
PIN_FRAC: float = 3.0
...
class CircleTissue08Cal4NoiseXXHighRow:
    noise: float
...
@dataclass(frozen=True)
class CircleTissue08Cal4NoiseXXHighBundle:
    dataset_seed: int
    stage1_seed: int
    ...
    rows: tuple[CircleTissue08Cal4NoiseXXHighRow, ...]
...
def circle_tissue08_cal4_noise_xxhigh_bundle() -> CircleTissue08Cal4NoiseXXHighBundle:
    """Cal4 neighborhood × extra-high-noise ladder at tissue=0.08."""
    rows: list[CircleTissue08Cal4NoiseXXHighRow] = []
    ...
    for noise in NOISE_GRID:
        ...
        for mult in MULT_ARMS:
            for frac in FRAC_GRID:
                ...
                rows.append(
                    CircleTissue08Cal4NoiseXXHighRow(
                        noise=float(noise),
                        filtration_mult=float(mult),
                        lifetime_frac=float(frac),
                        n_signal=n_signal,
    ...
    return CircleTissue08Cal4NoiseXXHighBundle(
...
def test_circle_tissue08_cal4_noise_xxhigh_harness_lands(
    circle_tissue08_cal4_noise_xxhigh_bundle,
) -> None:
    """Tissue0.08 cal4×extra-high-noise lands; SI defaults untouched."""
    bundle = circle_tissue08_cal4_noise_xxhigh_bundle
...
@pytest.mark.scenario
@pytest.mark.synthetic
def test_circle_tissue08_cal4_noise_xxhigh_documents_gap(
    circle_tissue08_cal4_noise_xxhigh_bundle,
) -> None:
    """Document cal4×frac3 clean pin vs extra-high noise; never flip awaiting.

    Soft: any pin/neighborhood clean at noise>0.20 is proposal-path only.
    Otherwise document collapse relative to T87 noise≤0.20 survival.
    """
    bundle = circle_tissue08_cal4_noise_xxhigh_bundle
