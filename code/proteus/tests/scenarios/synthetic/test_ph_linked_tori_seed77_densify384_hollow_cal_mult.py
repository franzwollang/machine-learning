"""Densify max_nodes=384 × hollow × lifetime×cal-mult (#41 / A4-T65-followon).

A4-T50/T59: densify384 regresses to ``(1,1,0)`` (max_b2=0); primary/mild hollow
does not restore both-partial. A4-T62: signal-only lifetime×cal-mult at 384
raises ``b1`` (max_b1=5) but never unlocks void (max_b2=0). A4-T49: hollow×
lifetime×cal-mult on denser256 cleaned neither dirty-b2 nor full ``(1,2,1)``.

This harness freezes densify ``max_nodes=384``, applies primary hollow
(``mid=0.5/h0=0.7/noGab``), then crosses ``lifetime_frac × filtration_mult``
asking whether the mid-regime + hollow + cal-mult combo unlocks ``b2`` or
clean ``(1,2,1)``.
...
class Seed77Densify384HollowCalMultBundle:
    dataset_seed: int
    stage1_seed: int
    max_nodes: int
    n_signal: int
...
def linked_tori_seed77_densify384_hollow_cal_mult_bundle() -> (
    Seed77Densify384HollowCalMultBundle
):
    """Fit seed77 max_nodes=384; hollow-prune; lifetime×cal-mult grid."""
    ...
    hollow_fixed = run_per_region_ph(
        ...
        h_labs,
        sigma,
        scenario="linked_tori_seed77_m384_hollow_fixed",
        include_labels=[0, 1],
        reading="fixed_threshold",
        max_dim=2,
        filtration_mult=FILTRATION_MULTIPLIER,
    ...
    grid = sweep_lifetime_mult_grid_per_region(
        ...
        fracs=FRAC_GRID,
        mults=MULT_GRID,
        scenario="linked_tori_seed77_m384_hollow_lifetime_cal_mult",
        include_labels=[0, 1],
        max_dim=2,
        expected_betti=EXPECTED_TORI,
        b1_target=2,
    ...
    return Seed77Densify384HollowCalMultBundle(
        dataset_seed=DATASET_SEED,
        stage1_seed=STAGE1_SEED,
        max_nodes=MAX_NODES,
        n_signal=int(signal_pos.shape[0]),
        n_after_hollow=n_after if not hollow_fallback else int(signal_pos.shape[0]),
...
def test_linked_tori_seed77_densify384_hollow_cal_mult_harness_lands(
    linked_tori_seed77_densify384_hollow_cal_mult_bundle,
) -> None:
    """Seed77 densify384×hollow×cal-mult lands; SI defaults untouched."""
    bundle = linked_tori_seed77_densify384_hollow_cal_mult_bundle
...
@pytest.mark.scenario
@pytest.mark.synthetic
def test_linked_tori_seed77_densify384_hollow_cal_mult_documents_gap(
    linked_tori_seed77_densify384_hollow_cal_mult_bundle,
) -> None:
    """Document densify384×hollow×cal-mult vs clean (1,2,1); never flip awaiting.

    Soft: any full ``(1,2,1)`` or clean ``b2`` is proposal-path evidence.
    Otherwise keep documenting mid-regime hollow+cal-mult gap.
    """
    bundle = linked_tori_seed77_densify384_hollow_cal_mult_bundle
