"""Circle tissue=0.12 cal/mult3 neighborhood × noise pin alt (#41 / A4-T93).

A4-T87: tissue0.08 cal4×frac3 pin survived noise through 0.20.
A4-T90: that pin collapsed for all noise>0.20; only residual clean cells were
``mult=3 × frac∈{2.5,3,3.5} @ noise=0.22``. This harness raises tissue to
``0.12`` and freezes the same ``mult∈{3,4,5} × frac∈{2.5,3,3.5}`` neighborhood
across ``noise∈{0.15,0.20,0.22}``, asking whether the mult3 residual (or the
cal4 pin) survives the tissue bump — proposal-path only.
...
# Bridge T87 pin band (≤0.20) and T90 residual (0.22).
NOISE_GRID: tuple[float, ...] = (0.15, 0.20, 0.22)
EXPECTED_B1: int = 1
PIN_MULT: float = 4.0
PIN_FRAC: float = 3.0
...
class CircleTissue12CalMult3NoiseRow:
    noise: float
...
@dataclass(frozen=True)
class CircleTissue12CalMult3NoiseBundle:
    dataset_seed: int
    stage1_seed: int
    ...
    rows: tuple[CircleTissue12CalMult3NoiseRow, ...]
...
def circle_tissue12_cal_mult3_noise_bundle() -> CircleTissue12CalMult3NoiseBundle:
    """Cal/mult3 neighborhood × noise ladder at tissue=0.12."""
    rows: list[CircleTissue12CalMult3NoiseRow] = []
    ...
    for noise in NOISE_GRID:
        ...
        for mult in MULT_ARMS:
            for frac in FRAC_GRID:
                ...
                rows.append(
                    CircleTissue12CalMult3NoiseRow(
                        noise=float(noise),
                        filtration_mult=float(mult),
                        lifetime_frac=float(frac),
                        n_signal=n_signal,
    ...
    return CircleTissue12CalMult3NoiseBundle(
...
def test_circle_tissue12_cal_mult3_noise_harness_lands(
    circle_tissue12_cal_mult3_noise_bundle,
) -> None:
    """Tissue0.12 cal/mult3×noise lands; SI defaults untouched."""
    bundle = circle_tissue12_cal_mult3_noise_bundle
...
@pytest.mark.scenario
@pytest.mark.synthetic
def test_circle_tissue12_cal_mult3_noise_documents_gap(
    circle_tissue12_cal_mult3_noise_bundle,
) -> None:
    """Document tissue0.12 pin/alt vs noise; never flip awaiting.

    Soft: any pin or mult3-alt clean is proposal-path only.
    Otherwise document collapse relative to tissue0.08 T87/T90.
    """
    bundle = circle_tissue12_cal_mult3_noise_bundle
