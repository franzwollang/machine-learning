"""Nested schedule {1:3,2:6} × densify{306,310} × hollow cliff fine-pin (#41 / A4-T98).

Cliff map (prior): 296full / 300mild / 304mild / 308full / 312fail.
This harness fine-pins the remaining gaps at ``max_nodes∈{306,310}``
between 304 mild and 308 win / 312 fail under signal vs primary/mild hollow.
...
MAX_NODES_GRID: tuple[int, ...] = (306, 310)
DATASET_SEED: int = 21
STAGE1_SEED: int = 77
...
@dataclass(frozen=True)
class NestedScheduleDensify306310HollowBundle:
    dataset_seed: int
    stage1_seed: int
    ...
    def _scheduled_arm(
        ...
        *,
        ...
        scheduled = run_scheduled_mult_per_region_ph(
            ...
            scenario=f"nested_schedule_densify{max_nodes}_cliff306_310_hollow_{name}",
...
def nested_schedule_densify306_310_hollow_bundle() -> (
    NestedScheduleDensify306310HollowBundle
):
    """Fit nested schedule×hollow at densify 306 and 310."""
    ...
    return NestedScheduleDensify306310HollowBundle(
...
def test_nested_schedule_densify306_310_hollow_harness_lands(
    nested_schedule_densify306_310_hollow_bundle,
) -> None:
    """Nested schedule×densify306/310×hollow lands; SI defaults untouched."""
    bundle = nested_schedule_densify306_310_hollow_bundle
...
@pytest.mark.scenario
@pytest.mark.synthetic
def test_nested_schedule_densify306_310_hollow_documents_gap(
    nested_schedule_densify306_310_hollow_bundle,
) -> None:
    """Document densify306/310 vs cliff map 304mild/308full/312fail.

    Soft: any rung recovering all-match fills the non-mono cliff map.
    Otherwise document hard fail between 304 mild and 308/312.
    Never flip awaiting.
    """
    bundle = nested_schedule_densify306_310_hollow_bundle
