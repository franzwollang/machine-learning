"""Seed7 erase × bridge_critical / soft bridge_mass (#41 / A4-T100).

A4-T94: erase×mst/soft_capacity (betweenness) reintroduced dirty-b2 and never
unlocked clean ``(1,2,1)``. A4-T97: mid×h0 retune under ungated hollow also
never unlocked clean void (max_b2=1 dirty-only). This harness freezes baseline
erase plus the T91/T94 dirty-reintro cfg and crosses
``bridge_critical_only`` / ``soft_capacity_method=bridge_mass`` (frac grid)
intersections, asking whether graph-bridge or min-cut-mass gating unlocks
clean void Betti after erase.
...
CAPACITY_MODES: tuple[tuple[bool, bool, float, str], ...] = (
    ...
    (False, True, 0.10, "bridge_mass"),
    (False, True, 0.25, "bridge_mass"),
...
@dataclass(frozen=True)
class Seed7EraseBridgeMassBundle:
    dataset_seed: int
    stage1_seed: int
    max_nodes: int
    n_signal: int
    ...
    rows: tuple[Seed7EraseBridgeMassRow, ...]
    any_full_match: bool
    any_clean_b2: bool
    any_dirty_b2: bool
    max_b1: int
...
def linked_tori_seed7_erase_cfg_bridge_mass_bundle() -> (
    Seed7EraseBridgeMassBundle
):
    """Fit seed7 denser256; erase × bridge_critical / soft bridge_mass."""
    ...
    signal_fixed = run_per_region_ph(
        ...
        signal_labs,
        sigma_star,
        scenario="linked_tori_seed7_erase_bridge_mass_signal_fixed",
        include_labels=[0, 1],
    ...
    rows: list[Seed7EraseBridgeMassRow] = []
    any_full = False
    any_clean = False
    any_dirty = False
    max_b1 = 0
    ...
    for (mid, h0, gab, min_end, conj), is_baseline in hollow_cfgs:
        for bridge, soft, soft_frac, method in CAPACITY_MODES:
            ...
            fixed = run_per_region_ph(
                ...
                sigma_star,
                scenario=(
                    f"seed7_erase_bridgemass_b{int(is_baseline)}_"
    ...
    return Seed7EraseBridgeMassBundle(
...
def test_linked_tori_seed7_erase_cfg_bridge_mass_harness_lands(
    linked_tori_seed7_erase_cfg_bridge_mass_bundle,
) -> None:
    """Seed7 erase×bridge_critical/bridge_mass lands; SI defaults untouched."""
    bundle = linked_tori_seed7_erase_cfg_bridge_mass_bundle
...
@pytest.mark.scenario
@pytest.mark.synthetic
def test_linked_tori_seed7_erase_cfg_bridge_mass_documents_gap(
    linked_tori_seed7_erase_cfg_bridge_mass_bundle,
) -> None:
    """Document erase×bridge/bridge_mass vs clean (1,2,1); never flip awaiting.

    Soft: any full ``(1,2,1)`` or clean ``b2`` is proposal-path evidence.
    Otherwise keep documenting bridge/mass gates ≠ clean recover after erase.
    """
    bundle = linked_tori_seed7_erase_cfg_bridge_mass_bundle
