...
def linked_tori_seed7_erase_cfg_soft_gab_bridge_denser_bundle() -> (
    Seed7EraseSoftGabBridgeDenserBundle
):
    """Fit seed7 denser384; erase × soft×gabriel∧H×bridge denser384."""
    ...
    signal_fixed = run_per_region_ph(
        ...
        signal_labs,
        sigma_star,
        scenario="linked_tori_seed7_erase_soft_gab_bridge_denser_signal_fixed",
        include_labels=[0, 1],
    ...
    for (mid, h0, gab, min_end, conj), is_baseline in hollow_cfgs:
        for bridge, soft, soft_frac, method in CAPACITY_MODES:
            ...
            fixed = run_per_region_ph(
                ...
                sigma_star,
                scenario=(
                    f"seed7_erase_softgab_d384_b{int(is_baseline)}_"
...
def test_linked_tori_seed7_erase_cfg_soft_gab_bridge_denser_harness_lands(
    linked_tori_seed7_erase_cfg_soft_gab_bridge_denser_bundle,
) -> None:
    """Seed7 soft×gabriel∧H×bridge denser384 lands; SI defaults untouched."""
...
@pytest.mark.scenario
@pytest.mark.synthetic
def test_linked_tori_seed7_erase_cfg_soft_gab_bridge_denser_documents_gap(
    linked_tori_seed7_erase_cfg_soft_gab_bridge_denser_bundle,
) -> None:
