"""Seed7 erase × expanded lifetime soft after gabriel dirty reintro (#41 / A4-T91).

A4-T88: erase mid/h0 with ``gabriel_fallback=False × min_end=0.5`` kept
``max_b2=0``; raising ``min_end≥1`` or enabling gabriel/conj reintroduced
**dirty-b2 only** (``max_b2=1``) — never clean ``(1,2,1)`` under SI-mult
soft fracs. This harness freezes those dirty-reintro hollow cfgs and
crosses an expanded soft ``lifetime_frac × filtration_mult`` grid, asking
whether broader lifetime soft unlocks clean void Betti after dirty reintro.
...
DATASET_SEED: int = 2
STAGE1_SEED: int = 7
# Dirty-reintro hollow cfgs from T88 (gab/me↑ reintroduces dirty-b2).
# Tuple: (mid, h0, gabriel_fallback, min_end_count, require_gabriel_and_h)
DIRTY_REINTRO_CFGS: tuple[tuple[float, float, bool, float, bool], ...] = (
    (0.35, 0.5, True, 0.25, True),   # T88 best_cfg
    (0.35, 0.5, True, 0.5, False),
    (0.35, 0.5, False, 1.0, False),
    (0.35, 0.5, False, 2.0, False),
    (0.35, 0.7, True, 0.5, False),
    (0.35, 0.7, False, 1.0, False),
)
# Baseline erase void (T85/T88 max_b2=0) for contrast.
BASELINE_ERASE: tuple[float, float, bool, float, bool] = (
    0.35, 0.5, False, 0.5, False,
)
# Expanded soft lifetime beyond T88 SI-mult-only ladder.
SOFT_FRAC_GRID: tuple[float, ...] = (0.125, 0.25, 0.5, 1.0, 2.0, 4.0)
SOFT_MULT_ARMS: tuple[float, ...] = (1.0, 1.5, 2.0, 3.0, 4.0, 6.0)
...
@dataclass(frozen=True)
class Seed7EraseLifeSoftBundle:
    dataset_seed: int
    stage1_seed: int
    max_nodes: int
    n_signal: int
    ...
    baseline_max_b2: int
    dirty_reintro_max_b2: int
    table: str


@pytest.fixture(scope="module")
def linked_tori_seed7_erase_cfg_lifetime_soft_bundle() -> (
    Seed7EraseLifeSoftBundle
):
    """Fit seed7 denser256; dirty-reintro erase × expanded lifetime soft."""
    ...
    signal_fixed = run_per_region_ph(
        ...
        signal_labs,
        sigma_star,
        scenario="linked_tori_seed7_erase_life_signal_fixed",
        include_labels=[0, 1],
    ...
    rows: list[Seed7EraseLifeSoftRow] = []
    any_full = False
    any_clean = False
    any_dirty = False
    max_b1 = 0
    max_b2 = 0
    baseline_max_b2 = 0
    dirty_reintro_max_b2 = 0
    best_cfg: tuple[float, float, bool, float, bool] | None = None
    best_score = -1
    table_lines = [
        "mid\th0\tgab\tmin_end\tconj\tbase\taxis\tkey\tregion\tbetti\tclean\tdirty"
    ]

    all_cfgs: list[tuple[tuple[float, float, bool, float, bool], bool]] = [
        (BASELINE_ERASE, True),
        *((cfg, False) for cfg in DIRTY_REINTRO_CFGS),
    ]

    for (mid, h0, gab, min_end, conj), is_baseline in all_cfgs:
        ...
        fixed = run_per_region_ph(
            ...
            sigma_star,
            scenario=(
                f"seed7_erase_life_mid{mid}_h0{h0}_"
        ...
        if is_baseline:
            baseline_max_b2 = max(baseline_max_b2, fixed_max_b2)
        else:
            dirty_reintro_max_b2 = max(dirty_reintro_max_b2, fixed_max_b2)
        ...
        for betti in fixed_betti.values():
            ...
            table_lines.append(
                ...
                f"{int(conj)}\t{int(is_baseline)}\tfixed\tSI\t-\t{betti}\t"
        ...
        soft_grid = sweep_lifetime_mult_grid_per_region(
            ...
            scenario=(
                f"seed7_erase_life_soft_mid{mid}_h0{h0}_"
        ...
        for cell in soft_grid.rows:
            ...
            table_lines.append(
                f"{mid:g}\t{h0:g}\t{int(gab)}\t{min_end:g}\t"
                f"{int(conj)}\t{int(is_baseline)}\tsoft\t{frac:g}x{mult:g}\t"
                f"{rid}\t{betti}\t{int(is_clean)}\t{int(is_dirty)}"
            )
        ...
        if not is_baseline:
            dirty_reintro_max_b2 = max(dirty_reintro_max_b2, row_max_b2)
        else:
            baseline_max_b2 = max(baseline_max_b2, row_max_b2)

        rows.append(
            Seed7EraseLifeSoftRow(
                mid_radius_frac=float(mid),
                h0=float(h0),
                gabriel_fallback=bool(gab),
                min_end_count=float(min_end),
    ...
    return Seed7EraseLifeSoftBundle(
        ...
        max_b2=max_b2,
        best_cfg=best_cfg,
        baseline_max_b2=int(baseline_max_b2),
        dirty_reintro_max_b2=int(dirty_reintro_max_b2),
        table="\n".join(table_lines),
    )
...
def test_linked_tori_seed7_erase_cfg_lifetime_soft_harness_lands(
    linked_tori_seed7_erase_cfg_lifetime_soft_bundle,
) -> None:
    """Seed7 erase×lifetime soft lands; SI defaults untouched."""
    bundle = linked_tori_seed7_erase_cfg_lifetime_soft_bundle
    ...
    assert bundle.dirty_cfgs == DIRTY_REINTRO_CFGS
    assert bundle.baseline_cfg == BASELINE_ERASE
    assert bundle.soft_fracs == SOFT_FRAC_GRID
    assert bundle.soft_mults == SOFT_MULT_ARMS
    # 1 baseline + 6 dirty-reintro
    assert len(bundle.rows) == 1 + len(DIRTY_REINTRO_CFGS)
    assert sum(1 for r in bundle.rows if r.is_baseline) == 1
...
@pytest.mark.scenario
@pytest.mark.synthetic
def test_linked_tori_seed7_erase_cfg_lifetime_soft_documents_gap(
    linked_tori_seed7_erase_cfg_lifetime_soft_bundle,
) -> None:
    """Document erase×lifetime soft vs clean (1,2,1); never flip awaiting.

    Soft: any full ``(1,2,1)`` or clean ``b2`` is proposal-path evidence.
    Otherwise keep documenting dirty-reintro ≠ clean recover under expanded
    lifetime soft.
    """
    bundle = linked_tori_seed7_erase_cfg_lifetime_soft_bundle
    if bundle.any_full_match or bundle.any_clean_b2:
        ...
        assert bundle.baseline_max_b2 >= 0
        assert bundle.dirty_reintro_max_b2 >= 0
