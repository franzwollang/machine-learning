"""Linked-tori fitted PH recipes: lifetime_frac + hollow prune (#41 / A4-T28).

Parallel to ``test_ph_nested_fitted_recipes`` (A4-T25): on a denser fitted
scaffold (``max_nodes=128``) try signal filter, lifetime_frac sweep, and
hollow-pruned neighbour edges toward per-torus SI ``(1, 2, 1)``.

Evidence-gathering only — does **not** flip ``test_linked_tori_betti_numbers``
``@awaiting`` or change SI defaults. Signal labels are ``0/1`` (tissue ``-1``).
"""
...
@dataclass(frozen=True)
class LinkedToriRecipeBundle:
    max_nodes: int
    n_nodes: int
    sigma_star: float
    rows: tuple[LinkedToriRecipeRow, ...]
    lifetime_sweep_any_match: bool


@pytest.fixture(scope="module")
def linked_tori_fitted_recipe_bundle() -> LinkedToriRecipeBundle:
    ...
    # Signal tori are labels 0 and 1; tissue = -1.
    ...
    def _row(
        ...
        *,
        ...
        report = run_per_region_ph(
            ...
            labs,
            [sigma, sigma],
            scenario=f"linked_tori_recipe_{name}",
            include_labels=[0, 1],
...
def test_linked_tori_fitted_recipes_report_table(
    linked_tori_fitted_recipe_bundle,
) -> None:
    """Recipe table lands with signal + lifetime + hollow variants; SI intact."""
    bundle = linked_tori_fitted_recipe_bundle
    ...
    for row in bundle.rows:
        ...
        for b in row.betti_per_torus.values():
            assert len(b) == 3
...
@pytest.mark.scenario
@pytest.mark.synthetic
def test_linked_tori_fitted_recipes_document_si_gap(
    linked_tori_fitted_recipe_bundle,
) -> None:
    """Document whether any recipe recovers per-torus (1,2,1); never flip awaiting.
