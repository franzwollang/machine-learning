"""Stage-1 seed=7 hollow prune on dirty-b2 scaffold (#41 / A4-T51-followon).

A4-T44/T48: denser seed2 / max_nodes=256 / Stage-1 seed=7 yields dirty
``b2`` on torus0 as ``(2,1,1)``; filtration×lifetime cannot clean to
``(1,2,1)``. This harness applies primary hollow-edge prune
(``mid=0.5/h0=0.7/noGab``) then lifetime-frac sweep asking whether hollow
cleans or worsens the dirty void signal.
...
def linked_tori_seed7_hollow_bundle() -> Seed7HollowBundle:
    """Fit seed7 denser256; contrast signal vs hollow lifetime sweeps."""
    ...
    signal_fixed = run_per_region_ph(
        ...
        signal_labs,
        sigma,
        scenario="linked_tori_seed7_signal_fixed",
        include_labels=[0, 1],
    ...
    hollow_fixed = run_per_region_ph(
        ...
        hollow_labs,
        sigma,
        scenario="linked_tori_seed7_hollow_fixed",
        include_labels=[0, 1],
        reading="fixed_threshold",
        max_dim=2,
        filtration_mult=FILTRATION_MULTIPLIER,
    ...
    (
        sig_full, _, sig_max_b1, sig_max_b2, _, sig_clean, sig_dirty,
    ) = _summarize_lifetime_rows(signal_rows)
    (
        hol_full, _, hol_max_b1, hol_max_b2, hol_rec, hol_clean, hol_dirty,
...
def test_linked_tori_seed7_hollow_harness_lands(
    linked_tori_seed7_hollow_bundle,
) -> None:
    """Seed7 hollow contrast lands; SI defaults untouched."""
    bundle = linked_tori_seed7_hollow_bundle
    ...
    # T44/T48 baseline: seed7 fixed reading carries dirty b2 on torus0.
...
@pytest.mark.scenario
@pytest.mark.synthetic
def test_linked_tori_seed7_hollow_documents_gap(
    linked_tori_seed7_hollow_bundle,
) -> None:
    """Document seed7 hollow vs clean (1,2,1); never flip awaiting.

    Soft: any full ``(1,2,1)`` or clean ``b2`` is proposal-path evidence.
    Otherwise keep documenting dirty-only / hollow gap. Follow-on ≠ SI recovery.
    """
    bundle = linked_tori_seed7_hollow_bundle
    if bundle.any_full_match or bundle.any_clean_b2:
        ...
        # Dirty may persist under hollow or be erased; baseline dirty fixed.
        assert bundle.signal_fixed_dirty_b2
