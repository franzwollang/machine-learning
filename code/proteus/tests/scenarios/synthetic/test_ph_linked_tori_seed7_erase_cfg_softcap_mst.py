"""Seed7 erase × soft_capacity / MST after lifetime soft dirty-only (#41 / A4-T94).

A4-T91: erase×expanded lifetime soft after gabriel dirty reintro still
``any_clean_b2=false`` / ``any_full=false`` with ``max_b2=1`` dirty-only —
never clean ``(1,2,1)``. This harness freezes baseline erase plus the T91
best dirty-reintro cfg and crosses ``mst_critical_only`` /
``soft_capacity_only`` (betweenness frac grid) intersections, asking whether
capacity/MST gating unlocks clean void Betti after erase.
...
# Capacity gate modes: (mst, soft, soft_frac). soft_frac ignored when soft=False.
CAPACITY_MODES: tuple[tuple[bool, bool, float], ...] = (
    (False, False, 0.25),   # ungated contrast
    (True, False, 0.25),    # MST-only
    (False, True, 0.10),
    (False, True, 0.25),
    (False, True, 0.50),
    (True, True, 0.25),     # MST ∩ soft@0.25
)
...
@dataclass(frozen=True)
class Seed7EraseSoftCapMstBundle:
    dataset_seed: int
    stage1_seed: int
    max_nodes: int
    n_signal: int
    ...
    rows: tuple[Seed7EraseSoftCapMstRow, ...]
    any_full_match: bool
    any_clean_b2: bool
    any_dirty_b2: bool
    max_b1: int
    max_b2: int
    best_mode: tuple[bool, bool, float, bool] | None
...
def linked_tori_seed7_erase_cfg_softcap_mst_bundle() -> (
    Seed7EraseSoftCapMstBundle
):
    """Fit seed7 denser256; erase × soft_capacity/MST capacity gates."""
    ...
    signal_fixed = run_per_region_ph(
        ...
        signal_labs,
        sigma_star,
        scenario="linked_tori_seed7_erase_softcap_signal_fixed",
        include_labels=[0, 1],
    ...
    rows: list[Seed7EraseSoftCapMstRow] = []
    any_full = False
    any_clean = False
    any_dirty = False
    max_b1 = 0
    ...
    best_mode: tuple[bool, bool, float, bool] | None = None
    best_score = -1
    table_lines = [
        "base\tmst\tsoft\tfrac\tn_after\taxis\tkey\tregion\tbetti\tclean\tdirty"
    ]
    ...
    for (mid, h0, gab, min_end, conj), is_baseline in hollow_cfgs:
        for mst, soft, soft_frac in CAPACITY_MODES:
            ...
            fixed = run_per_region_ph(
                ...
                sigma_star,
                scenario=(
                    f"seed7_erase_softcap_b{int(is_baseline)}_"
                    f"mst{int(mst)}_s{int(soft)}_f{soft_frac:g}_fixed"
                ),
                include_labels=[0, 1],
                reading="fixed_threshold",
                max_dim=2,
            ...
            for betti in fixed_betti.values():
                ...
                if betti == EXPECTED_TORI:
                    ...
                    any_full = True
                table_lines.append(
                    f"{int(is_baseline)}\t{int(mst)}\t{int(soft)}\t"
                    f"{soft_frac:g}\t{n_after}\tfixed\tSI\t-\t{betti}\t"
            ...
            soft_grid = sweep_lifetime_mult_grid_per_region(
                ...
                scenario=(
                    f"seed7_erase_softcap_b{int(is_baseline)}_"
                    f"mst{int(mst)}_s{int(soft)}_f{soft_frac:g}_soft"
                ),
                include_labels=[0, 1],
                max_dim=2,
                expected_betti=EXPECTED_TORI,
            ...
            for cell in soft_grid.rows:
                ...
                table_lines.append(
                    f"{int(is_baseline)}\t{int(mst)}\t{int(soft)}\t"
                    f"{soft_frac:g}\t{n_after}\tsoft\t{frac:g}x{mult:g}\t"
    ...
    return Seed7EraseSoftCapMstBundle(
...
def test_linked_tori_seed7_erase_cfg_softcap_mst_harness_lands(
    linked_tori_seed7_erase_cfg_softcap_mst_bundle,
) -> None:
    """Seed7 erase×soft_capacity/MST lands; SI defaults untouched."""
    bundle = linked_tori_seed7_erase_cfg_softcap_mst_bundle
    ...
    assert "mst" in header and "soft" in header and "frac" in header


@pytest.mark.scenario
@pytest.mark.synthetic
def test_linked_tori_seed7_erase_cfg_softcap_mst_documents_gap(
    linked_tori_seed7_erase_cfg_softcap_mst_bundle,
) -> None:
    """Document erase×soft_capacity/MST vs clean (1,2,1); never flip awaiting.

    Soft: any full ``(1,2,1)`` or clean ``b2`` is proposal-path evidence.
    Otherwise keep documenting capacity gates ≠ clean recover after erase.
    """
    bundle = linked_tori_seed7_erase_cfg_softcap_mst_bundle
