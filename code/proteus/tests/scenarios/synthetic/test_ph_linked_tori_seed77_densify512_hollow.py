"""Densify max_nodes=512 × hollow prune on seed77 (#41 / A4-T54-followon).

A4-T50: seed77 densify ladder keeps both-tori partial ``(1,2,0)`` at 512
(max_b2=0; 384 regresses). A4-T47/T52: primary hollow on denser256 unlocks
dirty ``b2``. This harness applies primary hollow
(``mid=0.5/h0=0.7/noGab``) on the **512-node** seed77 scaffold and sweeps
lifetime asking whether denser+hollow reaches clean ``(1,2,1)``.
...
class Seed77Densify512HollowBundle:
    dataset_seed: int
    stage1_seed: int
    max_nodes: int
    n_signal: int
...
def linked_tori_seed77_densify512_hollow_bundle() -> Seed77Densify512HollowBundle:
    """Fit seed77 max_nodes=512; contrast signal vs primary hollow lifetime."""
    ...
    signal_fixed = run_per_region_ph(
        ...
        signal_labs,
        sigma,
        scenario="linked_tori_seed77_m512_signal_fixed",
        include_labels=[0, 1],
    ...
    hollow_fixed = run_per_region_ph(
        ...
        hollow_labs,
        sigma,
        scenario="linked_tori_seed77_m512_hollow_fixed",
        include_labels=[0, 1],
        reading="fixed_threshold",
        max_dim=2,
        filtration_mult=FILTRATION_MULTIPLIER,
    ...
    return Seed77Densify512HollowBundle(
        ...
        both_tori_partial_signal=both_partial,
        fracs=FRAC_GRID,
...
def test_linked_tori_seed77_densify512_hollow_harness_lands(
    linked_tori_seed77_densify512_hollow_bundle,
) -> None:
    """Seed77 densify512×hollow lands; SI defaults untouched."""
    bundle = linked_tori_seed77_densify512_hollow_bundle
    ...
    # T50: 512 keeps both-tori partial interlocking on signal fixed.
    assert bundle.both_tori_partial_signal


@pytest.mark.scenario
@pytest.mark.synthetic
def test_linked_tori_seed77_densify512_hollow_documents_gap(
    linked_tori_seed77_densify512_hollow_bundle,
) -> None:
    """Document densify512×hollow vs clean (1,2,1); never flip awaiting.

    Soft: any full ``(1,2,1)`` or clean ``b2`` is proposal-path evidence.
    Otherwise keep documenting dirty-only / densify+hollow gap.
    """
    bundle = linked_tori_seed77_densify512_hollow_bundle
