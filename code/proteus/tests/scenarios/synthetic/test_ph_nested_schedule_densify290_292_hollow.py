"""Nested schedule {1:3,2:6} × densify{290,292} × hollow cliff fine-pin (#41 / A4-T110).

Cliff map (prior): 288full / 294fail / 296full / 298full / … / 318fail.
This harness fine-pins the NON-MONO gap between 288full and 294fail at
``max_nodes∈{290,292}`` under signal vs primary/mild hollow.
...
MAX_NODES_GRID: tuple[int, ...] = (290, 292)
DATASET_SEED: int = 21
STAGE1_SEED: int = 77
...
@dataclass(frozen=True)
class NestedScheduleDensify290292HollowBundle:
    dataset_seed: int
    stage1_seed: int
    ...
    def _scheduled_arm(
        ...
        *,
        ...
        scheduled = run_scheduled_mult_per_region_ph(
            ...
            scenario=f"nested_schedule_densify{max_nodes}_cliff290_292_hollow_{name}",
...
def nested_schedule_densify290_292_hollow_bundle() -> (
    NestedScheduleDensify290292HollowBundle
):
    """Fit nested schedule×hollow at densify 290 and 292."""
    ...
    # Cliff pin: neither rung recovers between 288full and 294fail gap.
    ...
    return NestedScheduleDensify290292HollowBundle(
...
def test_nested_schedule_densify290_292_hollow_harness_lands(
    nested_schedule_densify290_292_hollow_bundle,
) -> None:
    """Nested schedule×densify290/292×hollow lands; SI defaults untouched."""
    bundle = nested_schedule_densify290_292_hollow_bundle
...
@pytest.mark.scenario
@pytest.mark.synthetic
def test_nested_schedule_densify290_292_hollow_documents_gap(
    nested_schedule_densify290_292_hollow_bundle,
) -> None:
    """Document densify290/292 vs cliff map 288full/294fail/296full/…/318fail.
