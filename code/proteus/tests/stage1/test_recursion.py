...
def test_soft_x_proposed_h0_nested_tori_ari() -> None:
    """#44 / A2-T44-followon: soft×proposed h0 majors+sample-ARI.

    Proposed Youden/Poisson-LR h0 alone keeps chance-ARI K=2 on nested/
    tori. Soft×proposed collapses nested≤1 but retains tori K=2 ARI≈0.26
    — still **not** sample-ARI recovery. Flags default-off; no awaiting flip.
    """
    ...
    cfgs = {
        ...
        "youden_a4": proposed_h0_calibrated_config("youden_a4"),
        "poisson_lr": proposed_h0_calibrated_config("poisson_lr"),
        "soft_x_youden": proposed_h0_calibrated_config(
            "youden", soft_capacity_only=True, soft_capacity_frac=0.25,
        ),
        "soft_x_youden_a4": proposed_h0_calibrated_config(
            "youden_a4", soft_capacity_only=True, soft_capacity_frac=0.25,
        ),
        "soft_x_poisson_lr": proposed_h0_calibrated_config(
            "poisson_lr", soft_capacity_only=True, soft_capacity_frac=0.25,
        ),
    }
    ...
    for mode, cfg in cfgs.items():
        ...
        exp_nm, exp_na, exp_tm, exp_ta = SOFT_X_PROPOSED_H0_TABLE[mode]
        assert nm == exp_nm
        assert tm == exp_tm
        if exp_na is not None:
            assert na is not None and abs(na - exp_na) < 0.08
    ...
    assert cfgs["soft_x_youden"].soft_capacity_only is True
...
def test_denser_proposed_h0_nested_tori_ari() -> None:
    ...
    collapse both ≤1. Not sample-ARI recovery; flags off; no awaiting flip.
    """
    ...
    nested = make_nested_spheres(
        n_per_sphere=DENSER_PROPOSED_H0_NESTED_N, extrusion_dim=1, seed=0,
    )
    tori = make_linked_tori(n_per_torus=DENSER_PROPOSED_H0_TORI_N, seed=0)

    def _adapt(points, tau: float):
        sc = Stage1Scaffold(
            dim=int(points.shape[1]), tau=float(tau), k=8,
            max_nodes=DENSER_PROPOSED_H0_MAX_NODES,
            ann_backend="naive", rng=np.random.default_rng(0),
        )
        sc.init_from(points, n_seeds=8)
        sc.run_until_stable(
    ...
    for mode, cfg in cfgs.items():
        ...
        exp_nm, exp_na, exp_tm, exp_ta = DENSER_PROPOSED_H0_TABLE[mode]
        assert nm == exp_nm
        assert tm == exp_tm
        if exp_na is not None:
            assert na is not None and abs(na - exp_na) < 0.08
...
def test_soft_frac_x_youden_seed_inflate_nested_tori_ari() -> None:
    """#44 / A2-T47: soft_frac×Youden seed1 nested-inflate mechanism.

    Soft_frac grid × seeds 0..2 under Youden h0≈0.73: seed1 inflate is
    frac-windowed (soft_frac∈{0.1,0.25,0.5} → nested K=2 ARI≈0.05–0.08;
    frac≥0.75 ≤1); seed0/2 never inflate. Not sample-ARI recovery;
    flags default-off; no awaiting flip.
    """
    ...
    for seed in SOFT_FRAC_X_YOUDEN_SEED_INFLATE_SEEDS:
        ...
        for mode, cfg in cfgs.items():
            ...
            exp_nm, exp_na, exp_tm, exp_ta = (
                SOFT_FRAC_X_YOUDEN_SEED_INFLATE_TABLE[seed][mode]
            )
            assert nm == exp_nm
            assert tm == exp_tm
            if exp_na is not None:
    ...
    # seed1 inflate window vs seed0/2 non-inflate
    assert SOFT_FRAC_X_YOUDEN_SEED_INFLATE_TABLE[1]["soft_0.25"][0] == 2
    assert SOFT_FRAC_X_YOUDEN_SEED_INFLATE_TABLE[1]["soft_0.75"][0] <= 1
    assert SOFT_FRAC_X_YOUDEN_SEED_INFLATE_TABLE[0]["soft_0.25"][0] <= 1
    assert recovered == 0


def test_denser_soft_x_youden_multiseed_nested_tori_ari() -> None:
    """#44 / A2-T48/T49: denser soft×Youden multi-seed + h0-only contrast.

    Denser scaffolds across seeds 0..2: seed0 youden alone keeps tori
    chance-ARI K=2; soft×youden and seeds1–2 collapse both ≤1. Baseline
    seed1 soft inflate does not reproduce on denser. Not sample-ARI
    ...
    from proteus.stage1.edge_evidence import (
        ...
        DENSER_SOFT_X_YOUDEN_MULTISEED_SOFT_FRAC,
    ...
    for seed in DENSER_SOFT_X_YOUDEN_MULTISEED_SEEDS:
        nested = make_nested_spheres(
            n_per_sphere=DENSER_SOFT_X_YOUDEN_MULTISEED_NESTED_N,
            extrusion_dim=1, seed=seed,
        )
        tori = make_linked_tori(
            n_per_torus=DENSER_SOFT_X_YOUDEN_MULTISEED_TORI_N, seed=seed,
        )

        def _adapt(points, tau: float, rng_seed: int = seed):
            sc = Stage1Scaffold(
                dim=int(points.shape[1]), tau=float(tau), k=8,
                max_nodes=DENSER_SOFT_X_YOUDEN_MULTISEED_MAX_NODES,
                ann_backend="naive", rng=np.random.default_rng(rng_seed),
            )
            sc.init_from(points, n_seeds=8)
            sc.run_until_stable(
        ...
        cfgs = {
            ...
            "soft_x_youden": proposed_h0_calibrated_config(
                "youden",
                soft_capacity_only=True,
                soft_capacity_frac=DENSER_SOFT_X_YOUDEN_MULTISEED_SOFT_FRAC,
            ),
        }
        for mode, cfg in cfgs.items():
            nm, na = _hollow_majors_and_sample_ari(
            ...
            exp_nm, exp_na, exp_tm, exp_ta = (
                DENSER_SOFT_X_YOUDEN_MULTISEED_TABLE[seed][mode]
            )
            assert nm == exp_nm
            assert tm == exp_tm
            if exp_na is not None:
    ...
    assert DENSER_SOFT_X_YOUDEN_MULTISEED_TABLE[1]["soft_x_youden"][0] <= 1
    assert recovered == 0


def test_denser_soft_frac_x_youden_seed_inflate_nested_tori_ari() -> None:
    """#44 / A2-T50: denser soft_frac×Youden seed1 inflate window.

    Denser scaffolds: seed1 never inflates across soft_frac∈{0.1..0.9};
    denser kills the baseline frac-window. Seed0 soft_0.1 keeps tori
    chance-ARI K=2; soft≥0.25 collapses. Not sample-ARI recovery;
    ...
    for seed in DENSER_SOFT_FRAC_X_YOUDEN_SEED_INFLATE_SEEDS:
        nested = make_nested_spheres(
            n_per_sphere=DENSER_SOFT_FRAC_X_YOUDEN_SEED_INFLATE_NESTED_N,
            extrusion_dim=1, seed=seed,
        )
        tori = make_linked_tori(
            n_per_torus=DENSER_SOFT_FRAC_X_YOUDEN_SEED_INFLATE_TORI_N,
            seed=seed,
        )
        ...
        def _adapt(points, tau: float, rng_seed: int = seed):
            sc = Stage1Scaffold(
                dim=int(points.shape[1]), tau=float(tau), k=8,
                max_nodes=DENSER_SOFT_FRAC_X_YOUDEN_SEED_INFLATE_MAX_NODES,
                ann_backend="naive", rng=np.random.default_rng(rng_seed),
            )
            sc.init_from(points, n_seeds=8)
            sc.run_until_stable(
        ...
        for mode, cfg in cfgs.items():
            ...
            exp_nm, exp_na, exp_tm, exp_ta = (
                DENSER_SOFT_FRAC_X_YOUDEN_SEED_INFLATE_TABLE[seed][mode]
            )
            assert nm == exp_nm
            assert tm == exp_tm
            if exp_na is not None:
    ...
    # denser kills seed1 inflate across the frac window
    assert DENSER_SOFT_FRAC_X_YOUDEN_SEED_INFLATE_TABLE[1]["soft_0.25"][0] <= 1
...
def test_bridge_mass_x_youden_seed_inflate_nested_tori_ari() -> None:
    """#44 / A2-T51: bridge_mass vs betweenness soft×Youden seed1 inflate.

    Betweenness seed1 inflate is method-specific; bridge_mass never
    inflates nested across the frac window. Multi-seed@0.25 documents
    the contrast. Not sample-ARI recovery; flags off; no awaiting flip.
    """
    ...
    recovered = 0
    # Multi-seed @ frac=0.25 method contrast
    for seed in BRIDGE_MASS_X_YOUDEN_SEED_INFLATE_SEEDS:
        ...
        for mode, cfg in cfgs.items():
            ...
            exp_nm, exp_na, exp_tm, exp_ta = (
                BRIDGE_MASS_X_YOUDEN_SEED_INFLATE_TABLE[seed][mode]
            )
            assert nm == exp_nm
            assert tm == exp_tm
            if exp_na is not None:
    ...
    # Seed1 frac-window: betweenness inflates; bridge_mass never
    seed = 1
    nested = make_nested_spheres(n_per_sphere=80, extrusion_dim=1, seed=seed)
    tori = make_linked_tori(n_per_torus=120, seed=seed)
    sc_n = Stage1Scaffold(
    ...
    sc_t = Stage1Scaffold(
        dim=int(tori.points.shape[1]), tau=0.5, k=8, max_nodes=64,
        ann_backend="naive", rng=np.random.default_rng(seed),
    )
...
def test_soft_x_youden_tau_star_nested_tori_ari() -> None:
    """#44 / A2-T52: soft×Youden at operational scale-search tau*.

    Evaluate soft×Youden majors+ARI at lean ``tau*`` (n_grid=12), not
    fixed probe taus. Seed1 probe soft inflate is absent at tau*; seed0
    tori keeps chance-ARI K≥2. Not sample-ARI recovery; flags off; no
    awaiting flip.
    """
    ...
    from proteus.stage1.edge_evidence import (
        ...
        SOFT_X_YOUDEN_TAU_STAR_SCALE_SEED_BASE,
        SOFT_X_YOUDEN_TAU_STAR_SEEDS,
        SOFT_X_YOUDEN_TAU_STAR_SOFT_FRAC,
    ...
    def _lean(seed: int) -> ScaleSearchConfig:
        return ScaleSearchConfig(
            tau_min=1e-3,
            tau_max=2.0,
            max_grid_points=SOFT_X_YOUDEN_TAU_STAR_MAX_GRID_POINTS,
            k=8,
            n_seeds=8,
            ann_backend="naive",
            stabilization=StabilizationConfig(
    ...
    def _adapt(points, tau: float, rng_seed: int):
        sc = Stage1Scaffold(
            dim=int(points.shape[1]), tau=float(tau), k=8, max_nodes=64,
            ann_backend="naive", rng=np.random.default_rng(rng_seed),
        )
    ...
    for seed in SOFT_X_YOUDEN_TAU_STAR_SEEDS:
        nested = make_nested_spheres(n_per_sphere=80, extrusion_dim=1, seed=seed)
        tori = make_linked_tori(n_per_torus=120, seed=seed)
        scale_seed = SOFT_X_YOUDEN_TAU_STAR_SCALE_SEED_BASE + seed
        r_n = run_scale_search(
            nested.points, dim=nested.points.shape[1], config=_lean(scale_seed),
        )
        r_t = run_scale_search(
            tori.points, dim=tori.points.shape[1], config=_lean(scale_seed),
        )
        n_tau, t_tau = float(r_n.tau_star), float(r_t.tau_star)
        exp_n_tau, exp_t_tau = SOFT_X_YOUDEN_TAU_STAR_VALUES[seed]
        assert abs(n_tau - exp_n_tau) < 1e-6
        assert abs(t_tau - exp_t_tau) < 1e-6
        sc_n = _adapt(nested.points, n_tau, seed)
        sc_t = _adapt(tori.points, t_tau, seed)
        cfgs = {
            "youden": proposed_h0_calibrated_config("youden"),
            "soft_x_youden": proposed_h0_calibrated_config(
                "youden",
                soft_capacity_only=True,
                soft_capacity_frac=SOFT_X_YOUDEN_TAU_STAR_SOFT_FRAC,
            ),
        }
        for mode, cfg in cfgs.items():
            nm, na = _hollow_majors_and_sample_ari(
            ...
            exp_nm, exp_na, exp_tm, exp_ta = (
                SOFT_X_YOUDEN_TAU_STAR_TABLE[seed][mode]
            )
            assert nm == exp_nm
            assert tm == exp_tm
            if exp_na is not None:
...
def test_denser_soft_x_persist_tau_star_nested_tori_ari() -> None:
    """#44 / A2-T57: denser soft×persist_agree at operational tau* e2e.

    Denser kills T54 seed1 nested e2e inflate; denser-youden seed0 nested
    K=2 chance-ARI is killed by soft/persist. Uniforms stay 1. Not
    sample-ARI recovery; flags off; no awaiting flip.
    """
    ...
    from proteus.stage1.edge_evidence import (
        ...
        DENSER_SOFT_X_PERSIST_TAU_STAR_SCALE_SEED_BASE,
    ...
    def _lean(seed: int) -> ScaleSearchConfig:
        return ScaleSearchConfig(
            tau_min=1e-3,
            tau_max=2.0,
            max_grid_points=DENSER_SOFT_X_PERSIST_TAU_STAR_MAX_GRID_POINTS,
            k=8,
            n_seeds=8,
            ann_backend="naive",
            max_nodes=DENSER_SOFT_X_PERSIST_TAU_STAR_MAX_NODES,
            stabilization=StabilizationConfig(
                min_equilibrium_epochs=2, max_epochs=8,
            ),
            seed=seed,
    ...
    def _run(points, labels, dim, *, soft: bool, persist: bool, seed: int,
        ...
        cfg = RecursionConfig(
            scale_search=_lean(
                DENSER_SOFT_X_PERSIST_TAU_STAR_SCALE_SEED_BASE + seed
            ),
            min_samples=min_samples,
            max_depth=3,
            require_persistent_split=True,
            ...
            prefer_hollow_edge_prepass=True,
            hollow_mid_radius_frac=0.5,
            hollow_h0=float(DENSER_SOFT_X_PERSIST_TAU_STAR_H0),
            hollow_min_end_count=0.5,
            hollow_gabriel_fallback=False,
            hollow_soft_capacity_only=soft,
            hollow_soft_capacity_frac=DENSER_SOFT_X_PERSIST_TAU_STAR_SOFT_FRAC,
            hollow_soft_capacity_method="betweenness",
            hollow_require_persistent_agree=persist,
            seed=DENSER_SOFT_X_PERSIST_TAU_STAR_SCALE_SEED_BASE + seed,
        )
        tree = run_recursive_discovery(points, dim=dim, config=cfg)
        n_leaves = len(tree.leaves)
        ari = None
    ...
    for seed in DENSER_SOFT_X_PERSIST_TAU_STAR_SEEDS:
        nested = make_nested_spheres(
            n_per_sphere=DENSER_SOFT_X_PERSIST_TAU_STAR_NESTED_N,
            extrusion_dim=1, seed=seed,
        )
        tori = make_linked_tori(
            n_per_torus=DENSER_SOFT_X_PERSIST_TAU_STAR_TORI_N, seed=seed,
        )
        for mode, (soft, persist) in modes.items():
            nl, na = _run(
                nested.points, nested.labels, nested.points.shape[1],
            ...
            exp_nl, exp_na, exp_tl, exp_ta = (
                DENSER_SOFT_X_PERSIST_TAU_STAR_TABLE[seed][mode]
            )
            assert nl == exp_nl
            assert tl == exp_tl
            if exp_na is not None:
    ...
    for mode, (soft, persist) in modes.items():
        ...
        assert cl == DENSER_SOFT_X_PERSIST_TAU_STAR_UNIFORMS["circle"][mode]
        assert sl == DENSER_SOFT_X_PERSIST_TAU_STAR_UNIFORMS["swiss"][mode]

    # denser kills T54 seed1 nested e2e inflate; soft kills denser-youden seed0
...
def test_denser_soft_x_gabriel_x_persist_tau_star_nested_tori_ari() -> None:
    """#44 / A2-T61-followon: denser soft×gabriel×persist compose@tau*.

    Triple compose does not unlock beyond T57/T60 pairwise denser
    collapse. Denser-youden seed0 nested K=2 chance-ARI is killed by
    soft×conj / soft×persist / soft×conj×persist. Not sample-ARI
    recovery; flags off; no awaiting flip.
    """
    ...
    from proteus.stage1.edge_evidence import (
        ...
        DENSER_SOFT_X_GABRIEL_X_PERSIST_TAU_STAR_SCALE_SEED_BASE,
    ...
    def _lean(seed: int) -> ScaleSearchConfig:
        return ScaleSearchConfig(
            tau_min=1e-3,
            tau_max=2.0,
            max_grid_points=DENSER_SOFT_X_GABRIEL_X_PERSIST_TAU_STAR_MAX_GRID_POINTS,
            k=8,
            n_seeds=8,
            ann_backend="naive",
            max_nodes=DENSER_SOFT_X_GABRIEL_X_PERSIST_TAU_STAR_MAX_NODES,
            stabilization=StabilizationConfig(
                min_equilibrium_epochs=2, max_epochs=8,
            ),
            seed=seed,
    ...
    def _run(
        ...
        cfg = RecursionConfig(
            scale_search=_lean(
                DENSER_SOFT_X_GABRIEL_X_PERSIST_TAU_STAR_SCALE_SEED_BASE + seed
            ),
            min_samples=min_samples,
            max_depth=3,
            require_persistent_split=True,
            ...
            prefer_hollow_edge_prepass=True,
            hollow_mid_radius_frac=0.5,
            hollow_h0=float(DENSER_SOFT_X_GABRIEL_X_PERSIST_TAU_STAR_H0),
            hollow_min_end_count=0.5,
            hollow_gabriel_fallback=False,
            hollow_soft_capacity_only=soft,
    ...
    for seed in DENSER_SOFT_X_GABRIEL_X_PERSIST_TAU_STAR_SEEDS:
        nested = make_nested_spheres(
            n_per_sphere=DENSER_SOFT_X_GABRIEL_X_PERSIST_TAU_STAR_NESTED_N,
            extrusion_dim=1, seed=seed,
        )
        tori = make_linked_tori(
            n_per_torus=DENSER_SOFT_X_GABRIEL_X_PERSIST_TAU_STAR_TORI_N,
            seed=seed,
        )
        for mode, (soft, conj, persist) in modes.items():
            nl, na = _run(
                nested.points, nested.labels, nested.points.shape[1],
            ...
            exp_nl, exp_na, exp_tl, exp_ta = (
                DENSER_SOFT_X_GABRIEL_X_PERSIST_TAU_STAR_TABLE[seed][mode]
            )
            assert nl == exp_nl
            assert tl == exp_tl
            if exp_na is not None:
    ...
    for mode, (soft, conj, persist) in modes.items():
        ...
        assert cl == DENSER_SOFT_X_GABRIEL_X_PERSIST_TAU_STAR_UNIFORMS[
            "circle"
        ][mode]
        assert sl == DENSER_SOFT_X_GABRIEL_X_PERSIST_TAU_STAR_UNIFORMS[
            "swiss"
        ][mode]

    # compose does not unlock beyond pairwise denser collapse
...
def test_soft_keep_band_x_persist_majors_nested_tori() -> None:
    """#44 / A2-T61: non-denser soft keep-band × persist majors baseline.

    Fixed-tau majors keep-band soft≤0.5 → tori K=2 (wider than denser
    T55 ≤0.12); soft≥0.75 collapses. Soft×persist e2e all ≤1 — keep-band
    is majors-only. Not sample-ARI recovery; flags off; no awaiting flip.
    """
    ...
    nested = make_nested_spheres(
        n_per_sphere=SOFT_KEEP_BAND_X_PERSIST_MAJORS_NESTED_N,
        extrusion_dim=1, seed=seed,
    )
    tori = make_linked_tori(
        n_per_torus=SOFT_KEEP_BAND_X_PERSIST_MAJORS_TORI_N,
        seed=seed,
    )
    ...
    def _adapt(points, tau: float):
        sc = Stage1Scaffold(
            dim=int(points.shape[1]), tau=float(tau), k=8,
            max_nodes=SOFT_KEEP_BAND_X_PERSIST_MAJORS_MAX_NODES,
            ann_backend="naive", rng=np.random.default_rng(seed),
        )
        sc.init_from(points, n_seeds=8)
        sc.run_until_stable(
    ...
    sc_n = _adapt(nested.points, SOFT_KEEP_BAND_X_PERSIST_MAJORS_NESTED_TAU)
    sc_t = _adapt(tori.points, SOFT_KEEP_BAND_X_PERSIST_MAJORS_TORI_TAU)
    maj_cfgs: dict[str, object] = {
        "youden": proposed_h0_calibrated_config("youden"),
    }
    for frac in SOFT_KEEP_BAND_X_PERSIST_MAJORS_FRACS:
    ...
    for mode, cfg in maj_cfgs.items():
        ...
        exp_nm, exp_na, exp_tm, exp_ta = (
            SOFT_KEEP_BAND_X_PERSIST_MAJORS_TABLE[mode]
        )
        assert nm == exp_nm
        assert tm == exp_tm
        if exp_na is not None:
    ...
    def _lean() -> ScaleSearchConfig:
        return ScaleSearchConfig(
            tau_min=1e-3,
            tau_max=2.0,
            max_grid_points=SOFT_KEEP_BAND_X_PERSIST_MAJORS_MAX_GRID_POINTS,
            k=8,
            n_seeds=8,
            ann_backend="naive",
            max_nodes=SOFT_KEEP_BAND_X_PERSIST_MAJORS_MAX_NODES,
            ...
            seed=SOFT_KEEP_BAND_X_PERSIST_MAJORS_SCALE_SEED_BASE + seed,
        )

    def _run_e2e(points, labels, *, soft: bool, persist: bool, frac: float):
        cfg = RecursionConfig(
            scale_search=_lean(),
            min_samples=40,
            max_depth=3,
            ...
            prefer_hollow_edge_prepass=True,
            hollow_mid_radius_frac=0.5,
            hollow_h0=float(SOFT_KEEP_BAND_X_PERSIST_MAJORS_H0),
            hollow_min_end_count=0.5,
            hollow_gabriel_fallback=False,
            hollow_soft_capacity_only=soft,
            hollow_soft_capacity_frac=frac,
    ...
    e2e_modes: dict[str, tuple[bool, bool, float]] = {
        "youden": (False, False, 0.25),
        "persist": (False, True, 0.25),
    }
    for frac in (0.12, 0.25, 0.5, 0.75):
        e2e_modes[f"soft_{frac:g}"] = (True, False, frac)
        e2e_modes[f"soft_x_persist_{frac:g}"] = (True, True, frac)

    for mode, (soft, persist, frac) in e2e_modes.items():
        nl, na = _run_e2e(
            nested.points, nested.labels, soft=soft, persist=persist, frac=frac,
        )
        tl, ta = _run_e2e(
            tori.points, tori.labels, soft=soft, persist=persist, frac=frac,
        )
        exp_nl, exp_na, exp_tl, exp_ta = (
            SOFT_KEEP_BAND_X_PERSIST_MAJORS_E2E_TABLE[mode]
        )
        assert nl == exp_nl
        assert tl == exp_tl
        if exp_na is not None:
    ...
    # wider keep-band than denser T55; e2e kills majors keep
...
def test_soft_x_gabriel_x_persist_majors_seed1_inflate() -> None:
    """#44 / A2-T63: soft×gabriel×persist majors seed1 inflate window.

    Seed1 majors soft alone nested K=2 chance-ARI≈0.08 killed by conj;
    e2e seed1 nested K=2 chance-ARI≈0 survives soft×conj×persist
    (majors≠e2e). Not sample-ARI recovery; flags off; no awaiting flip.
    """
    ...
    from proteus.stage1.edge_evidence import (
        ...
        SOFT_X_GABRIEL_X_PERSIST_MAJORS_MAX_GRID_POINTS,
        SOFT_X_GABRIEL_X_PERSIST_MAJORS_MAX_NODES,
        SOFT_X_GABRIEL_X_PERSIST_MAJORS_NESTED_N,
        SOFT_X_GABRIEL_X_PERSIST_MAJORS_NESTED_TAU,
        SOFT_X_GABRIEL_X_PERSIST_MAJORS_SCALE_SEED_BASE,
        ...
        SOFT_X_GABRIEL_X_PERSIST_MAJORS_TORI_TAU,
    ...
    for seed in SOFT_X_GABRIEL_X_PERSIST_MAJORS_SEEDS:
        nested = make_nested_spheres(
            n_per_sphere=SOFT_X_GABRIEL_X_PERSIST_MAJORS_NESTED_N,
            extrusion_dim=1, seed=seed,
        )
        tori = make_linked_tori(
            n_per_torus=SOFT_X_GABRIEL_X_PERSIST_MAJORS_TORI_N,
            seed=seed,
        )
        ...
        def _adapt(points, tau: float, _seed=seed):
            sc = Stage1Scaffold(
                dim=int(points.shape[1]), tau=float(tau), k=8,
                max_nodes=SOFT_X_GABRIEL_X_PERSIST_MAJORS_MAX_NODES,
        ...
        sc_n = _adapt(nested.points, SOFT_X_GABRIEL_X_PERSIST_MAJORS_NESTED_TAU)
        sc_t = _adapt(tori.points, SOFT_X_GABRIEL_X_PERSIST_MAJORS_TORI_TAU)
        maj_cfgs = {
            "youden": proposed_h0_calibrated_config("youden"),
            "soft": proposed_h0_calibrated_config(
                "youden",
                soft_capacity_only=True,
                soft_capacity_frac=SOFT_X_GABRIEL_X_PERSIST_MAJORS_SOFT_FRAC,
            ),
        ...
        for mode, cfg in maj_cfgs.items():
            ...
            exp_nm, exp_na, exp_tm, exp_ta = (
                SOFT_X_GABRIEL_X_PERSIST_MAJORS_TABLE[seed][mode]
            )
            assert nm == exp_nm
            assert tm == exp_tm
            if exp_na is not None:
    ...
    def _lean(seed: int) -> ScaleSearchConfig:
        return ScaleSearchConfig(
            tau_min=1e-3,
            tau_max=2.0,
            max_grid_points=SOFT_X_GABRIEL_X_PERSIST_MAJORS_MAX_GRID_POINTS,
            k=8,
            n_seeds=8,
            ann_backend="naive",
            max_nodes=SOFT_X_GABRIEL_X_PERSIST_MAJORS_MAX_NODES,
            ...
            seed=SOFT_X_GABRIEL_X_PERSIST_MAJORS_SCALE_SEED_BASE + seed,
        )
    ...
    def _run_e2e(
        ...
        cfg = RecursionConfig(
            ...
            prefer_hollow_edge_prepass=True,
            hollow_mid_radius_frac=0.5,
            hollow_h0=float(SOFT_X_GABRIEL_X_PERSIST_MAJORS_H0),
            hollow_min_end_count=0.5,
            hollow_gabriel_fallback=False,
            hollow_soft_capacity_only=soft,
            hollow_soft_capacity_frac=(
    ...
    for seed in SOFT_X_GABRIEL_X_PERSIST_MAJORS_SEEDS:
        ...
        for mode, (soft, conj, persist) in e2e_modes.items():
            ...
            exp_nl, exp_na, exp_tl, exp_ta = (
                SOFT_X_GABRIEL_X_PERSIST_MAJORS_E2E_TABLE[seed][mode]
            )
            assert nl == exp_nl
            assert tl == exp_tl
            if exp_na is not None:
    ...
    for mode, (soft, conj, persist) in e2e_modes.items():
        ...
        assert cl == SOFT_X_GABRIEL_X_PERSIST_MAJORS_UNIFORMS["circle"][mode]
        assert sl == SOFT_X_GABRIEL_X_PERSIST_MAJORS_UNIFORMS["swiss"][mode]

    # seed1 majors soft inflate killed by conj; e2e survives compose
...
def test_denser_soft_keep_band_x_gabriel_majors() -> None:
    """#44 / A2-T64-followon: denser soft keep-band × gabriel majors.

    T55 denser majors keep-band soft≤0.12 → tori K=2 is killed by
    gabriel conj at majors; lean e2e soft/soft×conj all ≤1. Not
    sample-ARI recovery; flags off; no awaiting flip.
    """
    ...
    from proteus.stage1.edge_evidence import (
        ...
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_MAJORS_MAX_GRID_POINTS,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_MAJORS_MAX_NODES,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_MAJORS_NESTED_N,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_MAJORS_NESTED_TAU,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_MAJORS_SCALE_SEED_BASE,
        ...
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_MAJORS_TORI_TAU,
    ...
    nested = make_nested_spheres(
        n_per_sphere=DENSER_SOFT_KEEP_BAND_X_GABRIEL_MAJORS_NESTED_N,
        extrusion_dim=1, seed=seed,
    )
    tori = make_linked_tori(
        n_per_torus=DENSER_SOFT_KEEP_BAND_X_GABRIEL_MAJORS_TORI_N,
        seed=seed,
    )
    ...
    def _adapt(points, tau: float):
        sc = Stage1Scaffold(
            dim=int(points.shape[1]), tau=float(tau), k=8,
            max_nodes=DENSER_SOFT_KEEP_BAND_X_GABRIEL_MAJORS_MAX_NODES,
            ann_backend="naive", rng=np.random.default_rng(seed),
        )
        sc.init_from(points, n_seeds=8)
        sc.run_until_stable(
    ...
    sc_n = _adapt(
        nested.points, DENSER_SOFT_KEEP_BAND_X_GABRIEL_MAJORS_NESTED_TAU,
    )
    sc_t = _adapt(
        tori.points, DENSER_SOFT_KEEP_BAND_X_GABRIEL_MAJORS_TORI_TAU,
    )
    maj_cfgs: dict[str, object] = {
        "youden": proposed_h0_calibrated_config("youden"),
        "conj": proposed_h0_calibrated_config(
    ...
    for frac in DENSER_SOFT_KEEP_BAND_X_GABRIEL_MAJORS_FRACS:
        maj_cfgs[f"soft_{frac:g}"] = proposed_h0_calibrated_config(
            "youden",
            soft_capacity_only=True,
            soft_capacity_frac=float(frac),
    ...
    for mode, cfg in maj_cfgs.items():
        ...
        exp_nm, exp_na, exp_tm, exp_ta = (
            DENSER_SOFT_KEEP_BAND_X_GABRIEL_MAJORS_TABLE[mode]
        )
        assert nm == exp_nm
        assert tm == exp_tm
        if exp_na is not None:
    ...
    def _lean() -> ScaleSearchConfig:
        return ScaleSearchConfig(
            ...
            seed=DENSER_SOFT_KEEP_BAND_X_GABRIEL_MAJORS_SCALE_SEED_BASE + seed,
        )

    def _run_e2e(
        points, labels, *, soft: bool, conj: bool, frac: float,
        min_samples: int = 40,
    ):
        cfg = RecursionConfig(
            scale_search=_lean(),
            ...
            prefer_hollow_edge_prepass=True,
            hollow_mid_radius_frac=0.5,
            hollow_h0=float(DENSER_SOFT_KEEP_BAND_X_GABRIEL_MAJORS_H0),
            hollow_min_end_count=0.5,
            hollow_gabriel_fallback=False,
            hollow_soft_capacity_only=soft,
            hollow_soft_capacity_frac=frac,
            hollow_soft_capacity_method="betweenness",
            hollow_require_gabriel_and_h=conj,
            hollow_require_persistent_agree=False,
            seed=DENSER_SOFT_KEEP_BAND_X_GABRIEL_MAJORS_SCALE_SEED_BASE + seed,
        )
        tree = run_recursive_discovery(points, dim=points.shape[1], config=cfg)
        n_leaves = len(tree.leaves)
    ...
    e2e_modes: dict[str, tuple[bool, bool, float]] = {
        "youden": (False, False, 0.12),
        "conj": (False, True, 0.12),
    }
    for frac in (0.05, 0.12, 0.15, 0.25):
        e2e_modes[f"soft_{frac:g}"] = (True, False, frac)
        e2e_modes[f"soft_x_conj_{frac:g}"] = (True, True, frac)

    for mode, (soft, conj, frac) in e2e_modes.items():
        nl, na = _run_e2e(
            nested.points, nested.labels, soft=soft, conj=conj, frac=frac,
        )
        tl, ta = _run_e2e(
            tori.points, tori.labels, soft=soft, conj=conj, frac=frac,
        )
        exp_nl, exp_na, exp_tl, exp_ta = (
            DENSER_SOFT_KEEP_BAND_X_GABRIEL_MAJORS_E2E_TABLE[mode]
        )
        assert nl == exp_nl
        assert tl == exp_tl
        if exp_na is not None:
    ...
    swiss = make_swiss_roll(n_samples=400, noise=0.02, seed=0)
    uni_modes = (
        "youden", "conj", "soft_0.05", "soft_x_conj_0.05",
        "soft_0.12", "soft_x_conj_0.12",
    )
    for mode in uni_modes:
        soft, conj, frac = e2e_modes[mode]
        cl, _ = _run_e2e(
            circle.points, None, soft=soft, conj=conj, frac=frac,
            min_samples=80,
        )
        sl, _ = _run_e2e(
            swiss.points, None, soft=soft, conj=conj, frac=frac,
            min_samples=80,
        )
        assert cl == DENSER_SOFT_KEEP_BAND_X_GABRIEL_MAJORS_UNIFORMS[
            "circle"
        ][mode]
        assert sl == DENSER_SOFT_KEEP_BAND_X_GABRIEL_MAJORS_UNIFORMS[
            "swiss"
        ][mode]

    # T55 keep-band preserved under soft; gabriel kills it at majors
...
def test_denser_soft_keep_band_x_gabriel_x_persist_e2e_frac_grid() -> None:
    """#44 / A2-T65-followon: denser soft keep×gabriel×persist e2e frac grid.

    Soft×persist / soft×conj×persist across denser keep-band fracs all
    ≤1 nested+tori; youden alone keeps seed0 nested K=2 chance-ARI≈0.01.
    ...
    from proteus.stage1.edge_evidence import (
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_COLLAPSE_MIN_FRAC,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_FRACS,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_H0,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_KEEP_MAX_FRAC,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_MAX_GRID_POINTS,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_MAX_NODES,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_NESTED_N,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_SCALE_SEED_BASE,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_SEED,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_TABLE,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_TORI_N,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_UNIFORMS,
    )
    from tests.datasets.synthetic.linked_tori import make_linked_tori
    from tests.datasets.synthetic.nested_spheres import make_nested_spheres
    from tests.datasets.synthetic.swiss_roll import make_swiss_roll
    ...
    assert RecursionConfig().hollow_require_gabriel_and_h is False
    assert RecursionConfig().hollow_require_persistent_agree is False
    assert abs(DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_H0 - 0.73) < 1e-9
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_KEEP_MAX_FRAC == 0.12
    assert (
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_COLLAPSE_MIN_FRAC == 0.15
    )

    seed = DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_SEED
    nested = make_nested_spheres(
        n_per_sphere=DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_NESTED_N,
        extrusion_dim=1, seed=seed,
    )
    tori = make_linked_tori(
        n_per_torus=DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_TORI_N,
        seed=seed,
    )
    ...
    def _lean() -> ScaleSearchConfig:
        return ScaleSearchConfig(
            ...
            n_seeds=8,
            ann_backend="naive",
            max_nodes=DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_MAX_NODES,
            stabilization=StabilizationConfig(
                min_equilibrium_epochs=2, max_epochs=8,
            ),
            seed=(
    ...
    def _run_e2e(
        ...
        cfg = RecursionConfig(
            ...
            prefer_hollow_edge_prepass=True,
            hollow_mid_radius_frac=0.5,
            hollow_h0=float(DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_H0),
            hollow_min_end_count=0.5,
            hollow_gabriel_fallback=False,
            hollow_soft_capacity_only=soft,
            hollow_soft_capacity_frac=frac,
    ...
    for frac in DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_FRACS:
    ...
    for mode, (soft, conj, persist, frac) in e2e_modes.items():
        ...
        exp_nl, exp_na, exp_tl, exp_ta = (
            DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_TABLE[mode]
        )
        assert nl == exp_nl
        assert tl == exp_tl
        if exp_na is not None:
    ...
    for mode in uni_modes:
        ...
        assert cl == DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_UNIFORMS[
            "circle"
        ][mode]
        assert sl == DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_UNIFORMS[
            "swiss"
        ][mode]

    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_TABLE["youden"][0] == 2
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_TABLE[
        "soft_x_persist_0.12"
    ][0] <= 1
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_TABLE[
        "soft_x_conj_x_persist_0.25"
    ][0] <= 1
    assert recovered == 0


def test_denser_soft_keep_band_x_gabriel_multiseed() -> None:
    """#44 / A2-T66: denser soft keep×gabriel multi-seed majors/e2e.

    T55/T64 denser majors keep soft≤0.12 → tori K=2 is seed0-only;
    seeds 1–2 ≤1 at majors; e2e only seed0 youden nested K=2
    chance-ARI≈0.01. Not sample-ARI recovery; flags off; no awaiting.
    """
    ...
    from proteus.stage1.edge_evidence import (
        ...
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_MULTISEED_MAX_GRID_POINTS,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_MULTISEED_MAX_NODES,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_MULTISEED_NESTED_N,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_MULTISEED_NESTED_TAU,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_MULTISEED_SCALE_SEED_BASE,
        ...
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_MULTISEED_TORI_TAU,
    ...
    for seed in DENSER_SOFT_KEEP_BAND_X_GABRIEL_MULTISEED_SEEDS:
        nested = make_nested_spheres(
            n_per_sphere=DENSER_SOFT_KEEP_BAND_X_GABRIEL_MULTISEED_NESTED_N,
            extrusion_dim=1, seed=seed,
        )
        tori = make_linked_tori(
            n_per_torus=DENSER_SOFT_KEEP_BAND_X_GABRIEL_MULTISEED_TORI_N,
            seed=seed,
        )
        ...
        def _adapt(points, tau: float, _seed=seed):
            sc = Stage1Scaffold(
                dim=int(points.shape[1]), tau=float(tau), k=8,
                max_nodes=DENSER_SOFT_KEEP_BAND_X_GABRIEL_MULTISEED_MAX_NODES,
                ann_backend="naive", rng=np.random.default_rng(_seed),
            )
            sc.init_from(points, n_seeds=8)
            sc.run_until_stable(
        ...
        sc_t = _adapt(
            tori.points, DENSER_SOFT_KEEP_BAND_X_GABRIEL_MULTISEED_TORI_TAU,
        )
        maj_cfgs: dict = {
            "youden": proposed_h0_calibrated_config("youden"),
            "conj": proposed_h0_calibrated_config(
        ...
        for frac in DENSER_SOFT_KEEP_BAND_X_GABRIEL_MULTISEED_FRACS:
            maj_cfgs[f"soft_{frac:g}"] = proposed_h0_calibrated_config(
                "youden",
                soft_capacity_only=True,
                soft_capacity_frac=frac,
            )
            maj_cfgs[f"soft_x_conj_{frac:g}"] = proposed_h0_calibrated_config(
                "youden",
                soft_capacity_only=True,
        ...
        for mode, cfg in maj_cfgs.items():
            ...
            exp_nm, exp_na, exp_tm, exp_ta = (
                DENSER_SOFT_KEEP_BAND_X_GABRIEL_MULTISEED_TABLE[seed][mode]
            )
            assert nm == exp_nm
            assert tm == exp_tm
            if exp_na is not None:
    ...
    def _run_e2e(
        points, labels, dim, *, soft: bool, conj: bool, frac: float,
        seed: int, min_samples: int = 40,
    ):
        cfg = RecursionConfig(
            scale_search=_lean(seed),
            ...
            prefer_hollow_edge_prepass=True,
            hollow_mid_radius_frac=0.5,
            hollow_h0=float(DENSER_SOFT_KEEP_BAND_X_GABRIEL_MULTISEED_H0),
            hollow_min_end_count=0.5,
            hollow_gabriel_fallback=False,
            hollow_soft_capacity_only=soft,
            hollow_soft_capacity_frac=frac if soft else 0.25,
            hollow_soft_capacity_method="betweenness",
            hollow_require_gabriel_and_h=conj,
            hollow_require_persistent_agree=False,
            seed=(
    ...
    e2e_modes = {
        "youden": (False, False, 0.25),
        "soft_0.12": (True, False, 0.12),
        "soft_x_conj_0.12": (True, True, 0.12),
        "soft_0.15": (True, False, 0.15),
        "soft_x_conj_0.15": (True, True, 0.15),
    }
    for seed in DENSER_SOFT_KEEP_BAND_X_GABRIEL_MULTISEED_SEEDS:
        nested = make_nested_spheres(
            n_per_sphere=DENSER_SOFT_KEEP_BAND_X_GABRIEL_MULTISEED_NESTED_N,
        ...
        for mode, (soft, conj, frac) in e2e_modes.items():
            nl, na = _run_e2e(
                nested.points, nested.labels, nested.points.shape[1],
                soft=soft, conj=conj, frac=frac, seed=seed,
            )
            tl, ta = _run_e2e(
                tori.points, tori.labels, tori.points.shape[1],
                soft=soft, conj=conj, frac=frac, seed=seed,
            ...
            exp_nl, exp_na, exp_tl, exp_ta = (
                DENSER_SOFT_KEEP_BAND_X_GABRIEL_MULTISEED_E2E_TABLE[seed][mode]
            )
            assert nl == exp_nl
            assert tl == exp_tl
            if exp_na is not None:
    ...
    swiss = make_swiss_roll(n_samples=400, noise=0.02, seed=0)
    for mode, (soft, conj, frac) in e2e_modes.items():
        if mode not in DENSER_SOFT_KEEP_BAND_X_GABRIEL_MULTISEED_UNIFORMS[
            "circle"
        ]:
            continue
        cl, _ = _run_e2e(
            circle.points, None, circle.points.shape[1],
            soft=soft, conj=conj, frac=frac, seed=0, min_samples=80,
        )
        sl, _ = _run_e2e(
            swiss.points, None, swiss.points.shape[1],
            soft=soft, conj=conj, frac=frac, seed=0, min_samples=80,
        )
        assert cl == DENSER_SOFT_KEEP_BAND_X_GABRIEL_MULTISEED_UNIFORMS[
            "circle"
        ][mode]
        assert sl == DENSER_SOFT_KEEP_BAND_X_GABRIEL_MULTISEED_UNIFORMS[
            "swiss"
        ][mode]

    # seed0 keep singleton; seeds 1–2 no keep; e2e seed0 youden only
...
def test_denser_soft_keep_band_x_gabriel_x_persist_multiseed_e2e() -> None:
    """#44 / A2-T68: denser soft keep×gabriel×persist multi-seed e2e.

    Extends T65 seed0 persist frac grid across seeds 0..2; soft×persist
    / soft×conj×persist all ≤1 nested+tori; only seed0 youden nested
    K=2 chance-ARI≈0.01. Not sample-ARI recovery; flags off; no awaiting.
    """

    from sklearn.metrics import adjusted_rand_score

    from proteus.stage1.edge_evidence import (
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MULTISEED_COLLAPSE_MIN_FRAC,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MULTISEED_E2E_TABLE,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MULTISEED_FRACS,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MULTISEED_H0,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MULTISEED_KEEP_MAX_FRAC,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MULTISEED_MAX_GRID_POINTS,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MULTISEED_MAX_NODES,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MULTISEED_NESTED_N,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MULTISEED_SCALE_SEED_BASE,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MULTISEED_SEEDS,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MULTISEED_TORI_N,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MULTISEED_UNIFORMS,
    )
    from tests.datasets.synthetic.linked_tori import make_linked_tori
    from tests.datasets.synthetic.nested_spheres import make_nested_spheres
    from tests.datasets.synthetic.swiss_roll import make_swiss_roll
    ...
    def _run_e2e(
        points, labels, dim, *, soft: bool, conj: bool, persist: bool,
        frac: float, seed: int, min_samples: int = 40,
    ):
        cfg = RecursionConfig(
            scale_search=_lean(seed),
            min_samples=min_samples,
    ...
    for frac in DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MULTISEED_FRACS:
        e2e_modes[f"soft_x_persist_{frac:g}"] = (True, False, True, frac)
        e2e_modes[f"soft_x_conj_x_persist_{frac:g}"] = (
            True, True, True, frac,
        )
    ...
    for seed in DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MULTISEED_SEEDS:
        ...
        for mode, (soft, conj, persist, frac) in e2e_modes.items():
            nl, na = _run_e2e(
                nested.points, nested.labels, nested.points.shape[1],
                soft=soft, conj=conj, persist=persist, frac=frac, seed=seed,
            )
            tl, ta = _run_e2e(
                tori.points, tori.labels, tori.points.shape[1],
                soft=soft, conj=conj, persist=persist, frac=frac, seed=seed,
    ...
    swiss = make_swiss_roll(n_samples=400, noise=0.02, seed=0)
    for mode in DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MULTISEED_UNIFORMS[
        "circle"
    ]:
        soft, conj, persist, frac = e2e_modes[mode]
        cl, _ = _run_e2e(
            circle.points, None, circle.points.shape[1],
            soft=soft, conj=conj, persist=persist, frac=frac, seed=0,
            min_samples=80,
        )
        sl, _ = _run_e2e(
            swiss.points, None, swiss.points.shape[1],
    ...
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MULTISEED_E2E_TABLE[0][
        "youden"
    ][0] == 2
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MULTISEED_E2E_TABLE[0][
        "soft_x_persist_0.12"
    ][0] <= 1
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MULTISEED_E2E_TABLE[1][
        "youden"
    ][0] <= 1
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MULTISEED_E2E_TABLE[2][
        "soft_x_conj_x_persist_0.15"
    ][0] <= 1
    assert recovered == 0


def test_denser_soft_keep_band_x_gabriel_x_persist_majors_pin() -> None:
    """#44 / A2-T69: denser soft keep×gabriel seed0 keep × soft×persist pin.

    Seed0 denser majors soft≤0.12 → tori K=2 (gabriel kills; soft≥0.15
    collapses); soft×persist e2e all ≤1 — majors-only under persist
    compose. Not sample-ARI recovery; flags off; no awaiting flip.
    """
    ...
    from proteus.stage1.edge_evidence import (
        ...
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MAX_GRID_POINTS,
        ...
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_NESTED_TAU,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SCALE_SEED_BASE,
        ...
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_TORI_TAU,
    ...
    for frac in DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_FRACS:
        maj_cfgs[f"soft_{frac:g}"] = proposed_h0_calibrated_config(
            "youden",
            soft_capacity_only=True,
            soft_capacity_frac=float(frac),
    ...
    for mode, cfg in maj_cfgs.items():
        ...
        exp_nm, exp_na, exp_tm, exp_ta = (
            DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_TABLE[mode]
        )
        assert nm == exp_nm
        assert tm == exp_tm
        if exp_na is not None:
    ...
    for mode, (soft, conj, persist, frac) in e2e_modes.items():
        ...
        exp_nl, exp_na, exp_tl, exp_ta = (
            DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_E2E_TABLE[mode]
        )
        assert nl == exp_nl
        assert tl == exp_tl
        if exp_na is not None:
...
def test_denser_soft_keep_band_x_gabriel_x_persist_youden_only_frac_pin() -> None:
    """#44 / A2-T70: denser soft keep×gabriel×persist seed0 youden-only frac pin.

    Bare youden alone keeps nested K=2≈0.01; conj/persist and denser
    mid-band soft×persist all ≤1. Not sample-ARI recovery; flags off;
    no awaiting flip.
    """
    ...
    from proteus.stage1.edge_evidence import (
        ...
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_YOUDEN_ONLY_FRAC_PIN_MAX_GRID_POINTS,
        ...
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_YOUDEN_ONLY_FRAC_PIN_SCALE_SEED_BASE,
    ...
    for frac in DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_YOUDEN_ONLY_FRAC_PIN_FRACS:
        e2e_modes[f"soft_x_persist_{frac:g}"] = (True, False, True, frac)
        e2e_modes[f"soft_x_conj_x_persist_{frac:g}"] = (
            True, True, True, frac,
        )
    ...
    swiss = make_swiss_roll(n_samples=400, noise=0.02, seed=0)
    uni_modes = (
        "youden", "conj", "persist", "conj_x_persist",
        "soft_x_persist_0.12", "soft_x_conj_x_persist_0.12",
    )
    for mode in uni_modes:
        soft, conj, persist, frac = e2e_modes[mode]
...
def test_denser_soft_x_gabriel_majors_seed_stable_window() -> None:
    """#44 / A2-T71: denser soft×gabriel majors seed-stable window (non-persist).

    Denser mid-band soft fracs across seeds 0..2: seed0 soft≤0.12 →
    tori K=2; seeds 1–2 all ≤1 (NOT seed-stable); e2e only seed0 youden
    nested K=2≈0.01. Not sample-ARI recovery; flags off; no awaiting.
    """
    ...
    from proteus.stage1.edge_evidence import (
        ...
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_MAX_GRID_POINTS,
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_MAX_NODES,
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_NESTED_N,
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_NESTED_TAU,
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_SCALE_SEED_BASE,
        ...
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_TORI_TAU,
    ...
    assert RecursionConfig().hollow_soft_capacity_only is False
    assert RecursionConfig().hollow_require_gabriel_and_h is False
    assert abs(DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_H0 - 0.73) < 1e-9
    ...
    recovered = 0
    for seed in DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_SEEDS:
        nested = make_nested_spheres(
            n_per_sphere=DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_NESTED_N,
            extrusion_dim=1, seed=seed,
        )
        tori = make_linked_tori(
            n_per_torus=DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_TORI_N,
            seed=seed,
        )
        ...
        for frac in DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_FRACS:
            maj_cfgs[f"soft_{frac:g}"] = proposed_h0_calibrated_config(
                "youden",
                soft_capacity_only=True,
                soft_capacity_frac=frac,
        ...
        for mode, cfg in maj_cfgs.items():
            ...
            exp_nm, exp_na, exp_tm, exp_ta = (
                DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_TABLE[seed][
                    mode
                ]
            )
            assert nm == exp_nm
    ...
    e2e_modes = {
        "youden": (False, False, 0.25),
        "soft_0.03": (True, False, 0.03),
        "soft_x_conj_0.03": (True, True, 0.03),
        "soft_0.12": (True, False, 0.12),
        "soft_x_conj_0.12": (True, True, 0.12),
        "soft_0.25": (True, False, 0.25),
        "soft_x_conj_0.25": (True, True, 0.25),
    }
    for seed in DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_SEEDS:
        nested = make_nested_spheres(
            n_per_sphere=DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_NESTED_N,
    ...
    swiss = make_swiss_roll(n_samples=400, noise=0.02, seed=0)
    for mode, (soft, conj, frac) in {
        **e2e_modes,
        "conj": (False, True, 0.25),
    }.items():
        if mode not in DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_UNIFORMS[
            "circle"
        ]:
            continue
        cl, _ = _run_e2e(
        ...
        assert cl == DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_UNIFORMS[
            "circle"
        ][mode]
        assert sl == DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_UNIFORMS[
            "swiss"
        ][mode]

    # seed0 denser keep singleton; seeds 1–2 no keep; e2e seed0 youden only
...
def test_denser_soft_keep_band_x_gabriel_x_persist_majors_pin_multiseed() -> None:
    """#44 / A2-T72: denser soft keep×gabriel×persist majors-pin multi-seed.

    Seed0 soft≤0.12 → tori K=2; seeds 1–2 all ≤1 (NOT seed-stable);
    soft×persist e2e all ≤1; only seed0 youden nested K=2≈0.01. Not
    sample-ARI recovery; flags off; no awaiting flip.
    """
    ...
    from proteus.stage1.edge_evidence import (
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_COLLAPSE_MIN_FRAC,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_E2E_TABLE,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_FRACS,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_H0,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_KEEP_MAX_FRAC,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_MAX_GRID_POINTS,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_MAX_NODES,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_NESTED_N,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_NESTED_TAU,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_SCALE_SEED_BASE,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_SEEDS,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_TABLE,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_TORI_N,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_TORI_TAU,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_UNIFORMS,
        proposed_h0_calibrated_config,
    )
    from proteus.stage1.scaffold import Stage1Scaffold
    from tests.datasets.synthetic.linked_tori import make_linked_tori
    ...
    assert RecursionConfig().hollow_require_persistent_agree is False
    assert abs(
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_H0 - 0.73
    ) < 1e-9
    assert (
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_KEEP_MAX_FRAC
        == 0.12
    ...
    recovered = 0
    for seed in DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_SEEDS:
        nested = make_nested_spheres(
            n_per_sphere=(
                DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_NESTED_N
            ),
        ...
        for frac in DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_FRACS:
            maj_cfgs[f"soft_{frac:g}"] = proposed_h0_calibrated_config(
                "youden",
                soft_capacity_only=True,
                soft_capacity_frac=float(frac),
    ...
    # seed0 denser keep singleton under persist pin; seeds 1–2 no keep
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_TABLE[
        0
    ]["soft_0.12"][2] == 2
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_TABLE[
        0
    ...
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_E2E_TABLE[
        0
    ]["youden"][0] == 2
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_E2E_TABLE[
        0
    ]["soft_x_persist_0.12"][0] <= 1
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_E2E_TABLE[
        1
    ]["youden"][0] <= 1
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_E2E_TABLE[
        2
    ]["soft_x_conj_x_persist_0.15"][0] <= 1
    assert recovered == 0


def test_denser_soft_x_gabriel_majors_seed12_inflate_kill_vs_seed0_keep() -> None:
    """#44 / A2-T73: denser soft×gabriel seed1/2 inflate kill vs seed0 keep.

    Denser kills T63 seed1 soft_frac=0.25 nested majors inflate (seeds
    1–2 soft@0.05/0.12/0.25 all ≤1); seed0 keep soft≤0.12 → tori K=2
    (gabriel kills; soft=0.25 collapses); e2e only seed0 youden nested
    K=2≈0.01. Not sample-ARI recovery; flags off; no awaiting flip.
    """
    ...
    from proteus.stage1.edge_evidence import (
        ...
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED12_INFLATE_KILL_VS_SEED0_KEEP_MAX_GRID_POINTS,
        ...
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED12_INFLATE_KILL_VS_SEED0_KEEP_NESTED_TAU,
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED12_INFLATE_KILL_VS_SEED0_KEEP_SCALE_SEED_BASE,
        ...
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED12_INFLATE_KILL_VS_SEED0_KEEP_TORI_TAU,
    ...
    # baseline T63: seed1 soft alone inflates nested majors
    assert SOFT_X_GABRIEL_X_PERSIST_MAJORS_TABLE[1]["soft"][0] == 2
    assert SOFT_X_GABRIEL_X_PERSIST_MAJORS_TABLE[1]["soft_x_conj"][0] <= 1
    ...
    for seed in DENSER_SOFT_X_GABRIEL_MAJORS_SEED12_INFLATE_KILL_VS_SEED0_KEEP_SEEDS:
        ...
        for frac in DENSER_SOFT_X_GABRIEL_MAJORS_SEED12_INFLATE_KILL_VS_SEED0_KEEP_FRACS:
            maj_cfgs[f"soft_{frac:g}"] = proposed_h0_calibrated_config(
                "youden",
                soft_capacity_only=True,
                soft_capacity_frac=float(frac),
    ...
    e2e_modes = {
        "youden": (False, False, 0.25),
        "soft_0.05": (True, False, 0.05),
        "soft_x_conj_0.05": (True, True, 0.05),
        "soft_0.12": (True, False, 0.12),
        "soft_x_conj_0.12": (True, True, 0.12),
        "soft_0.25": (True, False, 0.25),
        "soft_x_conj_0.25": (True, True, 0.25),
    ...
    for mode, (soft, conj, frac) in {
        **e2e_modes,
        ...
        if mode not in DENSER_SOFT_X_GABRIEL_MAJORS_SEED12_INFLATE_KILL_VS_SEED0_KEEP_UNIFORMS[
            "circle"
        ]:
            continue
        cl, _ = _run_e2e(
        ...
        assert cl == DENSER_SOFT_X_GABRIEL_MAJORS_SEED12_INFLATE_KILL_VS_SEED0_KEEP_UNIFORMS[
            "circle"
        ][mode]
        assert sl == DENSER_SOFT_X_GABRIEL_MAJORS_SEED12_INFLATE_KILL_VS_SEED0_KEEP_UNIFORMS[
            "swiss"
        ][mode]

    # denser kills seed1/2 soft inflate; seed0 keep survives lean fracs only
...
def test_denser_soft_keep_band_x_gabriel_x_persist_youden_x_majors_compose() -> None:
    """#44 / A2-T74: denser soft keep×gabriel×persist youden×majors compose seed0.

    Majors soft≤0.12 → tori K=2 (gabriel kills; soft≥0.15 collapses)
    and e2e bare youden nested K=2≈0.01 coexist; soft×persist e2e all
    ≤1. Not sample-ARI recovery; flags off; no awaiting flip.
    """
    ...
    from proteus.stage1.edge_evidence import (
        ...
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_YOUDEN_X_MAJORS_COMPOSE_MAX_GRID_POINTS,
        ...
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_YOUDEN_X_MAJORS_COMPOSE_NESTED_TAU,
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_YOUDEN_X_MAJORS_COMPOSE_SCALE_SEED_BASE,
        ...
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_YOUDEN_X_MAJORS_COMPOSE_TORI_TAU,
    ...
    for frac in DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_YOUDEN_X_MAJORS_COMPOSE_FRACS:
        maj_cfgs[f"soft_{frac:g}"] = proposed_h0_calibrated_config(
            "youden",
            soft_capacity_only=True,
            soft_capacity_frac=float(frac),
    ...
    # compose claim: majors soft keep AND youden e2e singleton coexist
...
def test_denser_soft_x_gabriel_majors_seed0_keep_x_persist_compose_vs_t73_nonpersist() -> None:
    """#44 / A2-T75: denser soft×gabriel seed0 keep×persist vs T73 non-persist.

    Majors soft≤0.12 → tori K=2 (gabriel kills; soft=0.25 collapses);
    e2e non-persist matches T73; soft×persist e2e also ≤1 — persist
    does not rescue T73 soft collapse. Not sample-ARI recovery; flags
    ...
    from proteus.stage1.edge_evidence import (
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_E2E_NONPERSIST_TABLE,
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_E2E_PERSIST_TABLE,
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_FRACS,
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_H0,
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_INFLATE_FRAC,
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_KEEP_MAX_FRAC,
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_MAX_GRID_POINTS,
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_MAX_NODES,
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_NESTED_N,
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_NESTED_TAU,
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_SCALE_SEED_BASE,
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_SEED,
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_TABLE,
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_TORI_N,
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_TORI_TAU,
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_UNIFORMS,
        proposed_h0_calibrated_config,
    )
    from proteus.stage1.scaffold import Stage1Scaffold
    from tests.datasets.synthetic.linked_tori import make_linked_tori
    ...
    for frac in DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_FRACS:
        maj_cfgs[f"soft_{frac:g}"] = proposed_h0_calibrated_config(
            "youden",
            soft_capacity_only=True,
            soft_capacity_frac=float(frac),
    ...
    e2e_np_modes: dict[str, tuple[bool, bool, bool, float]] = {
        "youden": (False, False, False, 0.25),
        "soft_0.05": (True, False, False, 0.05),
        "soft_x_conj_0.05": (True, True, False, 0.05),
        "soft_0.12": (True, False, False, 0.12),
        "soft_x_conj_0.12": (True, True, False, 0.12),
        "soft_0.25": (True, False, False, 0.25),
        "soft_x_conj_0.25": (True, True, False, 0.25),
    }
    for mode, (soft, conj, persist, frac) in e2e_np_modes.items():
        nl, na = _run_e2e(
            nested.points, nested.labels,
            soft=soft, conj=conj, persist=persist, frac=frac,
        )
    ...
    e2e_p_modes: dict[str, tuple[bool, bool, bool, float]] = {
        "youden": (False, False, False, 0.12),
        "persist": (False, False, True, 0.12),
        "conj": (False, True, False, 0.12),
        "conj_x_persist": (False, True, True, 0.12),
    ...
    for frac in DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_FRACS:
        e2e_p_modes[f"soft_x_persist_{frac:g}"] = (True, False, True, frac)
        e2e_p_modes[f"soft_x_conj_x_persist_{frac:g}"] = (
            True, True, True, frac,
        )

    for mode, (soft, conj, persist, frac) in e2e_p_modes.items():
        nl, na = _run_e2e(
            nested.points, nested.labels,
            soft=soft, conj=conj, persist=persist, frac=frac,
        )
    ...
    for mode in uni_modes:
        soft, conj, persist, frac = e2e_p_modes[mode]
        cl, _ = _run_e2e(
            circle.points, None,
            soft=soft, conj=conj, persist=persist, frac=frac,
            min_samples=80,
    ...
    # contrast claim: T73 keep majors + non-persist soft collapse;
    # persist compose does not rescue soft e2e
    assert DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_TABLE[
        "soft_0.12"
    ][2] == 2
    assert DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_TABLE[
        "soft_0.25"
    ][2] <= 1
    assert DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_E2E_NONPERSIST_TABLE[
        "youden"
    ][0] == 2
    assert DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_E2E_NONPERSIST_TABLE[
        "soft_0.12"
    ][0] <= 1
    assert DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_E2E_PERSIST_TABLE[
        "soft_x_persist_0.12"
    ][0] <= 1
    assert DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_E2E_PERSIST_TABLE[
        "soft_x_persist_0.25"
