...
def test_denser_soft_keep_band_x_gabriel_x_persist_e2e_export() -> None:
    """#44 / A2-T65-followon: denser soft keep×gabriel×persist e2e export.

    Soft×persist / soft×conj×persist denser keep-band frac grid all ≤1;
    youden alone nested K=2 chance-ARI≈0.01. Defaults off; no awaiting.
    """
    ...
    from proteus.stage1.edge_evidence import (
        ...
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_SI_NOTE,
        ...
        format_denser_soft_keep_band_x_gabriel_x_persist_e2e_table,
    )

    assert HollowEdgeConfig().soft_capacity_only is False
    assert HollowEdgeConfig().require_gabriel_and_h is False
    assert abs(DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_H0 - 0.73) < 1e-9
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_SEED == 0
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_NESTED_N == 160
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_TORI_N == 240
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_MAX_NODES == 128
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_FRACS == (
        0.05, 0.12, 0.15, 0.25,
    )
    ...
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_TABLE[
        "soft_x_conj_x_persist_0.12"
    ][0] <= 1
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_UNIFORMS["circle"][
        "youden"
    ] == 1
    tsv = format_denser_soft_keep_band_x_gabriel_x_persist_e2e_table()
    assert "soft_x_conj_x_persist_0.12" in tsv and "e2e" in tsv
    assert "persist" in DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_SI_NOTE
    assert "keep-band" in DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_E2E_SI_NOTE
...
def test_denser_soft_x_gabriel_x_persist_seed_inflate_export() -> None:
    """#44 / A2-T67: denser soft×gabriel×persist seed1 inflate export.

    Denser kills T63 seed1 majors soft nested inflate and T63 seed1 e2e
    nested inflate; only seed0 youden remains. Flags off; no awaiting flip.
    """
    ...
    from proteus.stage1.edge_evidence import (
        ...
        DENSER_SOFT_X_GABRIEL_X_PERSIST_SEED_INFLATE_SI_NOTE,
        ...
        HollowEdgeConfig,
        format_denser_soft_x_gabriel_x_persist_seed_inflate_table,
    )
    ...
    assert DENSER_SOFT_X_GABRIEL_X_PERSIST_SEED_INFLATE_TORI_N == 240
    assert DENSER_SOFT_X_GABRIEL_X_PERSIST_SEED_INFLATE_MAX_NODES == 128
    assert DENSER_SOFT_X_GABRIEL_X_PERSIST_SEED_INFLATE_SOFT_FRAC == 0.25
    # denser kills T63 seed1 majors soft inflate
    ...
    # denser kills T63 seed1 e2e inflate under compose
    ...
    # seed0 youden singleton remains
    assert DENSER_SOFT_X_GABRIEL_X_PERSIST_SEED_INFLATE_TABLE[0][
        "youden"
    ][2] == 2
    ...
    tsv = format_denser_soft_x_gabriel_x_persist_seed_inflate_table()
    assert "soft_x_conj_x_persist" in tsv and "majors" in tsv and "e2e" in tsv
    assert "kills T63" in DENSER_SOFT_X_GABRIEL_X_PERSIST_SEED_INFLATE_SI_NOTE
    assert "seed1" in DENSER_SOFT_X_GABRIEL_X_PERSIST_SEED_INFLATE_SI_NOTE
    assert "sample-ARI" in DENSER_SOFT_X_GABRIEL_X_PERSIST_SEED_INFLATE_SI_NOTE
    assert "defaults off" in DENSER_SOFT_X_GABRIEL_X_PERSIST_SEED_INFLATE_SI_NOTE
    assert "awaiting" in DENSER_SOFT_X_GABRIEL_X_PERSIST_SEED_INFLATE_SI_NOTE


def test_denser_soft_keep_band_x_gabriel_x_persist_multiseed_export() -> None:
    """#44 / A2-T68: denser soft keep×gabriel×persist multi-seed e2e export.

    Extends T65 seed0 persist frac grid across seeds 0..2; soft×persist
    / soft×conj×persist all ≤1; only seed0 youden remains. Defaults off.
    """
    ...
    from proteus.stage1.edge_evidence import (
        ...
        format_denser_soft_keep_band_x_gabriel_x_persist_multiseed_table,
    )
    ...
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MULTISEED_SEEDS == (
        0, 1, 2,
    )
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MULTISEED_NESTED_N == 160
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MULTISEED_TORI_N == 240
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MULTISEED_MAX_NODES == 128
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MULTISEED_FRACS == (
        0.05, 0.12, 0.15,
    )
    assert (
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MULTISEED_KEEP_MAX_FRAC
    ...
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MULTISEED_UNIFORMS[
        "circle"
    ]["youden"] == 1
    tsv = format_denser_soft_keep_band_x_gabriel_x_persist_multiseed_table()
    assert "soft_x_persist_0.12" in tsv and "e2e" in tsv
    assert "multi-seed" in (
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MULTISEED_SI_NOTE
    )
    assert "keep-band" in (
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MULTISEED_SI_NOTE
...
def test_denser_soft_keep_band_x_gabriel_x_persist_majors_pin_multiseed_table() -> None:
    """#44 / A2-T72: denser soft keep×gabriel×persist majors-pin multi-seed table."""
    ...
    from proteus.stage1.edge_evidence import (
        ...
        format_denser_soft_keep_band_x_gabriel_x_persist_majors_pin_multiseed_table,
    )

    assert abs(
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_H0 - 0.73
    ) < 1e-9
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_SEEDS == (
        0, 1, 2,
    )
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_NESTED_N == 160
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_TORI_N == 240
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_MAX_NODES == 128
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_FRACS == (
        0.05, 0.12, 0.15,
    )
    assert (
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_KEEP_MAX_FRAC
    ...
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_TABLE[0][
        "soft_0.12"
    ][2] == 2
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_TABLE[0][
        "soft_x_conj_0.12"
    ][2] <= 1
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_TABLE[1][
        "soft_0.12"
    ][2] <= 1
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_TABLE[2][
        "youden"
    ][2] <= 1
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_E2E_TABLE[0][
        "youden"
    ][0] == 2
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_E2E_TABLE[0][
        "soft_x_persist_0.12"
    ][0] <= 1
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_E2E_TABLE[1][
        "youden"
    ][0] <= 1
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_UNIFORMS[
        "circle"
    ]["youden"] == 1
    tsv = format_denser_soft_keep_band_x_gabriel_x_persist_majors_pin_multiseed_table()
    assert "majors-pin multi-seed" in tsv and "soft_0.12" in tsv and "majors" in tsv
    assert "NOT seed-stable" in (
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_SI_NOTE
    )
    assert "multi-seed" in (
...
def test_denser_soft_x_gabriel_majors_seed0_keep_x_persist_compose_vs_t73_nonpersist_table() -> None:
    """#44 / A2-T75: denser soft×gabriel seed0 keep×persist vs T73 non-persist table."""
    ...
    from proteus.stage1.edge_evidence import (
        ...
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_SI_NOTE,
        ...
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED12_INFLATE_KILL_VS_SEED0_KEEP_E2E_TABLE,
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED12_INFLATE_KILL_VS_SEED0_KEEP_TABLE,
        format_denser_soft_x_gabriel_majors_seed0_keep_x_persist_compose_vs_t73_nonpersist_table,
    )
    ...
    # majors match T73 seed0 keep window
    ...
    assert (
        ...
        == DENSER_SOFT_X_GABRIEL_MAJORS_SEED12_INFLATE_KILL_VS_SEED0_KEEP_TABLE[0][
            "soft_0.12"
        ]
    )
    # e2e non-persist matches T73 seed0
    ...
    assert (
        ...
        == DENSER_SOFT_X_GABRIEL_MAJORS_SEED12_INFLATE_KILL_VS_SEED0_KEEP_E2E_TABLE[0][
            "youden"
        ]
    )
    # persist compose does not rescue soft e2e collapse
    ...
    assert DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_UNIFORMS[
        "circle"
    ]["youden"] == 1
    tsv = (
        format_denser_soft_x_gabriel_majors_seed0_keep_x_persist_compose_vs_t73_nonpersist_table()
    )
    assert "T73" in tsv and "soft_x_persist_0.12" in tsv and "e2e_persist" in tsv
    assert "T73" in (
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_SI_NOTE
    )
    assert "does not rescue" in (
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_SI_NOTE
    )
    assert "sample-ARI" in (
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_SI_NOTE
...
def test_denser_soft_keep_band_x_gabriel_x_persist_majors_pin_seed0_ari_probe_table() -> None:
    """#44 / A2-T76: denser soft×gabriel×persist majors-pin seed0 ARI probe table."""
    ...
    from proteus.stage1.edge_evidence import (
        ...
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_SI_NOTE,
        ...
        DENSER_SOFT_SEED0_TORI_ARI_WINDOW_FRACS,
        DENSER_SOFT_SEED0_TORI_ARI_WINDOW_TABLE,
        format_denser_soft_keep_band_x_gabriel_x_persist_majors_pin_seed0_ari_probe_table,
    )
    ...
    # majors keep band matches T55 soft ARI window
    ...
    assert (
        ...
        == DENSER_SOFT_SEED0_TORI_ARI_WINDOW_TABLE["soft_0.12"][2:]
    )
    ...
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_UNIFORMS[
        "circle"
    ]["youden"] == 1
    assert DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_UNIFORMS[
        "swiss"
    ]["soft_x_persist_0.12"] == 1
    tsv = format_denser_soft_keep_band_x_gabriel_x_persist_majors_pin_seed0_ari_probe_table()
    assert "majors-pin seed0-only ARI probe" in tsv
    assert "soft_0.08" in tsv
    assert "A2-T76" in (
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_SI_NOTE
    )
    assert "sample-ARI" in (
