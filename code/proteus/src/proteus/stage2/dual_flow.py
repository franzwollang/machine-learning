...
* **S6.2** loopy Gaussian BP conservative reconstruction (real factor-graph
  ...
  *cap-sweep residual trajectory export* lands behind
  ...
  (A5-T83; harness only; does not flip ``@awaiting``). A spectrum-safe
  × policy × mass_loopy *patience×cap grid* lands behind
  ``enable_spectrum_safe_policy_mass_patience_cap_probe`` (A5-T84;
  harness only; does not flip ``@awaiting``). A residual-stop ×
  mass_loopy × policy-in-loopy *patience×cap grid* lands behind
  ``enable_residual_mass_policy_patience_cap_probe`` (A5-T85;
  proposal-path; does not flip ``@awaiting``). A spectrum×policy×mass
  patience×cap × fail_closed EvidenceGate *bridge* lands behind
  ``enable_spectrum_safe_policy_mass_patience_cap_fail_closed_bridge_probe``
  ...
  (A5-T90; harness only; does not flip ``@awaiting``). A spectrum-safe
  × policy × mass_loopy *patience×cap traj export* lands behind
  ``enable_spectrum_safe_policy_mass_patience_cap_traj_probe``
  (A5-T91; harness only; does not flip ``@awaiting``). A residual-stop ×
  mass_loopy × policy patience×cap × fail_closed *matrix* lands behind
  ``enable_residual_mass_policy_patience_cap_fail_closed_matrix_probe``
  (A5-T92; harness only; does not flip ``@awaiting``). A spectrum×policy×mass
  patience×cap traj × fail_closed *bridge* lands behind
  ``enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_bridge_probe``
  (A5-T93; harness only; does not flip ``@awaiting``). A residual-stop ×
  mass_loopy × policy patience×cap traj × fail_closed dry_run×EvidenceGate
  *bridge* lands behind
  ``enable_residual_mass_policy_patience_cap_traj_fail_closed_dry_run_probe``
  (A5-T94; harness only; does not flip ``@awaiting``). A spectrum×policy×mass
  patience×cap traj × fail_closed EvidenceGate *matrix* lands behind
  ``enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_matrix_probe``
  (A5-T95; harness only; does not flip ``@awaiting``). A residual-stop ×
  mass_loopy × policy patience×cap × fail_closed dry_run×EvidenceGate
  *bridge* lands behind
  ``enable_residual_mass_policy_patience_cap_fail_closed_dry_run_probe``
  (A5-T96; harness only; does not flip ``@awaiting``). A spectrum×policy×mass
  patience×cap traj × fail_closed dry_run×EvidenceGate *bridge* lands behind
  ``enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_dry_run_probe``
  (A5-T97; harness only; does not flip ``@awaiting``). A residual-stop ×
  mass_loopy × policy patience×cap traj × fail_closed *matrix* lands behind
  ``enable_residual_mass_policy_patience_cap_traj_fail_closed_matrix_probe``
  (A5-T98; harness only; does not flip ``@awaiting``). A residual-stop ×
  mass_loopy × policy patience×cap × fail_closed *reconnect bridge* lands
  behind
  ``enable_residual_mass_policy_patience_cap_fail_closed_bridge_probe``
  (A5-T99; harness only; does not flip ``@awaiting``). A residual×spectrum
  dual-path patience×cap × fail_closed *compose* lands behind
  ``enable_residual_spectrum_dual_path_patience_cap_fail_closed_compose_probe``
  (A5-T100; harness only; does not flip ``@awaiting``). A spectrum×policy×mass
  patience×cap traj × fail_closed dry_run×reconnect *bridge* lands behind
  ``enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_dry_run_reconnect_probe``
  (A5-T101; harness only; does not flip ``@awaiting``). A residual-stop ×
  mass_loopy × policy patience×cap traj × fail_closed matrix×dry_run
  *EvidenceGate triple* lands behind
  ``enable_residual_mass_policy_patience_cap_traj_fail_closed_matrix_dry_run_probe``
  (A5-T102; harness only; does not flip ``@awaiting``). A residual×spectrum
  dual-path patience×cap×traj *compose* lands behind
  ``enable_residual_spectrum_dual_path_patience_cap_traj_compose_probe``
  (A5-T103; harness only; does not flip ``@awaiting``). A spectrum×policy×mass
  patience×cap traj × fail_closed dry_run×matrix×reconnect *bridge* lands
  behind
  ``enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_dry_run_matrix_reconnect_probe``
  (A5-T104; harness only; does not flip ``@awaiting``). A residual×spectrum
  dual-path × fail_closed dry_run EG *compose* lands behind
  ``enable_residual_spectrum_dual_path_fail_closed_dry_run_compose_probe``
  (A5-T105; harness only; does not flip ``@awaiting``). A residual×spectrum
  dual-path patience×cap×traj × fail_closed dry_run EG *compose* lands
  behind
  ``enable_residual_spectrum_dual_path_patience_cap_traj_fail_closed_dry_run_compose_probe``
  (A5-T106; harness only; does not flip ``@awaiting``). A residual×spectrum
  dual-path patience×cap×traj × fail_closed reconnect EG *compose* lands
  behind
  ``enable_residual_spectrum_dual_path_patience_cap_traj_fail_closed_reconnect_compose_probe``
  (A5-T107; harness only; does not flip ``@awaiting``). A residual×spectrum
  dual-path patience×cap×traj × fail_closed matrix EG *compose* lands
  behind
  ``enable_residual_spectrum_dual_path_patience_cap_traj_fail_closed_matrix_compose_probe``
  (A5-T108; harness only; does not flip ``@awaiting``). A residual×spectrum
  dual-path patience×cap×traj × fail_closed dry_run×matrix×reconnect
  *mega-compose* lands behind
  ``enable_residual_spectrum_dual_path_patience_cap_traj_fail_closed_dry_run_matrix_reconnect_compose_probe``
  (A5-T109; harness only; does not flip ``@awaiting``). A residual×mass×policy
  patience×cap traj × fail_closed dry_run×matrix×reconnect *bridge* lands
  behind
  ``enable_residual_mass_policy_patience_cap_traj_fail_closed_dry_run_matrix_reconnect_probe``
  (A5-T110; harness only; does not flip ``@awaiting``). A residual×spectrum
  dual-path patience×cap (non-traj) × fail_closed dry_run×matrix×reconnect
  *compose* lands behind
  ``enable_residual_spectrum_dual_path_patience_cap_fail_closed_dry_run_matrix_reconnect_compose_probe``
  (A5-T111; harness only; does not flip ``@awaiting``). Remaining
...
* ``DualFlowConfig.enable_spectrum_safe_policy_mass_patience_probe`` —
  when off, :func:`probe_spectrum_safe_policy_mass_patience` returns
...
* ``DualFlowConfig.enable_spectrum_safe_policy_mass_patience_cap_probe``
  — when off, :func:`probe_spectrum_safe_policy_mass_patience_cap`
  returns ``None``; when on, sweeps ``bp_residual_stop_patience`` ×
  ``spectrum_cond_cap`` under spectrum-safe×policy pin + mass×loopy
  compose (A5-T84; harness only — **not** a production certificate;
  does not flip ``@awaiting``).
* ``DualFlowConfig.enable_residual_mass_policy_patience_cap_probe`` —
  when off, :func:`probe_residual_mass_policy_patience_cap` returns
  ``None``; when on, sweeps patience × ``spectrum_cond_cap`` under
  mass×loopy compose with residual-stop **and**
  ``enable_bp_policy_in_loopy`` (A5-T85; proposal-path; does not flip
  ``@awaiting``).
* ``DualFlowConfig.enable_spectrum_safe_policy_mass_patience_cap_fail_closed_bridge_probe``
  — when off,
  :func:`probe_spectrum_safe_policy_mass_patience_cap_fail_closed_bridge`
  returns ``None``; when on, bridges spectrum×policy×mass
  patience×cap grid with fail_closed dry_run reconnect×EvidenceGate
  (A5-T86; harness only; does not flip ``@awaiting`` / GateConfig
  defaults).
* ``DualFlowConfig.enable_residual_mass_policy_patience_cap_traj_probe``
  — when off, :func:`probe_residual_mass_policy_patience_cap_traj`
...
* ``DualFlowConfig.enable_spectrum_safe_policy_mass_patience_cap_traj_probe``
  — when off, :func:`probe_spectrum_safe_policy_mass_patience_cap_traj`
  returns ``None``; when on, sweeps patience × ``spectrum_cond_cap``
  exporting residual trajectories under spectrum-safe×policy×mass
  compose (A5-T91; harness only; does not flip ``@awaiting``).
* ``DualFlowConfig.enable_residual_mass_policy_patience_cap_fail_closed_matrix_probe``
  — when off,
  :func:`probe_residual_mass_policy_patience_cap_fail_closed_matrix`
  returns ``None``; when on, bridges residual×mass×policy patience×cap
  with fail_closed EvidenceGate.evaluate matrix (A5-T92; harness only;
  does not flip ``@awaiting`` / GateConfig defaults).
* ``DualFlowConfig.enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_bridge_probe``
  — when off,
  :func:`probe_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_bridge`
  returns ``None``; when on, bridges spectrum×policy×mass patience×cap
  traj with fail_closed dry_run reconnect×EvidenceGate (A5-T93; harness
  only; does not flip ``@awaiting`` / GateConfig defaults).
* ``DualFlowConfig.enable_residual_mass_policy_patience_cap_traj_fail_closed_dry_run_probe``
  — when off,
  :func:`probe_residual_mass_policy_patience_cap_traj_fail_closed_dry_run`
  returns ``None``; when on, bridges residual×mass×policy patience×cap
  traj with fail_closed dry_run×EvidenceGate (A5-T94; harness only; does
  not flip ``@awaiting`` / GateConfig defaults).
* ``DualFlowConfig.enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_matrix_probe``
  — when off,
  :func:`probe_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_matrix`
  returns ``None``; when on, bridges spectrum×policy×mass patience×cap
  traj with fail_closed EvidenceGate.evaluate matrix (A5-T95; harness
  only; does not flip ``@awaiting`` / GateConfig defaults).
* ``DualFlowConfig.enable_residual_mass_policy_patience_cap_fail_closed_dry_run_probe``
  — when off,
  :func:`probe_residual_mass_policy_patience_cap_fail_closed_dry_run`
  returns ``None``; when on, bridges residual×mass×policy patience×cap
  with fail_closed dry_run×EvidenceGate (A5-T96; harness only; does not
  flip ``@awaiting`` / GateConfig defaults).
* ``DualFlowConfig.enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_dry_run_probe``
  — when off,
  :func:`probe_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_dry_run`
  returns ``None``; when on, bridges spectrum×policy×mass patience×cap
  traj with fail_closed dry_run×EvidenceGate (A5-T97; harness only; does
  not flip ``@awaiting`` / GateConfig defaults).
* ``DualFlowConfig.enable_residual_mass_policy_patience_cap_traj_fail_closed_matrix_probe``
  — when off,
  :func:`probe_residual_mass_policy_patience_cap_traj_fail_closed_matrix`
  returns ``None``; when on, bridges residual×mass×policy patience×cap
  traj with fail_closed EvidenceGate.evaluate matrix (A5-T98; harness
  only; does not flip ``@awaiting`` / GateConfig defaults).
* ``DualFlowConfig.enable_residual_mass_policy_patience_cap_fail_closed_bridge_probe``
  — when off,
  :func:`probe_residual_mass_policy_patience_cap_fail_closed_bridge`
  returns ``None``; when on, bridges residual×mass×policy patience×cap
  with fail_closed dry_run reconnect×EvidenceGate (A5-T99; harness only;
  does not flip ``@awaiting`` / GateConfig defaults).
* ``DualFlowConfig.enable_residual_spectrum_dual_path_patience_cap_fail_closed_compose_probe``
  — when off,
  :func:`probe_residual_spectrum_dual_path_patience_cap_fail_closed_compose`
  returns ``None``; when on, composes residual×mass×policy patience×cap
  with spectrum×policy×mass patience×cap under a shared fail_closed
  reconnect×EvidenceGate (A5-T100; harness only; does not flip
  ``@awaiting`` / GateConfig defaults).
* ``DualFlowConfig.enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_dry_run_reconnect_probe``
  — when off,
  :func:`probe_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_dry_run_reconnect`
  returns ``None``; when on, bridges spectrum×policy×mass patience×cap
  traj with fail_closed dry_run×EvidenceGate *and* reconnect×EvidenceGate
  (A5-T101; harness only; does not flip ``@awaiting`` / GateConfig defaults).
* ``DualFlowConfig.enable_residual_mass_policy_patience_cap_traj_fail_closed_matrix_dry_run_probe``
  — when off,
  :func:`probe_residual_mass_policy_patience_cap_traj_fail_closed_matrix_dry_run`
  returns ``None``; when on, bridges residual×mass×policy patience×cap
  traj with fail_closed EvidenceGate.evaluate matrix *and*
  dry_run×EvidenceGate (A5-T102; harness only; does not flip ``@awaiting``
  / GateConfig defaults).
* ``DualFlowConfig.enable_residual_spectrum_dual_path_patience_cap_traj_compose_probe``
  — when off,
  :func:`probe_residual_spectrum_dual_path_patience_cap_traj_compose`
  returns ``None``; when on, composes residual×mass×policy patience×cap
  traj with spectrum×policy×mass patience×cap traj (A5-T103; harness
  only; does not flip ``@awaiting`` / GateConfig defaults).
* ``DualFlowConfig.enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_dry_run_matrix_reconnect_probe``
  — when off,
  :func:`probe_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_dry_run_matrix_reconnect`
  returns ``None``; when on, bridges spectrum×policy×mass patience×cap
  traj with fail_closed dry_run×matrix×reconnect EvidenceGate paths
  (A5-T104; harness only; does not flip ``@awaiting`` / GateConfig defaults).
* ``DualFlowConfig.enable_residual_spectrum_dual_path_fail_closed_dry_run_compose_probe``
  — when off,
  :func:`probe_residual_spectrum_dual_path_fail_closed_dry_run_compose`
  returns ``None``; when on, composes residual×mass×policy patience×cap
  with spectrum×policy×mass patience×cap under a shared fail_closed
  dry_run×EvidenceGate (A5-T105; harness only; does not flip ``@awaiting``
  / GateConfig defaults).
* ``DualFlowConfig.enable_residual_spectrum_dual_path_patience_cap_traj_fail_closed_dry_run_compose_probe``
  — when off,
  :func:`probe_residual_spectrum_dual_path_patience_cap_traj_fail_closed_dry_run_compose`
  returns ``None``; when on, composes residual×spectrum patience×cap traj
  dual-paths under a shared fail_closed dry_run×EvidenceGate (A5-T106;
  harness only; does not flip ``@awaiting`` / GateConfig defaults).
* ``DualFlowConfig.enable_residual_spectrum_dual_path_patience_cap_traj_fail_closed_reconnect_compose_probe``
  — when off,
  :func:`probe_residual_spectrum_dual_path_patience_cap_traj_fail_closed_reconnect_compose`
  returns ``None``; when on, composes residual×spectrum patience×cap traj
  dual-paths under a shared fail_closed reconnect×EvidenceGate (A5-T107;
  harness only; does not flip ``@awaiting`` / GateConfig defaults).
* ``DualFlowConfig.enable_residual_spectrum_dual_path_patience_cap_traj_fail_closed_matrix_compose_probe``
  — when off,
  :func:`probe_residual_spectrum_dual_path_patience_cap_traj_fail_closed_matrix_compose`
  returns ``None``; when on, composes residual×spectrum patience×cap traj
  dual-paths under a shared fail_closed EvidenceGate.evaluate matrix
  (A5-T108; harness only; does not flip ``@awaiting`` / GateConfig defaults).
* ``DualFlowConfig.enable_residual_spectrum_dual_path_patience_cap_traj_fail_closed_dry_run_matrix_reconnect_compose_probe``
  — when off,
  :func:`probe_residual_spectrum_dual_path_patience_cap_traj_fail_closed_dry_run_matrix_reconnect_compose`
  returns ``None``; when on, composes residual×spectrum patience×cap traj
  dual-paths under shared fail_closed dry_run×matrix×reconnect EvidenceGate
  paths (A5-T109; harness only; does not flip ``@awaiting`` / GateConfig
  defaults).
* ``DualFlowConfig.enable_residual_mass_policy_patience_cap_traj_fail_closed_dry_run_matrix_reconnect_probe``
  — when off,
  :func:`probe_residual_mass_policy_patience_cap_traj_fail_closed_dry_run_matrix_reconnect`
  returns ``None``; when on, bridges residual×mass×policy patience×cap traj
  with fail_closed dry_run×matrix×reconnect EvidenceGate paths (A5-T110;
  harness only; does not flip ``@awaiting`` / GateConfig defaults).
* ``DualFlowConfig.enable_residual_spectrum_dual_path_patience_cap_fail_closed_dry_run_matrix_reconnect_compose_probe``
  — when off,
  :func:`probe_residual_spectrum_dual_path_patience_cap_fail_closed_dry_run_matrix_reconnect_compose`
  returns ``None``; when on, composes residual×spectrum patience×cap
  (non-traj) dual-paths under shared fail_closed dry_run×matrix×reconnect
  EvidenceGate paths (A5-T111; harness only; does not flip ``@awaiting`` /
  GateConfig defaults).
...
__all__ = [
    ...
    "probe_spectrum_safe_policy_mass_patience_cap_traj",
    "probe_residual_mass_policy_patience_cap_fail_closed_matrix",
    "probe_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_bridge",
    "probe_residual_mass_policy_patience_cap_traj_fail_closed_dry_run",
    "probe_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_matrix",
    "probe_residual_mass_policy_patience_cap_fail_closed_dry_run",
    ...
    "probe_residual_spectrum_dual_path_patience_cap_traj_compose",
    "probe_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_dry_run_matrix_reconnect",
    "probe_residual_spectrum_dual_path_fail_closed_dry_run_compose",
    "probe_residual_spectrum_dual_path_patience_cap_traj_fail_closed_dry_run_compose",
    "probe_residual_spectrum_dual_path_patience_cap_traj_fail_closed_reconnect_compose",
    "probe_residual_spectrum_dual_path_patience_cap_traj_fail_closed_matrix_compose",
    "probe_residual_spectrum_dual_path_patience_cap_traj_fail_closed_dry_run_matrix_reconnect_compose",
    "probe_residual_mass_policy_patience_cap_traj_fail_closed_dry_run_matrix_reconnect",
    "probe_residual_spectrum_dual_path_patience_cap_fail_closed_dry_run_matrix_reconnect_compose",
...
class DualFlowConfig:
    ...
    enable_spectrum_safe_policy_mass_patience_probe:
        When ``False`` (default),
        :func:`probe_spectrum_safe_policy_mass_patience` returns
        ``None``. When ``True``, sweeps ``bp_residual_stop_patience``
    ...
    enable_spectrum_policy_mass_traj_fail_closed_bridge_probe:
        ...
        harness only; does not flip ``@awaiting`` / GateConfig).
    enable_spectrum_safe_policy_mass_patience_cap_probe:
        When ``False`` (default),
        :func:`probe_spectrum_safe_policy_mass_patience_cap` returns
        ``None``. When ``True``, sweeps patience × ``spectrum_cond_cap``
        under spectrum-safe×policy pin + mass×loopy compose (A5-T84;
        harness only — **not** a production certificate; does not
        flip ``@awaiting``).
    enable_residual_mass_policy_patience_cap_probe:
        When ``False`` (default),
        :func:`probe_residual_mass_policy_patience_cap` returns
        ``None``. When ``True``, sweeps patience × ``spectrum_cond_cap``
        under mass×loopy compose with residual-stop **and**
        ``enable_bp_policy_in_loopy`` (A5-T85; proposal-path; does not
        flip mass/density ``@awaiting``).
    enable_spectrum_safe_policy_mass_patience_cap_fail_closed_bridge_probe:
        When ``False`` (default),
        :func:`probe_spectrum_safe_policy_mass_patience_cap_fail_closed_bridge`
        returns ``None``. When ``True``, bridges spectrum×policy×mass
        patience×cap grid with fail_closed dry_run
        reconnect×EvidenceGate (A5-T86; harness only; does not flip
        ``@awaiting`` / GateConfig).
    enable_residual_mass_policy_patience_cap_traj_probe:
        When ``False`` (default),
        :func:`probe_residual_mass_policy_patience_cap_traj` returns
        ``None``. When ``True``, sweeps patience × ``spectrum_cond_cap``
    ...
    enable_spectrum_safe_policy_mass_patience_cap_traj_probe:
        When ``False`` (default),
        :func:`probe_spectrum_safe_policy_mass_patience_cap_traj` returns
        ``None``. When ``True``, sweeps patience × ``spectrum_cond_cap``
        exporting residual trajectories under spectrum-safe×policy×mass
        compose (A5-T91; harness only — **not** a production certificate;
        does not flip ``@awaiting``).
    enable_residual_mass_policy_patience_cap_fail_closed_matrix_probe:
        When ``False`` (default),
        :func:`probe_residual_mass_policy_patience_cap_fail_closed_matrix`
        returns ``None``. When ``True``, bridges residual×mass×policy
        patience×cap with fail_closed EvidenceGate.evaluate matrix
        (A5-T92; harness only; does not flip ``@awaiting`` / GateConfig).
    enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_bridge_probe:
        When ``False`` (default),
        :func:`probe_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_bridge`
        returns ``None``. When ``True``, bridges spectrum×policy×mass
        patience×cap traj with fail_closed dry_run
        reconnect×EvidenceGate (A5-T93; harness only; does not flip
        ``@awaiting`` / GateConfig).
    enable_residual_mass_policy_patience_cap_traj_fail_closed_dry_run_probe:
        When ``False`` (default),
        :func:`probe_residual_mass_policy_patience_cap_traj_fail_closed_dry_run`
        returns ``None``. When ``True``, bridges residual×mass×policy
        patience×cap traj with fail_closed dry_run×EvidenceGate
        (A5-T94; harness only; does not flip ``@awaiting`` / GateConfig).
    enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_matrix_probe:
        When ``False`` (default),
        :func:`probe_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_matrix`
        returns ``None``. When ``True``, bridges spectrum×policy×mass
        patience×cap traj with fail_closed EvidenceGate.evaluate matrix
        (A5-T95; harness only; does not flip ``@awaiting`` / GateConfig).
    enable_residual_mass_policy_patience_cap_fail_closed_dry_run_probe:
        When ``False`` (default),
        :func:`probe_residual_mass_policy_patience_cap_fail_closed_dry_run`
        returns ``None``. When ``True``, bridges residual×mass×policy
        patience×cap with fail_closed dry_run×EvidenceGate (A5-T96;
        harness only; does not flip ``@awaiting`` / GateConfig).
    enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_dry_run_probe:
        When ``False`` (default),
        :func:`probe_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_dry_run`
        returns ``None``. When ``True``, bridges spectrum×policy×mass
        patience×cap traj with fail_closed dry_run×EvidenceGate
        (A5-T97; harness only; does not flip ``@awaiting`` / GateConfig).
    enable_residual_mass_policy_patience_cap_traj_fail_closed_matrix_probe:
        When ``False`` (default),
        :func:`probe_residual_mass_policy_patience_cap_traj_fail_closed_matrix`
        returns ``None``. When ``True``, bridges residual×mass×policy
        patience×cap traj with fail_closed EvidenceGate.evaluate matrix
        (A5-T98; harness only; does not flip ``@awaiting`` / GateConfig).
    enable_residual_mass_policy_patience_cap_fail_closed_bridge_probe:
        When ``False`` (default),
        :func:`probe_residual_mass_policy_patience_cap_fail_closed_bridge`
        returns ``None``. When ``True``, bridges residual×mass×policy
        patience×cap with fail_closed dry_run reconnect×EvidenceGate
        (A5-T99; harness only; does not flip ``@awaiting`` / GateConfig).
    enable_residual_spectrum_dual_path_patience_cap_fail_closed_compose_probe:
        When ``False`` (default),
        :func:`probe_residual_spectrum_dual_path_patience_cap_fail_closed_compose`
        returns ``None``. When ``True``, composes residual and spectrum
        patience×cap dual-paths with fail_closed reconnect×EvidenceGate
        (A5-T100; harness only; does not flip ``@awaiting`` / GateConfig).
    enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_dry_run_reconnect_probe:
        When ``False`` (default),
        :func:`probe_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_dry_run_reconnect`
        returns ``None``. When ``True``, bridges spectrum patience×cap
        traj with fail_closed dry_run and reconnect EvidenceGate paths
        (A5-T101; harness only; does not flip ``@awaiting`` / GateConfig).
    enable_residual_mass_policy_patience_cap_traj_fail_closed_matrix_dry_run_probe:
        When ``False`` (default),
        :func:`probe_residual_mass_policy_patience_cap_traj_fail_closed_matrix_dry_run`
        returns ``None``. When ``True``, bridges residual patience×cap
        traj with fail_closed matrix and dry_run EvidenceGate paths
        (A5-T102; harness only; does not flip ``@awaiting`` / GateConfig).
    enable_residual_spectrum_dual_path_patience_cap_traj_compose_probe:
        When ``False`` (default),
        :func:`probe_residual_spectrum_dual_path_patience_cap_traj_compose`
        returns ``None``. When ``True``, composes residual and spectrum
        patience×cap traj dual-paths (A5-T103; harness only; does not
        flip ``@awaiting`` / GateConfig).
    enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_dry_run_matrix_reconnect_probe:
        When ``False`` (default),
        :func:`probe_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_dry_run_matrix_reconnect`
        returns ``None``. When ``True``, bridges spectrum patience×cap
        traj with fail_closed dry_run, matrix, and reconnect EvidenceGate
        paths (A5-T104; harness only; does not flip ``@awaiting`` /
        GateConfig).
    enable_residual_spectrum_dual_path_fail_closed_dry_run_compose_probe:
        When ``False`` (default),
        :func:`probe_residual_spectrum_dual_path_fail_closed_dry_run_compose`
        returns ``None``. When ``True``, composes residual and spectrum
        patience×cap dual-paths with fail_closed dry_run×EvidenceGate
        (A5-T105; harness only; does not flip ``@awaiting`` / GateConfig).
    enable_residual_spectrum_dual_path_patience_cap_traj_fail_closed_dry_run_compose_probe:
        When ``False`` (default),
        :func:`probe_residual_spectrum_dual_path_patience_cap_traj_fail_closed_dry_run_compose`
        returns ``None``. When ``True``, composes residual×spectrum
        patience×cap traj dual-paths with fail_closed dry_run×EvidenceGate
        (A5-T106; harness only; does not flip ``@awaiting`` / GateConfig).
    enable_residual_spectrum_dual_path_patience_cap_traj_fail_closed_reconnect_compose_probe:
        When ``False`` (default),
        :func:`probe_residual_spectrum_dual_path_patience_cap_traj_fail_closed_reconnect_compose`
        returns ``None``. When ``True``, composes residual×spectrum
        patience×cap traj dual-paths with fail_closed reconnect×EvidenceGate
        (A5-T107; harness only; does not flip ``@awaiting`` / GateConfig).
    enable_residual_spectrum_dual_path_patience_cap_traj_fail_closed_matrix_compose_probe:
        When ``False`` (default),
        :func:`probe_residual_spectrum_dual_path_patience_cap_traj_fail_closed_matrix_compose`
        returns ``None``. When ``True``, composes residual×spectrum
        patience×cap traj dual-paths with fail_closed EvidenceGate matrix
        (A5-T108; harness only; does not flip ``@awaiting`` / GateConfig).
    enable_residual_spectrum_dual_path_patience_cap_traj_fail_closed_dry_run_matrix_reconnect_compose_probe:
        When ``False`` (default),
        :func:`probe_residual_spectrum_dual_path_patience_cap_traj_fail_closed_dry_run_matrix_reconnect_compose`
        returns ``None``. When ``True``, composes residual×spectrum
        patience×cap traj dual-paths with fail_closed dry_run×matrix×
        reconnect EvidenceGate paths (A5-T109; harness only; does not flip
        ``@awaiting`` / GateConfig).
    enable_residual_mass_policy_patience_cap_traj_fail_closed_dry_run_matrix_reconnect_probe:
        When ``False`` (default),
        :func:`probe_residual_mass_policy_patience_cap_traj_fail_closed_dry_run_matrix_reconnect`
        returns ``None``. When ``True``, bridges residual patience×cap traj
        with fail_closed dry_run×matrix×reconnect EvidenceGate paths
        (A5-T110; harness only; does not flip ``@awaiting`` / GateConfig).
    enable_residual_spectrum_dual_path_patience_cap_fail_closed_dry_run_matrix_reconnect_compose_probe:
        When ``False`` (default),
        :func:`probe_residual_spectrum_dual_path_patience_cap_fail_closed_dry_run_matrix_reconnect_compose`
        returns ``None``. When ``True``, composes residual×spectrum
        patience×cap (non-traj) dual-paths with fail_closed dry_run×matrix×
        reconnect EvidenceGate paths (A5-T111; harness only; does not flip
        ``@awaiting`` / GateConfig).
    ...
    enable_spectrum_safe_policy_mass_patience_cap_probe: bool = False
    ...
    enable_spectrum_safe_policy_mass_patience_cap_traj_probe: bool = False
    enable_residual_mass_policy_patience_cap_fail_closed_matrix_probe: bool = False
    enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_bridge_probe: bool = False
    enable_residual_mass_policy_patience_cap_traj_fail_closed_dry_run_probe: bool = False
    enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_matrix_probe: bool = False
    enable_residual_mass_policy_patience_cap_fail_closed_dry_run_probe: bool = False
    enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_dry_run_probe: bool = False
    enable_residual_mass_policy_patience_cap_traj_fail_closed_matrix_probe: bool = False
    enable_residual_mass_policy_patience_cap_fail_closed_bridge_probe: bool = False
    enable_residual_spectrum_dual_path_patience_cap_fail_closed_compose_probe: bool = False
    enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_dry_run_reconnect_probe: bool = False
    enable_residual_mass_policy_patience_cap_traj_fail_closed_matrix_dry_run_probe: bool = False
    enable_residual_spectrum_dual_path_patience_cap_traj_compose_probe: bool = False
    enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_dry_run_matrix_reconnect_probe: bool = False
    enable_residual_spectrum_dual_path_fail_closed_dry_run_compose_probe: bool = False
    enable_residual_spectrum_dual_path_patience_cap_traj_fail_closed_dry_run_compose_probe: bool = False
    enable_residual_spectrum_dual_path_patience_cap_traj_fail_closed_reconnect_compose_probe: bool = False
    enable_residual_spectrum_dual_path_patience_cap_traj_fail_closed_matrix_compose_probe: bool = False
    enable_residual_spectrum_dual_path_patience_cap_traj_fail_closed_dry_run_matrix_reconnect_compose_probe: bool = False
    enable_residual_mass_policy_patience_cap_traj_fail_closed_dry_run_matrix_reconnect_probe: bool = False
    enable_residual_spectrum_dual_path_patience_cap_fail_closed_dry_run_matrix_reconnect_compose_probe: bool = False
...
@dataclass(frozen=True)
class SpectrumSafePolicyMassPatienceCase:
    """One patience cell of spectrum×policy×mass compose (A5-T81)."""
...
@dataclass(frozen=True)
class SpectrumSafePolicyMassPatienceProbe:
    """Spectrum-safe × policy × mass_loopy patience compose (SI S6.2; A5-T81).

    Mass-normalizes online winners once, then for each patience value
    pins the T70 spectrum-safe×policy residual-stop harness sketch and
    runs online→offline loopy compose under policy-in-loopy + residual-
    stop at ``spectrum_cond_cap``. Harness only — **not** a production
    certificate. Do **not** flip mass/density ``@awaiting``.
    """
    ...
    patience_grid: tuple[int, ...]
    spectrum_cond_cap: float
    cases: tuple[SpectrumSafePolicyMassPatienceCase, ...]
    note: str = (
        "harness only: spectrum-safe × policy-in-loopy × mass_loopy "
        "patience compose; NOT a certified production convergence "
        "proof; do not flip mass/density @awaiting"
    )


def probe_spectrum_safe_policy_mass_patience(
    samples: Sequence[np.ndarray],
    simplex_positions: Mapping[Hashable, np.ndarray],
    simplices: Sequence[Sequence[Hashable]]
    | Mapping[Hashable, Sequence[Hashable]],
    *,
    ...
    patience_grid: Sequence[int] | None = None,
    config: DualFlowConfig | None = None,
) -> SpectrumSafePolicyMassPatienceProbe | None:
    """Sweep residual-stop patience under spectrum×policy×mass compose (A5-T81).

    When ``enable_spectrum_safe_policy_mass_patience_probe`` is off,
    returns ``None``. When on, for each patience in ``patience_grid``
    (default ``(1, 2, 4, 8)``):

    1. **Spectrum-safe × policy pin** — :func:`solve_loopy_bp_schedule`
       with policy-in-loopy + residual-stop early-exit at
       ``config.spectrum_cond_cap`` (same ``spectrum_safe_sketch_ok``
       rule as T70).
    2. **Mass × compose** — mass-normalize online winners (once) and
       run online→offline loopy compose with policy + residual-stop at
       that patience / cap.
    ...
    cfg = config or DualFlowConfig()
    if not cfg.enable_spectrum_safe_policy_mass_patience_probe:
        return None
    if not samples:
        raise ValueError("samples must be non-empty")
    if not simplex_positions:
    ...
    if not mass_map:
        raise ValueError(
            "masses must be non-empty for spectrum×policy×mass patience"
        )
    ...
    cap = float(cfg.spectrum_cond_cap)
    max_iters = max(int(cfg.bp_max_iters), max(grid) + 2, 4)
    cases: list[SpectrumSafePolicyMassPatienceCase] = []
    for patience in grid:
        pin_cfg = DualFlowConfig(
            enable_loopy_bp_schedule=True,
            enable_loopy_bp_residual_stop=True,
        ...
        if pin is None:
            raise RuntimeError(
                "loopy BP unexpectedly None under spectrum×policy×mass patience pin cfg"
            )
        rd = float(pin.r_data)
        rc = float(pin.r_cons)
        reason = pin.residual_stop_reason
        ...
        if compose is None:
            raise RuntimeError(
                "loopy compose unexpectedly None under spectrum×policy×mass patience cfg"
            )

        cases.append(
            SpectrumSafePolicyMassPatienceCase(
                patience=int(patience),
                spectrum_cond_cap=cap,
                residual_stop_reason=reason,
                iters_executed=int(pin.iters),
    ...
    return SpectrumSafePolicyMassPatienceProbe(
        probe_flag_default_off=not DualFlowConfig().enable_spectrum_safe_policy_mass_patience_probe,
        patience_grid=grid,
        spectrum_cond_cap=cap,
        cases=tuple(cases),
    )
...
@dataclass(frozen=True)
class SpectrumSafePolicyMassPatienceCapCase:
    """One (patience, cap) cell of spectrum×policy×mass grid (A5-T84)."""
...
@dataclass(frozen=True)
class SpectrumSafePolicyMassPatienceCapProbe:
    """Spectrum-safe × policy × mass_loopy patience×cap grid (SI S6.2; A5-T84).

    Mass-normalizes online winners once, then for each
    ``(patience, spectrum_cond_cap)`` pair pins the T70
    spectrum-safe×policy residual-stop harness sketch and runs
    online→offline loopy compose under policy-in-loopy + residual-stop.
    Harness only — **not** a production certificate. Do **not** flip
    mass/density ``@awaiting``.
    """
    ...
    patience_grid: tuple[int, ...]
    caps: tuple[float, ...]
    cases: tuple[SpectrumSafePolicyMassPatienceCapCase, ...]
    note: str = (
        "harness only: spectrum-safe × policy-in-loopy × mass_loopy "
        "patience×cap grid; NOT a certified production convergence "
        "proof; do not flip mass/density @awaiting"
    )


def probe_spectrum_safe_policy_mass_patience_cap(
    samples: Sequence[np.ndarray],
    simplex_positions: Mapping[Hashable, np.ndarray],
    simplices: Sequence[Sequence[Hashable]]
    | Mapping[Hashable, Sequence[Hashable]],
    *,
    ...
    spectrum_cond_caps: Sequence[float] | None = None,
    config: DualFlowConfig | None = None,
) -> SpectrumSafePolicyMassPatienceCapProbe | None:
    """Sweep patience × spectrum_cond_cap under spectrum×policy×mass (A5-T84).

    When ``enable_spectrum_safe_policy_mass_patience_cap_probe`` is off,
    returns ``None``. When on, for each patience in ``patience_grid``
    (default ``(1, 2, 4)``) and each cap in ``spectrum_cond_caps``
    (default ``(1e-12, 1.0, 1e6)``):
    ...
    cfg = config or DualFlowConfig()
    if not cfg.enable_spectrum_safe_policy_mass_patience_cap_probe:
        return None
    if not samples:
        raise ValueError("samples must be non-empty")
    if not simplex_positions:
    ...
    grid = tuple(
        int(p)
        for p in (
            patience_grid if patience_grid is not None else (1, 2, 4)
        )
    )
    if not grid:
        raise ValueError("patience_grid must be non-empty")
    ...
    if not mass_map:
        raise ValueError(
            "masses must be non-empty for spectrum×policy×mass patience×cap"
        )
    ...
    max_iters = max(int(cfg.bp_max_iters), max(grid) + 2, 4)
    cases: list[SpectrumSafePolicyMassPatienceCapCase] = []
    for patience in grid:
        for cap in caps:
            pin_cfg = DualFlowConfig(
                enable_loopy_bp_schedule=True,
            ...
            if pin is None:
                raise RuntimeError(
                    "loopy BP unexpectedly None under spectrum×policy×mass "
                    "patience×cap pin cfg"
                )
            rd = float(pin.r_data)
            rc = float(pin.r_cons)
            reason = pin.residual_stop_reason
            ...
            if compose is None:
                raise RuntimeError(
                    "loopy compose unexpectedly None under "
                    "spectrum×policy×mass patience×cap cfg"
                )

            cases.append(
                SpectrumSafePolicyMassPatienceCapCase(
                    patience=int(patience),
                    spectrum_cond_cap=float(cap),
                    residual_stop_reason=reason,
                    iters_executed=int(pin.iters),
    ...
    return SpectrumSafePolicyMassPatienceCapProbe(
        probe_flag_default_off=not DualFlowConfig().enable_spectrum_safe_policy_mass_patience_cap_probe,
        patience_grid=grid,
        caps=caps,
        cases=tuple(cases),
    )
...
@dataclass(frozen=True)
class ResidualMassPolicyPatienceCapCase:
    """One (patience, cap) cell of residual×mass×policy grid (A5-T85)."""
...
@dataclass(frozen=True)
class ResidualMassPolicyPatienceCapProbe:
    """Residual-stop × mass_loopy × policy patience×cap grid (SI S6.2; A5-T85).

    Mass-normalizes online winners once, then for each
    ``(patience, spectrum_cond_cap)`` pair runs online→offline loopy
    compose under residual-stop early-exit with
    ``enable_bp_policy_in_loopy``. Proposal-path only — **not** a
    production certificate. Do **not** flip mass/density ``@awaiting``.
    """
    ...
    patience_grid: tuple[int, ...]
    caps: tuple[float, ...]
    cases: tuple[ResidualMassPolicyPatienceCapCase, ...]
    note: str = (
        "sketch only: mass-norm × policy-in-loopy × loopy compose "
        "residual-stop patience×cap grid; not a production "
        "certificate; do not flip mass/density @awaiting"
    )


def probe_residual_mass_policy_patience_cap(
    samples: Sequence[np.ndarray],
    simplex_positions: Mapping[Hashable, np.ndarray],
    simplices: Sequence[Sequence[Hashable]]
    | Mapping[Hashable, Sequence[Hashable]],
    *,
    ...
    spectrum_cond_caps: Sequence[float] | None = None,
    config: DualFlowConfig | None = None,
) -> ResidualMassPolicyPatienceCapProbe | None:
    """Sweep patience × spectrum_cond_cap under mass×policy×loopy (A5-T85).

    When ``enable_residual_mass_policy_patience_cap_probe`` is off,
    returns ``None``. When on:

    1. **Mass** — mass-normalize online winners.
    2. **Patience × cap grid** — for each patience in ``patience_grid``
       (default ``(1, 2, 4)``) and each cap in ``spectrum_cond_caps``
       (default ``(1e-12, 1.0, 1e6)``), run online→offline loopy
       compose with residual-stop early-exit **and**
       ``enable_bp_policy_in_loopy``.
    ...
    cfg = config or DualFlowConfig()
    if not cfg.enable_residual_mass_policy_patience_cap_probe:
        return None
    if not samples:
        raise ValueError("samples must be non-empty")
    if not simplex_positions:
    ...
    if not mass_map:
        raise ValueError(
            "masses must be non-empty for residual×mass×policy patience×cap"
        )
    ...
    compose_max = max(int(cfg.bp_max_iters), max(grid) + 2, 4)
    cases: list[ResidualMassPolicyPatienceCapCase] = []
    for patience in grid:
        for cap in caps:
            compose_cfg = DualFlowConfig(
                enable_online_offline_loopy_compose=True,
            ...
            if compose is None:
                raise RuntimeError(
                    "loopy compose unexpectedly None under "
                    "residual×mass×policy patience×cap cfg"
                )
            cases.append(
                ResidualMassPolicyPatienceCapCase(
                    patience=int(patience),
                    spectrum_cond_cap=float(cap),
                    epsilon_mass=float(mass_out.epsilon_mass),
                    mass_total_before=float(mass_out.total_before),
    ...
    return ResidualMassPolicyPatienceCapProbe(
        probe_flag_default_off=not DualFlowConfig().enable_residual_mass_policy_patience_cap_probe,
        patience_grid=grid,
        caps=caps,
        cases=tuple(cases),
    )
...
@dataclass(frozen=True)
class SpectrumSafePolicyMassPatienceCapFailClosedBridgeProbe:
    """Spectrum×policy×mass patience×cap × fail_closed reconnect (A5-T86).

    Harness only — packages
    :func:`probe_spectrum_safe_policy_mass_patience_cap` with
    :func:`proteus.evidence.gate.probe_fail_closed_dry_run_reconnect_bridge`.
    Does **not** flip mass/density ``@awaiting`` or GateConfig defaults.
    """
...
def probe_spectrum_safe_policy_mass_patience_cap_fail_closed_bridge(
    samples: Sequence[np.ndarray],
    simplex_positions: Mapping[Hashable, np.ndarray],
    simplices: Sequence[Sequence[Hashable]]
    | Mapping[Hashable, Sequence[Hashable]],
    ...
    *,
    ...
    complex_path: object | None = None,
    config: DualFlowConfig | None = None,
) -> SpectrumSafePolicyMassPatienceCapFailClosedBridgeProbe | None:
    """Bridge spectrum×policy×mass patience×cap with fail_closed (A5-T86).

    When ``enable_spectrum_safe_policy_mass_patience_cap_fail_closed_bridge_probe``
    ...
    cfg = config or DualFlowConfig()
    if not cfg.enable_spectrum_safe_policy_mass_patience_cap_fail_closed_bridge_probe:
        return None
    if not samples:
        raise ValueError("samples must be non-empty")
    ...
    if spectrum_mass_patience_cap is None:
        raise RuntimeError(
            "spectrum×policy×mass patience×cap unexpectedly None under bridge cfg"
        )
    ...
    gate_defaults = GateConfig()
    dual_defaults = DualFlowConfig()
    return SpectrumSafePolicyMassPatienceCapFailClosedBridgeProbe(
        probe_flag_default_off=not DualFlowConfig().enable_spectrum_safe_policy_mass_patience_cap_fail_closed_bridge_probe,
...
@dataclass(frozen=True)
class ResidualMassPolicyPatienceCapTrajCase:
    """One (patience, cap) cell of residual×mass×policy traj (A5-T87)."""
...
@dataclass(frozen=True)
class ResidualMassPolicyPatienceCapTrajProbe:
    """Residual-stop × mass_loopy × policy patience×cap traj (SI S6.2; A5-T87).

    Mass-normalizes online winners once, then for each
    ``(patience, spectrum_cond_cap)`` pair exports the multi-iter residual
    trajectory under ``enable_bp_policy_in_loopy`` (full horizon) and runs
    online→offline loopy compose under residual-stop at that patience /
    cap. Proposal-path only — **not** a production certificate. Do **not**
    flip mass/density ``@awaiting``.
    """
    ...
    patience_grid: tuple[int, ...]
    caps: tuple[float, ...]
    cases: tuple[ResidualMassPolicyPatienceCapTrajCase, ...]
    note: str = (
        "sketch only: mass-norm × policy-in-loopy × loopy compose "
        "residual-stop patience×cap traj export; not a production "
        "certificate; do not flip mass/density @awaiting"
    )


def probe_residual_mass_policy_patience_cap_traj(
    samples: Sequence[np.ndarray],
    simplex_positions: Mapping[Hashable, np.ndarray],
    simplices: Sequence[Sequence[Hashable]]
    | Mapping[Hashable, Sequence[Hashable]],
    *,
    ...
    max_traj_iters: int | None = None,
    config: DualFlowConfig | None = None,
) -> ResidualMassPolicyPatienceCapTrajProbe | None:
    """Sweep patience × cap with residual traj under mass×policy (A5-T87).

    When ``enable_residual_mass_policy_patience_cap_traj_probe`` is off,
    returns ``None``. When on, for each patience in ``patience_grid``
    (default ``(1, 2, 4)``) and each cap in ``spectrum_cond_caps``
    (default ``(1e-12, 1.0, 1e6)``):
    ...
    1. **Trajectory** — re-run :func:`solve_loopy_bp_schedule` at
       ``bp_max_iters = 1..max_traj_iters`` with policy-in-loopy on and
       residual-stop off (full horizon) at that cap.
    2. **Mass × compose** — mass-normalize online winners (once) and run
       online→offline loopy compose with policy + residual-stop at
       that patience / cap.
    ...
    cfg = config or DualFlowConfig()
    if not cfg.enable_residual_mass_policy_patience_cap_traj_probe:
        return None
    if not samples:
        raise ValueError("samples must be non-empty")
    if not simplex_positions:
    ...
    n_max = int(
        max_traj_iters
        if max_traj_iters is not None
        else max(int(cfg.bp_max_iters), max(grid) + 2, 4)
    )
    if n_max < 1:
        raise ValueError("max_traj_iters must be >= 1")
    ...
    if not mass_map:
        raise ValueError(
            "masses must be non-empty for residual×mass×policy patience×cap traj"
        )
    ...
    compose_max = max(int(cfg.bp_max_iters), max(grid) + 2, n_max, 4)
    cases: list[ResidualMassPolicyPatienceCapTrajCase] = []
    for patience in grid:
        for cap in caps:
            iters: list[int] = []
            r_data_t: list[float] = []
            ...
            for k in range(1, n_max + 1):
                ...
                if out is None:
                    raise RuntimeError(
                        "loopy BP unexpectedly None under "
                        "residual×mass×policy patience×cap traj cfg"
                    )
                iters.append(k)
                r_data_t.append(float(out.r_data))
                r_cons_t.append(float(out.r_cons))
            ...
            cases.append(
                ResidualMassPolicyPatienceCapTrajCase(
                    patience=int(patience),
                    spectrum_cond_cap=float(cap),
                    iters=tuple(iters),
                    r_data_traj=tuple(r_data_t),
    ...
    return ResidualMassPolicyPatienceCapTrajProbe(
        probe_flag_default_off=not DualFlowConfig().enable_residual_mass_policy_patience_cap_traj_probe,
        patience_grid=grid,
        caps=caps,
        cases=tuple(cases),
    )
...
@dataclass(frozen=True)
class SpectrumSafePolicyMassPatienceCapTrajCase:
    """One (patience, cap) cell of spectrum×policy×mass traj (A5-T91)."""
...
@dataclass(frozen=True)
class SpectrumSafePolicyMassPatienceCapTrajProbe:
    """Spectrum-safe × policy × mass_loopy patience×cap traj (SI S6.2; A5-T91).

    Mass-normalizes online winners once, then for each
    ``(patience, spectrum_cond_cap)`` pair exports the multi-iter residual
    trajectory under ``enable_bp_policy_in_loopy`` (full horizon), reports
    the T70 harness ``spectrum_safe_sketch_ok`` claim on a residual-stop
    final run, and runs online→offline loopy compose under policy +
    residual-stop at that patience / cap. Harness only — **not** a
    production certificate. Do **not** flip mass/density ``@awaiting``.
    """
    ...
    patience_grid: tuple[int, ...]
    caps: tuple[float, ...]
    cases: tuple[SpectrumSafePolicyMassPatienceCapTrajCase, ...]
    note: str = (
        "harness only: spectrum-safe × policy-in-loopy × mass_loopy "
        "patience×cap traj export; NOT a certified production "
        "convergence proof; do not flip mass/density @awaiting"
    )


def probe_spectrum_safe_policy_mass_patience_cap_traj(
    samples: Sequence[np.ndarray],
    simplex_positions: Mapping[Hashable, np.ndarray],
    simplices: Sequence[Sequence[Hashable]]
    | Mapping[Hashable, Sequence[Hashable]],
    *,
    ...
    max_traj_iters: int | None = None,
    config: DualFlowConfig | None = None,
) -> SpectrumSafePolicyMassPatienceCapTrajProbe | None:
    """Sweep patience × cap with spectrum×policy×mass traj (A5-T91).

    When ``enable_spectrum_safe_policy_mass_patience_cap_traj_probe`` is off,
    returns ``None``. When on, for each patience in ``patience_grid``
    (default ``(1, 2, 4)``) and each cap in ``spectrum_cond_caps``
    (default ``(1e-12, 1.0, 1e6)``):
    ...
    1. **Trajectory** — re-run :func:`solve_loopy_bp_schedule` at
       ...
       residual-stop off (full horizon) at that cap.
    2. **Final sketch** — one residual-stop early-exit run at
       ``max_traj_iters`` with that patience reporting T70's
       ``spectrum_safe_sketch_ok``.
    3. **Mass × compose** — mass-normalize online winners (once) and run
       online→offline loopy compose with policy + residual-stop at
       that patience / cap.
    ...
    cfg = config or DualFlowConfig()
    if not cfg.enable_spectrum_safe_policy_mass_patience_cap_traj_probe:
        return None
    if not samples:
        raise ValueError("samples must be non-empty")
    if not simplex_positions:
    ...
    if not mass_map:
        raise ValueError(
            "masses must be non-empty for spectrum×policy×mass patience×cap traj"
        )
    ...
    compose_max = max(int(cfg.bp_max_iters), max(grid) + 2, n_max, 4)
    cases: list[SpectrumSafePolicyMassPatienceCapTrajCase] = []
    for patience in grid:
        for cap in caps:
            iters: list[int] = []
            r_data_t: list[float] = []
            ...
            for k in range(1, n_max + 1):
                ...
                if out is None:
                    raise RuntimeError(
                        "loopy BP unexpectedly None under "
                        "spectrum×policy×mass patience×cap traj cfg"
                    )
                iters.append(k)
                r_data_t.append(float(out.r_data))
                r_cons_t.append(float(out.r_cons))
            ...
            if final is None:
                raise RuntimeError(
                    "loopy BP unexpectedly None under "
                    "spectrum×policy×mass patience×cap traj final cfg"
                )
            rd = float(final.r_data)
            rc = float(final.r_cons)
            reason = final.residual_stop_reason
            ...
            cases.append(
                SpectrumSafePolicyMassPatienceCapTrajCase(
                    patience=int(patience),
                    spectrum_cond_cap=float(cap),
                    iters=tuple(iters),
                    r_data_traj=tuple(r_data_t),
    ...
    return SpectrumSafePolicyMassPatienceCapTrajProbe(
        probe_flag_default_off=not DualFlowConfig().enable_spectrum_safe_policy_mass_patience_cap_traj_probe,
        patience_grid=grid,
        caps=caps,
        cases=tuple(cases),
    )
...
@dataclass(frozen=True)
class ResidualMassPolicyPatienceCapFailClosedMatrixProbe:
    """Residual patience×cap × fail_closed EG matrix (A5-T92).

    Harness only — packages :func:`probe_residual_mass_policy_patience_cap`
    with :func:`proteus.evidence.gate.probe_fail_closed_evidence_gate_matrix`.
    Does **not** flip mass/density ``@awaiting`` or GateConfig defaults.
    """

    probe_flag_default_off: bool
    residual_patience_cap: ResidualMassPolicyPatienceCapProbe
    fail_closed_matrix_all_matched: bool
    fail_closed_n_cases: int
    fail_closed_n_matched: int
    gate_apply_dual_default: bool
    ...
    dual_adjacency_default: bool
    note: str = (
        "harness only: residual×mass×policy patience×cap bridged to "
        "fail_closed EvidenceGate.evaluate matrix; defaults unchanged; "
        "do not flip @awaiting / apply_dual / fail_closed / "
        "enable_dual_adjacency"
    )
...
def probe_residual_mass_policy_patience_cap_fail_closed_matrix(
    samples: Sequence[np.ndarray],
    simplex_positions: Mapping[Hashable, np.ndarray],
    simplices: Sequence[Sequence[Hashable]]
    | Mapping[Hashable, Sequence[Hashable]],
    ...
    *,
    ...
    affected_simplices: Sequence[Hashable] | None = None,
    config: DualFlowConfig | None = None,
) -> ResidualMassPolicyPatienceCapFailClosedMatrixProbe | None:
    """Bridge residual patience×cap with EG matrix (A5-T92).

    When ``enable_residual_mass_policy_patience_cap_fail_closed_matrix_probe``
    is off, returns ``None``. When on:

    1. Run :func:`probe_residual_mass_policy_patience_cap` (flag
       forced on for the nested call).
    2. Run gate :func:`probe_fail_closed_evidence_gate_matrix` on the
       provided edit regions / proposal.
    ...
    cfg = config or DualFlowConfig()
    if not cfg.enable_residual_mass_policy_patience_cap_fail_closed_matrix_probe:
        return None
    if not samples:
        raise ValueError("samples must be non-empty")
    ...
    nested = DualFlowConfig(
        enable_residual_mass_policy_patience_cap_probe=True,
        bp_damping=float(cfg.bp_damping),
        bp_max_iters=max(int(cfg.bp_max_iters), 4),
        bp_residual_stop_tol=float(cfg.bp_residual_stop_tol),
        bp_residual_stop_patience=int(cfg.bp_residual_stop_patience),
    ...
    residual_patience_cap = probe_residual_mass_policy_patience_cap(
        samples,
        simplex_positions,
        simplices,
        masses=masses,
    ...
    if residual_patience_cap is None:
        raise RuntimeError(
            "residual×mass×policy patience×cap unexpectedly None under matrix cfg"
        )
    ...
    gate_defaults = GateConfig()
    dual_defaults = DualFlowConfig()
    return ResidualMassPolicyPatienceCapFailClosedMatrixProbe(
        probe_flag_default_off=not DualFlowConfig().enable_residual_mass_policy_patience_cap_fail_closed_matrix_probe,
        residual_patience_cap=residual_patience_cap,
        fail_closed_matrix_all_matched=bool(fc.all_matched),
        fail_closed_n_cases=int(fc.n_cases),
        fail_closed_n_matched=int(fc.n_matched),
        gate_apply_dual_default=bool(gate_defaults.apply_dual_adjacency),
...
@dataclass(frozen=True)
class SpectrumSafePolicyMassPatienceCapTrajFailClosedBridgeProbe:
    """Spectrum patience×cap traj × fail_closed reconnect bridge (A5-T93).

    Harness only — packages
    :func:`probe_spectrum_safe_policy_mass_patience_cap_traj` with
    :func:`proteus.evidence.gate.probe_fail_closed_dry_run_reconnect_bridge`.
    Does **not** flip mass/density ``@awaiting`` or GateConfig defaults.
    """

    probe_flag_default_off: bool
    spectrum_mass_patience_cap_traj: SpectrumSafePolicyMassPatienceCapTrajProbe
    fail_closed_reconnect_all_matched: bool
    fail_closed_n_cases: int
    fail_closed_n_matched: int
    fail_closed_disconnect_connected: bool
    ...
    dual_adjacency_default: bool
    note: str = (
        "harness only: spectrum×policy×mass patience×cap traj bridged to "
        "fail_closed dry_run disconnect→reconnect × EvidenceGate; "
        "defaults unchanged; do not flip @awaiting / apply_dual / "
        "fail_closed / enable_dual_adjacency"
    )
...
def probe_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_bridge(
    samples: Sequence[np.ndarray],
    simplex_positions: Mapping[Hashable, np.ndarray],
    simplices: Sequence[Sequence[Hashable]]
    | Mapping[Hashable, Sequence[Hashable]],
    ...
    *,
    ...
    complex_path: object | None = None,
    config: DualFlowConfig | None = None,
) -> SpectrumSafePolicyMassPatienceCapTrajFailClosedBridgeProbe | None:
    """Bridge spectrum patience×cap traj with fail_closed reconnect (A5-T93).

    When ``enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_bridge_probe``
    is off, returns ``None``. When on:

    1. Run :func:`probe_spectrum_safe_policy_mass_patience_cap_traj` (flag
       forced on for the nested call).
    2. Run gate :func:`probe_fail_closed_dry_run_reconnect_bridge` on the
       provided edit regions / proposal.
    ...
    cfg = config or DualFlowConfig()
    if not cfg.enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_bridge_probe:
        return None
    if not samples:
        raise ValueError("samples must be non-empty")
    ...
    nested = DualFlowConfig(
        enable_spectrum_safe_policy_mass_patience_cap_traj_probe=True,
        bp_damping=float(cfg.bp_damping),
        bp_max_iters=max(int(cfg.bp_max_iters), 4),
        bp_residual_stop_tol=float(cfg.bp_residual_stop_tol),
        bp_residual_stop_patience=int(cfg.bp_residual_stop_patience),
    ...
    spectrum_mass_patience_cap_traj = probe_spectrum_safe_policy_mass_patience_cap_traj(
        samples,
        simplex_positions,
        simplices,
        masses=masses,
    ...
    if spectrum_mass_patience_cap_traj is None:
        raise RuntimeError(
            "spectrum×policy×mass patience×cap traj unexpectedly None under bridge cfg"
        )
    ...
    gate_defaults = GateConfig()
    dual_defaults = DualFlowConfig()
    return SpectrumSafePolicyMassPatienceCapTrajFailClosedBridgeProbe(
        probe_flag_default_off=not DualFlowConfig().enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_bridge_probe,
        spectrum_mass_patience_cap_traj=spectrum_mass_patience_cap_traj,
        fail_closed_reconnect_all_matched=bool(fc.all_matched),
        fail_closed_n_cases=int(fc.n_cases),
        fail_closed_n_matched=int(fc.n_matched),
        fail_closed_disconnect_connected=bool(fc.disconnect_connected),
...
@dataclass(frozen=True)
class ResidualMassPolicyPatienceCapTrajFailClosedDryRunProbe:
    """Residual patience×cap traj × fail_closed dry_run EG (A5-T94).

    Harness only — packages :func:`probe_residual_mass_policy_patience_cap_traj`
    with :func:`proteus.evidence.gate.probe_fail_closed_dry_run_evidence_gate`.
    Complements A5-T89 (reconnect bridge) with the dry_run×EvidenceGate path.
    Does **not** flip mass/density ``@awaiting`` or GateConfig defaults.
    """
...
def probe_residual_mass_policy_patience_cap_traj_fail_closed_dry_run(
    samples: Sequence[np.ndarray],
    simplex_positions: Mapping[Hashable, np.ndarray],
    simplices: Sequence[Sequence[Hashable]]
    | Mapping[Hashable, Sequence[Hashable]],
    ...
    *,
    ...
    complex_path: object | None = None,
    config: DualFlowConfig | None = None,
) -> ResidualMassPolicyPatienceCapTrajFailClosedDryRunProbe | None:
    """Bridge residual patience×cap traj with dry_run EG (A5-T94).

    When ``enable_residual_mass_policy_patience_cap_traj_fail_closed_dry_run_probe``
    is off, returns ``None``. When on:
    ...
    cfg = config or DualFlowConfig()
    if not cfg.enable_residual_mass_policy_patience_cap_traj_fail_closed_dry_run_probe:
        return None
    if not samples:
        raise ValueError("samples must be non-empty")
    ...
    if residual_patience_cap_traj is None:
        raise RuntimeError(
            "residual×mass×policy patience×cap traj unexpectedly None under dry_run cfg"
        )
    ...
    gate_defaults = GateConfig()
    dual_defaults = DualFlowConfig()
    return ResidualMassPolicyPatienceCapTrajFailClosedDryRunProbe(
        probe_flag_default_off=not DualFlowConfig().enable_residual_mass_policy_patience_cap_traj_fail_closed_dry_run_probe,
        residual_patience_cap_traj=residual_patience_cap_traj,
        fail_closed_dry_run_all_matched=bool(fc.all_matched),
        fail_closed_n_cases=int(fc.n_cases),
        fail_closed_n_matched=int(fc.n_matched),
...
@dataclass(frozen=True)
class SpectrumSafePolicyMassPatienceCapTrajFailClosedMatrixProbe:
    """Spectrum patience×cap traj × fail_closed EG matrix (A5-T95).

    Harness only — packages
    :func:`probe_spectrum_safe_policy_mass_patience_cap_traj` with
    :func:`proteus.evidence.gate.probe_fail_closed_evidence_gate_matrix`.
    Does **not** flip mass/density ``@awaiting`` or GateConfig defaults.
    """
...
def probe_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_matrix(
    samples: Sequence[np.ndarray],
    simplex_positions: Mapping[Hashable, np.ndarray],
    simplices: Sequence[Sequence[Hashable]]
    | Mapping[Hashable, Sequence[Hashable]],
    ...
    *,
    ...
    affected_simplices: Sequence[Hashable] | None = None,
    config: DualFlowConfig | None = None,
) -> SpectrumSafePolicyMassPatienceCapTrajFailClosedMatrixProbe | None:
    """Bridge spectrum patience×cap traj with EG matrix (A5-T95).

    When ``enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_matrix_probe``
    is off, returns ``None``. When on:
    ...
    cfg = config or DualFlowConfig()
    if not cfg.enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_matrix_probe:
        return None
    if not samples:
        raise ValueError("samples must be non-empty")
    ...
    if spectrum_mass_patience_cap_traj is None:
        raise RuntimeError(
            "spectrum×policy×mass patience×cap traj unexpectedly None under matrix cfg"
        )
    ...
    gate_defaults = GateConfig()
    dual_defaults = DualFlowConfig()
    return SpectrumSafePolicyMassPatienceCapTrajFailClosedMatrixProbe(
        probe_flag_default_off=not DualFlowConfig().enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_matrix_probe,
        spectrum_mass_patience_cap_traj=spectrum_mass_patience_cap_traj,
        fail_closed_matrix_all_matched=bool(fc.all_matched),
        fail_closed_n_cases=int(fc.n_cases),
        fail_closed_n_matched=int(fc.n_matched),
...
@dataclass(frozen=True)
class ResidualMassPolicyPatienceCapFailClosedDryRunProbe:
    """Residual patience×cap × fail_closed dry_run EG (A5-T96).

    Harness only — packages :func:`probe_residual_mass_policy_patience_cap`
    with :func:`proteus.evidence.gate.probe_fail_closed_dry_run_evidence_gate`.
    Complements A5-T92 (EG matrix) with the dry_run×EvidenceGate path.
    Does **not** flip mass/density ``@awaiting`` or GateConfig defaults.
    """
...
def probe_residual_mass_policy_patience_cap_fail_closed_dry_run(
    samples: Sequence[np.ndarray],
    simplex_positions: Mapping[Hashable, np.ndarray],
    simplices: Sequence[Sequence[Hashable]]
    | Mapping[Hashable, Sequence[Hashable]],
    ...
    *,
    ...
    complex_path: object | None = None,
    config: DualFlowConfig | None = None,
) -> ResidualMassPolicyPatienceCapFailClosedDryRunProbe | None:
    """Bridge residual patience×cap with dry_run EG (A5-T96).

    When ``enable_residual_mass_policy_patience_cap_fail_closed_dry_run_probe``
    is off, returns ``None``. When on:
    ...
    cfg = config or DualFlowConfig()
    if not cfg.enable_residual_mass_policy_patience_cap_fail_closed_dry_run_probe:
        return None
    if not samples:
        raise ValueError("samples must be non-empty")
    ...
    if residual_patience_cap is None:
        raise RuntimeError(
            "residual×mass×policy patience×cap unexpectedly None under dry_run cfg"
        )
    ...
    gate_defaults = GateConfig()
    dual_defaults = DualFlowConfig()
    return ResidualMassPolicyPatienceCapFailClosedDryRunProbe(
        probe_flag_default_off=not DualFlowConfig().enable_residual_mass_policy_patience_cap_fail_closed_dry_run_probe,
        residual_patience_cap=residual_patience_cap,
        fail_closed_dry_run_all_matched=bool(fc.all_matched),
        fail_closed_n_cases=int(fc.n_cases),
        fail_closed_n_matched=int(fc.n_matched),
...
@dataclass(frozen=True)
class SpectrumSafePolicyMassPatienceCapTrajFailClosedDryRunProbe:
    """Spectrum patience×cap traj × fail_closed dry_run EG (A5-T97).

    Harness only — packages
    :func:`probe_spectrum_safe_policy_mass_patience_cap_traj` with
    :func:`proteus.evidence.gate.probe_fail_closed_dry_run_evidence_gate`.
    Complements A5-T95 (EG matrix) and A5-T93 (reconnect) with the
    dry_run×EvidenceGate path. Does **not** flip mass/density ``@awaiting``
    or GateConfig defaults.
    """
...
def probe_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_dry_run(
    samples: Sequence[np.ndarray],
    simplex_positions: Mapping[Hashable, np.ndarray],
    simplices: Sequence[Sequence[Hashable]]
    | Mapping[Hashable, Sequence[Hashable]],
    ...
    *,
    ...
    complex_path: object | None = None,
    config: DualFlowConfig | None = None,
) -> SpectrumSafePolicyMassPatienceCapTrajFailClosedDryRunProbe | None:
    """Bridge spectrum patience×cap traj with dry_run EG (A5-T97).

    When ``enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_dry_run_probe``
    is off, returns ``None``. When on:
    ...
    cfg = config or DualFlowConfig()
    if not cfg.enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_dry_run_probe:
        return None
    if not samples:
        raise ValueError("samples must be non-empty")
    ...
    if spectrum_mass_patience_cap_traj is None:
        raise RuntimeError(
            "spectrum×policy×mass patience×cap traj unexpectedly None under dry_run cfg"
        )
    ...
    gate_defaults = GateConfig()
    dual_defaults = DualFlowConfig()
    return SpectrumSafePolicyMassPatienceCapTrajFailClosedDryRunProbe(
        probe_flag_default_off=not DualFlowConfig().enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_dry_run_probe,
        spectrum_mass_patience_cap_traj=spectrum_mass_patience_cap_traj,
        fail_closed_dry_run_all_matched=bool(fc.all_matched),
        fail_closed_n_cases=int(fc.n_cases),
        fail_closed_n_matched=int(fc.n_matched),
...
@dataclass(frozen=True)
class ResidualMassPolicyPatienceCapTrajFailClosedMatrixProbe:
    """Residual patience×cap traj × fail_closed EG matrix (A5-T98).

    Harness only — packages :func:`probe_residual_mass_policy_patience_cap_traj`
    with :func:`proteus.evidence.gate.probe_fail_closed_evidence_gate_matrix`.
    Complements A5-T94 (dry_run EG) and A5-T89 (reconnect) with the EG
    matrix path. Does **not** flip mass/density ``@awaiting`` or GateConfig
    defaults.
    """
...
def probe_residual_mass_policy_patience_cap_traj_fail_closed_matrix(
    samples: Sequence[np.ndarray],
    simplex_positions: Mapping[Hashable, np.ndarray],
    simplices: Sequence[Sequence[Hashable]]
    | Mapping[Hashable, Sequence[Hashable]],
    ...
    *,
    ...
    affected_simplices: Sequence[Hashable] | None = None,
    config: DualFlowConfig | None = None,
) -> ResidualMassPolicyPatienceCapTrajFailClosedMatrixProbe | None:
    """Bridge residual patience×cap traj with EG matrix (A5-T98).

    When ``enable_residual_mass_policy_patience_cap_traj_fail_closed_matrix_probe``
    is off, returns ``None``. When on:
    ...
    cfg = config or DualFlowConfig()
    if not cfg.enable_residual_mass_policy_patience_cap_traj_fail_closed_matrix_probe:
        return None
    if not samples:
        raise ValueError("samples must be non-empty")
    ...
    if residual_patience_cap_traj is None:
        raise RuntimeError(
            "residual×mass×policy patience×cap traj unexpectedly None under matrix cfg"
        )
    ...
    gate_defaults = GateConfig()
    dual_defaults = DualFlowConfig()
    return ResidualMassPolicyPatienceCapTrajFailClosedMatrixProbe(
        probe_flag_default_off=not DualFlowConfig().enable_residual_mass_policy_patience_cap_traj_fail_closed_matrix_probe,
        residual_patience_cap_traj=residual_patience_cap_traj,
        fail_closed_matrix_all_matched=bool(fc.all_matched),
        fail_closed_n_cases=int(fc.n_cases),
        fail_closed_n_matched=int(fc.n_matched),
...
@dataclass(frozen=True)
class ResidualMassPolicyPatienceCapFailClosedBridgeProbe:
    """Residual patience×cap × fail_closed reconnect bridge (A5-T99).

    Harness only — packages :func:`probe_residual_mass_policy_patience_cap`
    with :func:`proteus.evidence.gate.probe_fail_closed_dry_run_reconnect_bridge`.
    Complements A5-T92 (EG matrix) and A5-T96 (dry_run EG) with the
    reconnect path. Does **not** flip mass/density ``@awaiting`` or
    GateConfig defaults.
    """
...
def probe_residual_mass_policy_patience_cap_fail_closed_bridge(
    samples: Sequence[np.ndarray],
    simplex_positions: Mapping[Hashable, np.ndarray],
    simplices: Sequence[Sequence[Hashable]]
    | Mapping[Hashable, Sequence[Hashable]],
    ...
    *,
    ...
    complex_path: object | None = None,
    config: DualFlowConfig | None = None,
) -> ResidualMassPolicyPatienceCapFailClosedBridgeProbe | None:
    """Bridge residual patience×cap with fail_closed reconnect (A5-T99).

    When ``enable_residual_mass_policy_patience_cap_fail_closed_bridge_probe``
    is off, returns ``None``. When on:
    ...
    cfg = config or DualFlowConfig()
    if not cfg.enable_residual_mass_policy_patience_cap_fail_closed_bridge_probe:
        return None
    if not samples:
        raise ValueError("samples must be non-empty")
    ...
    if residual_patience_cap is None:
        raise RuntimeError(
            "residual×mass×policy patience×cap unexpectedly None under bridge cfg"
        )
    ...
    gate_defaults = GateConfig()
    dual_defaults = DualFlowConfig()
    return ResidualMassPolicyPatienceCapFailClosedBridgeProbe(
        probe_flag_default_off=not DualFlowConfig().enable_residual_mass_policy_patience_cap_fail_closed_bridge_probe,
        residual_patience_cap=residual_patience_cap,
        fail_closed_reconnect_all_matched=bool(fc.all_matched),
        fail_closed_n_cases=int(fc.n_cases),
        fail_closed_n_matched=int(fc.n_matched),
...
@dataclass(frozen=True)
class ResidualSpectrumDualPathPatienceCapFailClosedComposeProbe:
    """Residual×spectrum dual-path patience×cap × fail_closed compose (A5-T100).

    Harness only — packages :func:`probe_residual_mass_policy_patience_cap`
    and :func:`probe_spectrum_safe_policy_mass_patience_cap` under a shared
    :func:`proteus.evidence.gate.probe_fail_closed_dry_run_reconnect_bridge`.
    Does **not** flip mass/density ``@awaiting`` or GateConfig defaults.
    """
    ...
    dual_adjacency_default: bool
    note: str = (
        "harness only: residual×spectrum dual-path patience×cap compose "
        "bridged to fail_closed dry_run disconnect→reconnect × EvidenceGate; "
        "defaults unchanged; do not flip @awaiting / apply_dual / "
        "fail_closed / enable_dual_adjacency"
    )


def probe_residual_spectrum_dual_path_patience_cap_fail_closed_compose(
    samples: Sequence[np.ndarray],
    simplex_positions: Mapping[Hashable, np.ndarray],
    simplices: Sequence[Sequence[Hashable]]
    | Mapping[Hashable, Sequence[Hashable]],
    ...
    *,
    ...
    complex_path: object | None = None,
    config: DualFlowConfig | None = None,
) -> ResidualSpectrumDualPathPatienceCapFailClosedComposeProbe | None:
    """Compose residual + spectrum patience×cap with fail_closed reconnect (A5-T100).

    When ``enable_residual_spectrum_dual_path_patience_cap_fail_closed_compose_probe``
    is off, returns ``None``. When on:

    1. Run :func:`probe_residual_mass_policy_patience_cap` (flag forced on).
    2. Run :func:`probe_spectrum_safe_policy_mass_patience_cap` (flag forced on).
    3. Run gate :func:`probe_fail_closed_dry_run_reconnect_bridge`.

    Does **not** flip mass/density ``@awaiting`` or GateConfig defaults.
    """

    cfg = config or DualFlowConfig()
    if not cfg.enable_residual_spectrum_dual_path_patience_cap_fail_closed_compose_probe:
        return None
    if not samples:
        raise ValueError("samples must be non-empty")
    ...
    if residual_patience_cap is None:
        raise RuntimeError(
            "residual×mass×policy patience×cap unexpectedly None under dual-path compose cfg"
        )
    spectrum_mass_patience_cap = probe_spectrum_safe_policy_mass_patience_cap(
        samples,
        simplex_positions,
    ...
    if spectrum_mass_patience_cap is None:
        raise RuntimeError(
            "spectrum×policy×mass patience×cap unexpectedly None under dual-path compose cfg"
        )
    ...
    gate_defaults = GateConfig()
    dual_defaults = DualFlowConfig()
    return ResidualSpectrumDualPathPatienceCapFailClosedComposeProbe(
        probe_flag_default_off=not DualFlowConfig().enable_residual_spectrum_dual_path_patience_cap_fail_closed_compose_probe,
        residual_patience_cap=residual_patience_cap,
        spectrum_mass_patience_cap=spectrum_mass_patience_cap,
        fail_closed_reconnect_all_matched=bool(fc.all_matched),
        fail_closed_n_cases=int(fc.n_cases),
...
@dataclass(frozen=True)
class SpectrumSafePolicyMassPatienceCapTrajFailClosedDryRunReconnectProbe:
    """Spectrum patience×cap traj × fail_closed dry_run×reconnect (A5-T101).

    Harness only — packages
    :func:`probe_spectrum_safe_policy_mass_patience_cap_traj` with both
    :func:`proteus.evidence.gate.probe_fail_closed_dry_run_evidence_gate`
    and
    :func:`proteus.evidence.gate.probe_fail_closed_dry_run_reconnect_bridge`.
    Complements A5-T97 (dry_run only) and A5-T93 (reconnect only).
    Does **not** flip mass/density ``@awaiting`` or GateConfig defaults.
    """
    ...
    spectrum_mass_patience_cap_traj: SpectrumSafePolicyMassPatienceCapTrajProbe
    fail_closed_dry_run_all_matched: bool
    fail_closed_dry_run_n_cases: int
    fail_closed_dry_run_n_matched: int
    fail_closed_reconnect_all_matched: bool
    fail_closed_reconnect_n_cases: int
    fail_closed_reconnect_n_matched: int
    fail_closed_disconnect_connected: bool
    fail_closed_reconnect_connected: bool
    gate_apply_dual_default: bool
    gate_fail_closed_default: bool
    ...
    note: str = (
        "harness only: spectrum×policy×mass patience×cap traj bridged to "
        "fail_closed dry_run_dual × EvidenceGate and disconnect→reconnect; "
        "defaults unchanged; do not flip @awaiting / apply_dual / "
        "fail_closed / enable_dual_adjacency"
    )


def probe_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_dry_run_reconnect(
    samples: Sequence[np.ndarray],
    simplex_positions: Mapping[Hashable, np.ndarray],
    simplices: Sequence[Sequence[Hashable]]
    | Mapping[Hashable, Sequence[Hashable]],
    ...
    *,
    ...
    complex_path: object | None = None,
    config: DualFlowConfig | None = None,
) -> SpectrumSafePolicyMassPatienceCapTrajFailClosedDryRunReconnectProbe | None:
    """Bridge spectrum traj with dry_run EG and reconnect (A5-T101).

    When ``enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_dry_run_reconnect_probe``
    is off, returns ``None``. When on:

    1. Run :func:`probe_spectrum_safe_policy_mass_patience_cap_traj`.
    2. Run gate :func:`probe_fail_closed_dry_run_evidence_gate`.
    3. Run gate :func:`probe_fail_closed_dry_run_reconnect_bridge`.
    ...
    cfg = config or DualFlowConfig()
    if not cfg.enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_dry_run_reconnect_probe:
        return None
    if not samples:
        raise ValueError("samples must be non-empty")
    ...
    if spectrum_mass_patience_cap_traj is None:
        raise RuntimeError(
            "spectrum×policy×mass patience×cap traj unexpectedly None under dry_run×reconnect cfg"
        )

    dry = probe_fail_closed_dry_run_evidence_gate(
        keep_region,  # type: ignore[arg-type]
        edit_region,  # type: ignore[arg-type]
        proposal,  # type: ignore[arg-type]
        edit_stars=edit_stars,
    ...
    recon = probe_fail_closed_dry_run_reconnect_bridge(
        keep_region,  # type: ignore[arg-type]
        edit_region,  # type: ignore[arg-type]
        proposal,  # type: ignore[arg-type]
        edit_stars=edit_stars,
    ...
    gate_defaults = GateConfig()
    dual_defaults = DualFlowConfig()
    return SpectrumSafePolicyMassPatienceCapTrajFailClosedDryRunReconnectProbe(
        probe_flag_default_off=not DualFlowConfig().enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_dry_run_reconnect_probe,
        spectrum_mass_patience_cap_traj=spectrum_mass_patience_cap_traj,
        fail_closed_dry_run_all_matched=bool(dry.all_matched),
        fail_closed_dry_run_n_cases=int(dry.n_cases),
        fail_closed_dry_run_n_matched=int(dry.n_matched),
        fail_closed_reconnect_all_matched=bool(recon.all_matched),
        fail_closed_reconnect_n_cases=int(recon.n_cases),
        fail_closed_reconnect_n_matched=int(recon.n_matched),
        fail_closed_disconnect_connected=bool(recon.disconnect_connected),
        fail_closed_reconnect_connected=bool(recon.reconnect_connected),
        gate_apply_dual_default=bool(gate_defaults.apply_dual_adjacency),
        gate_fail_closed_default=bool(gate_defaults.fail_closed_dual_adjacency),
        dual_adjacency_default=bool(dual_defaults.enable_dual_adjacency),
    )
...
@dataclass(frozen=True)
class ResidualMassPolicyPatienceCapTrajFailClosedMatrixDryRunProbe:
    """Residual patience×cap traj × fail_closed matrix×dry_run EG (A5-T102).

    Harness only — packages :func:`probe_residual_mass_policy_patience_cap_traj`
    with both
    :func:`proteus.evidence.gate.probe_fail_closed_evidence_gate_matrix` and
    :func:`proteus.evidence.gate.probe_fail_closed_dry_run_evidence_gate`.
    Complements A5-T98 (matrix) and A5-T94 (dry_run). Does **not** flip
    mass/density ``@awaiting`` or GateConfig defaults.
    """

    probe_flag_default_off: bool
    residual_patience_cap_traj: ResidualMassPolicyPatienceCapTrajProbe
    fail_closed_matrix_all_matched: bool
    fail_closed_matrix_n_cases: int
    fail_closed_matrix_n_matched: int
    fail_closed_dry_run_all_matched: bool
    fail_closed_dry_run_n_cases: int
    fail_closed_dry_run_n_matched: int
    gate_apply_dual_default: bool
    ...
    note: str = (
        "harness only: residual×mass×policy patience×cap traj bridged to "
        "fail_closed EvidenceGate.evaluate matrix and dry_run_dual × "
        "EvidenceGate; defaults unchanged; do not flip @awaiting / "
        "apply_dual / fail_closed / enable_dual_adjacency"
    )


def probe_residual_mass_policy_patience_cap_traj_fail_closed_matrix_dry_run(
    samples: Sequence[np.ndarray],
    simplex_positions: Mapping[Hashable, np.ndarray],
    simplices: Sequence[Sequence[Hashable]]
    | Mapping[Hashable, Sequence[Hashable]],
    ...
    *,
    ...
    complex_path: object | None = None,
    config: DualFlowConfig | None = None,
) -> ResidualMassPolicyPatienceCapTrajFailClosedMatrixDryRunProbe | None:
    """Bridge residual traj with EG matrix and dry_run EG (A5-T102).

    When ``enable_residual_mass_policy_patience_cap_traj_fail_closed_matrix_dry_run_probe``
    is off, returns ``None``. When on:

    1. Run :func:`probe_residual_mass_policy_patience_cap_traj`.
    2. Run gate :func:`probe_fail_closed_evidence_gate_matrix`.
    3. Run gate :func:`probe_fail_closed_dry_run_evidence_gate`.

    Does **not** flip mass/density ``@awaiting`` or GateConfig defaults.
    """

    cfg = config or DualFlowConfig()
    if not cfg.enable_residual_mass_policy_patience_cap_traj_fail_closed_matrix_dry_run_probe:
        return None
    if not samples:
        raise ValueError("samples must be non-empty")
    ...
    if residual_patience_cap_traj is None:
        raise RuntimeError(
            "residual×mass×policy patience×cap traj unexpectedly None under matrix×dry_run cfg"
        )

    matrix = probe_fail_closed_evidence_gate_matrix(
        keep_region,  # type: ignore[arg-type]
        edit_region,  # type: ignore[arg-type]
        proposal,  # type: ignore[arg-type]
        edit_stars=edit_stars,
    ...
    gate_defaults = GateConfig()
    dual_defaults = DualFlowConfig()
    return ResidualMassPolicyPatienceCapTrajFailClosedMatrixDryRunProbe(
        probe_flag_default_off=not DualFlowConfig().enable_residual_mass_policy_patience_cap_traj_fail_closed_matrix_dry_run_probe,
        residual_patience_cap_traj=residual_patience_cap_traj,
        fail_closed_matrix_all_matched=bool(matrix.all_matched),
        fail_closed_matrix_n_cases=int(matrix.n_cases),
        fail_closed_matrix_n_matched=int(matrix.n_matched),
        fail_closed_dry_run_all_matched=bool(dry.all_matched),
        fail_closed_dry_run_n_cases=int(dry.n_cases),
        fail_closed_dry_run_n_matched=int(dry.n_matched),
        gate_apply_dual_default=bool(gate_defaults.apply_dual_adjacency),
...
@dataclass(frozen=True)
class ResidualSpectrumDualPathPatienceCapTrajComposeProbe:
    """Residual×spectrum dual-path patience×cap×traj compose (A5-T103).

    Harness only — packages :func:`probe_residual_mass_policy_patience_cap_traj`
    and :func:`probe_spectrum_safe_policy_mass_patience_cap_traj`. Does **not**
    flip mass/density ``@awaiting`` or GateConfig defaults.
    """
    ...
    dual_adjacency_default: bool
    note: str = (
        "harness only: residual×spectrum dual-path patience×cap traj compose; "
        "defaults unchanged; do not flip @awaiting / apply_dual / "
        "fail_closed / enable_dual_adjacency"
    )


def probe_residual_spectrum_dual_path_patience_cap_traj_compose(
    samples: Sequence[np.ndarray],
    simplex_positions: Mapping[Hashable, np.ndarray],
    simplices: Sequence[Sequence[Hashable]]
    | Mapping[Hashable, Sequence[Hashable]],
    *,
    ...
    max_traj_iters: int | None = None,
    config: DualFlowConfig | None = None,
) -> ResidualSpectrumDualPathPatienceCapTrajComposeProbe | None:
    """Compose residual + spectrum patience×cap traj dual-paths (A5-T103).

    When ``enable_residual_spectrum_dual_path_patience_cap_traj_compose_probe``
    is off, returns ``None``. When on:

    1. Run :func:`probe_residual_mass_policy_patience_cap_traj` (flag forced on).
    2. Run :func:`probe_spectrum_safe_policy_mass_patience_cap_traj` (flag forced on).

    Does **not** flip mass/density ``@awaiting`` or GateConfig defaults.
    """

    cfg = config or DualFlowConfig()
    if not cfg.enable_residual_spectrum_dual_path_patience_cap_traj_compose_probe:
        return None
    if not samples:
        raise ValueError("samples must be non-empty")
    ...
    if residual_patience_cap_traj is None:
        raise RuntimeError(
            "residual×mass×policy patience×cap traj unexpectedly None under dual-path traj compose cfg"
        )
    spectrum_mass_patience_cap_traj = probe_spectrum_safe_policy_mass_patience_cap_traj(
        samples,
        simplex_positions,
    ...
    if spectrum_mass_patience_cap_traj is None:
        raise RuntimeError(
            "spectrum×policy×mass patience×cap traj unexpectedly None under dual-path traj compose cfg"
        )

    dual_defaults = DualFlowConfig()
    return ResidualSpectrumDualPathPatienceCapTrajComposeProbe(
        probe_flag_default_off=not DualFlowConfig().enable_residual_spectrum_dual_path_patience_cap_traj_compose_probe,
        residual_patience_cap_traj=residual_patience_cap_traj,
        spectrum_mass_patience_cap_traj=spectrum_mass_patience_cap_traj,
        dual_adjacency_default=bool(dual_defaults.enable_dual_adjacency),
    )
...
@dataclass(frozen=True)
class SpectrumSafePolicyMassPatienceCapTrajFailClosedDryRunMatrixReconnectProbe:
    """Spectrum traj × fail_closed dry_run×matrix×reconnect (A5-T104).
    ...
    :func:`proteus.evidence.gate.probe_fail_closed_evidence_gate_matrix`, and
    :func:`proteus.evidence.gate.probe_fail_closed_dry_run_reconnect_bridge`.
    Complements A5-T101 (dry_run×reconnect) by adding matrix. Does **not**
    flip mass/density ``@awaiting`` or GateConfig defaults.
    """
    ...
    note: str = (
        "harness only: spectrum×policy×mass patience×cap traj bridged to "
        "fail_closed dry_run × matrix × reconnect EvidenceGate paths; "
        "defaults unchanged; do not flip @awaiting / apply_dual / "
        "fail_closed / enable_dual_adjacency"
    )


def probe_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_dry_run_matrix_reconnect(
    samples: Sequence[np.ndarray],
    simplex_positions: Mapping[Hashable, np.ndarray],
    simplices: Sequence[Sequence[Hashable]]
    | Mapping[Hashable, Sequence[Hashable]],
    ...
    *,
    ...
    complex_path: object | None = None,
    config: DualFlowConfig | None = None,
) -> SpectrumSafePolicyMassPatienceCapTrajFailClosedDryRunMatrixReconnectProbe | None:
    """Bridge spectrum traj with dry_run, matrix, and reconnect (A5-T104).

    When ``enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_dry_run_matrix_reconnect_probe``
    is off, returns ``None``. When on:

    1. Run :func:`probe_spectrum_safe_policy_mass_patience_cap_traj`.
    2. Run gate :func:`probe_fail_closed_dry_run_evidence_gate`.
    3. Run gate :func:`probe_fail_closed_evidence_gate_matrix`.
    4. Run gate :func:`probe_fail_closed_dry_run_reconnect_bridge`.

    Does **not** flip mass/density ``@awaiting`` or GateConfig defaults.
    """

    cfg = config or DualFlowConfig()
    if not cfg.enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_dry_run_matrix_reconnect_probe:
        return None
    if not samples:
        raise ValueError("samples must be non-empty")
    ...
    if spectrum_mass_patience_cap_traj is None:
        raise RuntimeError(
            "spectrum×policy×mass patience×cap traj unexpectedly None under dry_run×matrix×reconnect cfg"
        )
    ...
    gate_defaults = GateConfig()
    dual_defaults = DualFlowConfig()
    return SpectrumSafePolicyMassPatienceCapTrajFailClosedDryRunMatrixReconnectProbe(
        probe_flag_default_off=not DualFlowConfig().enable_spectrum_safe_policy_mass_patience_cap_traj_fail_closed_dry_run_matrix_reconnect_probe,
        spectrum_mass_patience_cap_traj=spectrum_mass_patience_cap_traj,
        fail_closed_dry_run_all_matched=bool(dry.all_matched),
        fail_closed_dry_run_n_cases=int(dry.n_cases),
        fail_closed_dry_run_n_matched=int(dry.n_matched),
...
@dataclass(frozen=True)
class ResidualSpectrumDualPathFailClosedDryRunComposeProbe:
    """Residual×spectrum dual-path × fail_closed dry_run EG compose (A5-T105).

    Harness only — packages :func:`probe_residual_mass_policy_patience_cap`
    and :func:`probe_spectrum_safe_policy_mass_patience_cap` under a shared
    :func:`proteus.evidence.gate.probe_fail_closed_dry_run_evidence_gate`.
    Complements A5-T100 (reconnect) with dry_run EG. Does **not** flip
    mass/density ``@awaiting`` or GateConfig defaults.
    """
    ...
    note: str = (
        "harness only: residual×spectrum dual-path patience×cap compose "
        "bridged to fail_closed dry_run × EvidenceGate; "
        "defaults unchanged; do not flip @awaiting / apply_dual / "
        "fail_closed / enable_dual_adjacency"
    )


def probe_residual_spectrum_dual_path_fail_closed_dry_run_compose(
    samples: Sequence[np.ndarray],
    simplex_positions: Mapping[Hashable, np.ndarray],
    simplices: Sequence[Sequence[Hashable]]
    | Mapping[Hashable, Sequence[Hashable]],
    ...
    *,
    ...
    complex_path: object | None = None,
    config: DualFlowConfig | None = None,
) -> ResidualSpectrumDualPathFailClosedDryRunComposeProbe | None:
    """Compose residual + spectrum patience×cap with fail_closed dry_run EG (A5-T105).

    When ``enable_residual_spectrum_dual_path_fail_closed_dry_run_compose_probe``
    is off, returns ``None``. When on:
    ...
    cfg = config or DualFlowConfig()
    if not cfg.enable_residual_spectrum_dual_path_fail_closed_dry_run_compose_probe:
        return None
    if not samples:
        raise ValueError("samples must be non-empty")
    ...
    if residual_patience_cap is None:
        raise RuntimeError(
            "residual×mass×policy patience×cap unexpectedly None under dual-path dry_run compose cfg"
        )
    spectrum_mass_patience_cap = probe_spectrum_safe_policy_mass_patience_cap(
        samples,
        simplex_positions,
    ...
    if spectrum_mass_patience_cap is None:
        raise RuntimeError(
            "spectrum×policy×mass patience×cap unexpectedly None under dual-path dry_run compose cfg"
        )
    ...
    gate_defaults = GateConfig()
    dual_defaults = DualFlowConfig()
    return ResidualSpectrumDualPathFailClosedDryRunComposeProbe(
        probe_flag_default_off=not DualFlowConfig().enable_residual_spectrum_dual_path_fail_closed_dry_run_compose_probe,
        residual_patience_cap=residual_patience_cap,
        spectrum_mass_patience_cap=spectrum_mass_patience_cap,
        fail_closed_dry_run_all_matched=bool(dry.all_matched),
        fail_closed_dry_run_n_cases=int(dry.n_cases),
...
@dataclass(frozen=True)
class ResidualSpectrumDualPathPatienceCapTrajFailClosedDryRunComposeProbe:
    """Residual×spectrum traj × fail_closed dry_run EG compose (A5-T106).

    Harness only — packages :func:`probe_residual_mass_policy_patience_cap_traj`
    and :func:`probe_spectrum_safe_policy_mass_patience_cap_traj` under a shared
    :func:`proteus.evidence.gate.probe_fail_closed_dry_run_evidence_gate`.
    Complements A5-T103 (traj compose) and A5-T105 (non-traj dry_run). Does
    **not** flip mass/density ``@awaiting`` or GateConfig defaults.
    """
...
def probe_residual_spectrum_dual_path_patience_cap_traj_fail_closed_dry_run_compose(
    samples: Sequence[np.ndarray],
    simplex_positions: Mapping[Hashable, np.ndarray],
    simplices: Sequence[Sequence[Hashable]]
    | Mapping[Hashable, Sequence[Hashable]],
    ...
    *,
    ...
    complex_path: object | None = None,
    config: DualFlowConfig | None = None,
) -> ResidualSpectrumDualPathPatienceCapTrajFailClosedDryRunComposeProbe | None:
    """Compose residual + spectrum traj with fail_closed dry_run EG (A5-T106).

    When ``enable_residual_spectrum_dual_path_patience_cap_traj_fail_closed_dry_run_compose_probe``
    is off, returns ``None``. When on:
    ...
    cfg = config or DualFlowConfig()
    if not cfg.enable_residual_spectrum_dual_path_patience_cap_traj_fail_closed_dry_run_compose_probe:
        return None
    if not samples:
        raise ValueError("samples must be non-empty")
    ...
    if residual_patience_cap_traj is None:
        raise RuntimeError(
            "residual×mass×policy patience×cap traj unexpectedly None under dual-path traj dry_run compose cfg"
        )
    spectrum_mass_patience_cap_traj = probe_spectrum_safe_policy_mass_patience_cap_traj(
        samples,
        simplex_positions,
    ...
    if spectrum_mass_patience_cap_traj is None:
        raise RuntimeError(
            "spectrum×policy×mass patience×cap traj unexpectedly None under dual-path traj dry_run compose cfg"
        )
    ...
    gate_defaults = GateConfig()
    dual_defaults = DualFlowConfig()
    return ResidualSpectrumDualPathPatienceCapTrajFailClosedDryRunComposeProbe(
        probe_flag_default_off=not DualFlowConfig().enable_residual_spectrum_dual_path_patience_cap_traj_fail_closed_dry_run_compose_probe,
        residual_patience_cap_traj=residual_patience_cap_traj,
        spectrum_mass_patience_cap_traj=spectrum_mass_patience_cap_traj,
        fail_closed_dry_run_all_matched=bool(dry.all_matched),
        fail_closed_dry_run_n_cases=int(dry.n_cases),
...
@dataclass(frozen=True)
class ResidualSpectrumDualPathPatienceCapTrajFailClosedReconnectComposeProbe:
    """Residual×spectrum traj × fail_closed reconnect EG compose (A5-T107).

    Harness only — packages :func:`probe_residual_mass_policy_patience_cap_traj`
    and :func:`probe_spectrum_safe_policy_mass_patience_cap_traj` under a shared
    :func:`proteus.evidence.gate.probe_fail_closed_dry_run_reconnect_bridge`.
    Complements A5-T103 (traj) and A5-T100 (non-traj reconnect). Does **not**
    flip mass/density ``@awaiting`` or GateConfig defaults.
    """
    ...
    note: str = (
        "harness only: residual×spectrum dual-path patience×cap traj compose "
        "bridged to fail_closed reconnect × EvidenceGate; "
        "defaults unchanged; do not flip @awaiting / apply_dual / "
        "fail_closed / enable_dual_adjacency"
    )


def probe_residual_spectrum_dual_path_patience_cap_traj_fail_closed_reconnect_compose(
    samples: Sequence[np.ndarray],
    simplex_positions: Mapping[Hashable, np.ndarray],
    simplices: Sequence[Sequence[Hashable]]
    | Mapping[Hashable, Sequence[Hashable]],
    ...
    *,
    ...
    complex_path: object | None = None,
    config: DualFlowConfig | None = None,
) -> ResidualSpectrumDualPathPatienceCapTrajFailClosedReconnectComposeProbe | None:
    """Compose residual + spectrum traj with fail_closed reconnect EG (A5-T107).

    When ``enable_residual_spectrum_dual_path_patience_cap_traj_fail_closed_reconnect_compose_probe``
    is off, returns ``None``. When on:
    ...
    cfg = config or DualFlowConfig()
    if not cfg.enable_residual_spectrum_dual_path_patience_cap_traj_fail_closed_reconnect_compose_probe:
        return None
    if not samples:
        raise ValueError("samples must be non-empty")
    ...
    if residual_patience_cap_traj is None:
        raise RuntimeError(
            "residual×mass×policy patience×cap traj unexpectedly None under dual-path traj reconnect compose cfg"
        )
    spectrum_mass_patience_cap_traj = probe_spectrum_safe_policy_mass_patience_cap_traj(
        samples,
        simplex_positions,
    ...
    if spectrum_mass_patience_cap_traj is None:
        raise RuntimeError(
            "spectrum×policy×mass patience×cap traj unexpectedly None under dual-path traj reconnect compose cfg"
        )
    ...
    gate_defaults = GateConfig()
    dual_defaults = DualFlowConfig()
    return ResidualSpectrumDualPathPatienceCapTrajFailClosedReconnectComposeProbe(
        probe_flag_default_off=not DualFlowConfig().enable_residual_spectrum_dual_path_patience_cap_traj_fail_closed_reconnect_compose_probe,
        residual_patience_cap_traj=residual_patience_cap_traj,
        spectrum_mass_patience_cap_traj=spectrum_mass_patience_cap_traj,
        fail_closed_reconnect_all_matched=bool(recon.all_matched),
        fail_closed_reconnect_n_cases=int(recon.n_cases),
...
@dataclass(frozen=True)
class ResidualSpectrumDualPathPatienceCapTrajFailClosedMatrixComposeProbe:
    """Residual×spectrum traj × fail_closed matrix EG compose (A5-T108).

    Harness only — packages :func:`probe_residual_mass_policy_patience_cap_traj`
    and :func:`probe_spectrum_safe_policy_mass_patience_cap_traj` under a shared
    :func:`proteus.evidence.gate.probe_fail_closed_evidence_gate_matrix`.
    Complements A5-T103 (traj) and matrix bridges. Does **not** flip
    mass/density ``@awaiting`` or GateConfig defaults.
    """
    ...
    note: str = (
        "harness only: residual×spectrum dual-path patience×cap traj compose "
        "bridged to fail_closed matrix × EvidenceGate; "
        "defaults unchanged; do not flip @awaiting / apply_dual / "
        "fail_closed / enable_dual_adjacency"
    )


def probe_residual_spectrum_dual_path_patience_cap_traj_fail_closed_matrix_compose(
    samples: Sequence[np.ndarray],
    simplex_positions: Mapping[Hashable, np.ndarray],
    simplices: Sequence[Sequence[Hashable]]
    | Mapping[Hashable, Sequence[Hashable]],
    ...
    *,
    ...
    affected_simplices: Sequence[Hashable] | None = None,
    config: DualFlowConfig | None = None,
) -> ResidualSpectrumDualPathPatienceCapTrajFailClosedMatrixComposeProbe | None:
    """Compose residual + spectrum traj with fail_closed matrix EG (A5-T108).

    When ``enable_residual_spectrum_dual_path_patience_cap_traj_fail_closed_matrix_compose_probe``
    is off, returns ``None``. When on:
    ...
    cfg = config or DualFlowConfig()
    if not cfg.enable_residual_spectrum_dual_path_patience_cap_traj_fail_closed_matrix_compose_probe:
        return None
    if not samples:
        raise ValueError("samples must be non-empty")
    ...
    if residual_patience_cap_traj is None:
        raise RuntimeError(
            "residual×mass×policy patience×cap traj unexpectedly None under dual-path traj matrix compose cfg"
        )
    spectrum_mass_patience_cap_traj = probe_spectrum_safe_policy_mass_patience_cap_traj(
        samples,
        simplex_positions,
    ...
    if spectrum_mass_patience_cap_traj is None:
        raise RuntimeError(
            "spectrum×policy×mass patience×cap traj unexpectedly None under dual-path traj matrix compose cfg"
        )
    ...
    gate_defaults = GateConfig()
    dual_defaults = DualFlowConfig()
    return ResidualSpectrumDualPathPatienceCapTrajFailClosedMatrixComposeProbe(
        probe_flag_default_off=not DualFlowConfig().enable_residual_spectrum_dual_path_patience_cap_traj_fail_closed_matrix_compose_probe,
        residual_patience_cap_traj=residual_patience_cap_traj,
        spectrum_mass_patience_cap_traj=spectrum_mass_patience_cap_traj,
        fail_closed_matrix_all_matched=bool(matrix.all_matched),
        fail_closed_matrix_n_cases=int(matrix.n_cases),
...
@dataclass(frozen=True)
class ResidualSpectrumDualPathPatienceCapTrajFailClosedDryRunMatrixReconnectComposeProbe:
    """Residual×spectrum traj × fail_closed dry_run×matrix×reconnect (A5-T109).

    Harness only — packages :func:`probe_residual_mass_policy_patience_cap_traj`
    and :func:`probe_spectrum_safe_policy_mass_patience_cap_traj` under shared
    :func:`proteus.evidence.gate.probe_fail_closed_dry_run_evidence_gate`,
    :func:`proteus.evidence.gate.probe_fail_closed_evidence_gate_matrix`, and
    :func:`proteus.evidence.gate.probe_fail_closed_dry_run_reconnect_bridge`.
    Mega-compose of A5-T106/T107/T108. Does **not** flip mass/density
    ``@awaiting`` or GateConfig defaults.
    """
    ...
    note: str = (
        "harness only: residual×spectrum dual-path patience×cap traj compose "
        "bridged to fail_closed dry_run × matrix × reconnect EvidenceGate; "
        "defaults unchanged; do not flip @awaiting / apply_dual / "
        "fail_closed / enable_dual_adjacency"
    )


def probe_residual_spectrum_dual_path_patience_cap_traj_fail_closed_dry_run_matrix_reconnect_compose(
    samples: Sequence[np.ndarray],
    simplex_positions: Mapping[Hashable, np.ndarray],
    simplices: Sequence[Sequence[Hashable]]
    | Mapping[Hashable, Sequence[Hashable]],
    ...
    *,
    ...
    complex_path: object | None = None,
    config: DualFlowConfig | None = None,
) -> ResidualSpectrumDualPathPatienceCapTrajFailClosedDryRunMatrixReconnectComposeProbe | None:
    """Compose residual + spectrum traj with dry_run×matrix×reconnect EG (A5-T109).

    When ``enable_residual_spectrum_dual_path_patience_cap_traj_fail_closed_dry_run_matrix_reconnect_compose_probe``
    is off, returns ``None``. When on:

    1. Run :func:`probe_residual_mass_policy_patience_cap_traj` (flag forced on).
    2. Run :func:`probe_spectrum_safe_policy_mass_patience_cap_traj` (flag forced on).
    3. Run gate :func:`probe_fail_closed_dry_run_evidence_gate`.
    4. Run gate :func:`probe_fail_closed_evidence_gate_matrix`.
    5. Run gate :func:`probe_fail_closed_dry_run_reconnect_bridge`.

    Does **not** flip mass/density ``@awaiting`` or GateConfig defaults.
    """

    cfg = config or DualFlowConfig()
    if not cfg.enable_residual_spectrum_dual_path_patience_cap_traj_fail_closed_dry_run_matrix_reconnect_compose_probe:
        return None
    if not samples:
        raise ValueError("samples must be non-empty")
    ...
    if residual_patience_cap_traj is None:
        raise RuntimeError(
            "residual×mass×policy patience×cap traj unexpectedly None under dual-path traj mega-compose cfg"
        )
    spectrum_mass_patience_cap_traj = probe_spectrum_safe_policy_mass_patience_cap_traj(
        samples,
        simplex_positions,
    ...
    if spectrum_mass_patience_cap_traj is None:
        raise RuntimeError(
            "spectrum×policy×mass patience×cap traj unexpectedly None under dual-path traj mega-compose cfg"
        )
    ...
    gate_defaults = GateConfig()
    dual_defaults = DualFlowConfig()
    return ResidualSpectrumDualPathPatienceCapTrajFailClosedDryRunMatrixReconnectComposeProbe(
        probe_flag_default_off=not DualFlowConfig().enable_residual_spectrum_dual_path_patience_cap_traj_fail_closed_dry_run_matrix_reconnect_compose_probe,
        residual_patience_cap_traj=residual_patience_cap_traj,
        spectrum_mass_patience_cap_traj=spectrum_mass_patience_cap_traj,
        fail_closed_dry_run_all_matched=bool(dry.all_matched),
        fail_closed_dry_run_n_cases=int(dry.n_cases),
...
@dataclass(frozen=True)
class ResidualMassPolicyPatienceCapTrajFailClosedDryRunMatrixReconnectProbe:
    """Residual traj × fail_closed dry_run×matrix×reconnect (A5-T110).
    ...
    :func:`proteus.evidence.gate.probe_fail_closed_evidence_gate_matrix`, and
    :func:`proteus.evidence.gate.probe_fail_closed_dry_run_reconnect_bridge`.
    Residual counterpart to A5-T104. Does **not** flip mass/density
    ``@awaiting`` or GateConfig defaults.
    """
...
def probe_residual_mass_policy_patience_cap_traj_fail_closed_dry_run_matrix_reconnect(
    samples: Sequence[np.ndarray],
    simplex_positions: Mapping[Hashable, np.ndarray],
    simplices: Sequence[Sequence[Hashable]]
    | Mapping[Hashable, Sequence[Hashable]],
    ...
    *,
    ...
    complex_path: object | None = None,
    config: DualFlowConfig | None = None,
) -> ResidualMassPolicyPatienceCapTrajFailClosedDryRunMatrixReconnectProbe | None:
    """Bridge residual traj with dry_run, matrix, and reconnect (A5-T110).

    When ``enable_residual_mass_policy_patience_cap_traj_fail_closed_dry_run_matrix_reconnect_probe``
    is off, returns ``None``. When on:
    ...
    cfg = config or DualFlowConfig()
    if not cfg.enable_residual_mass_policy_patience_cap_traj_fail_closed_dry_run_matrix_reconnect_probe:
        return None
    if not samples:
        raise ValueError("samples must be non-empty")
    ...
    if residual_patience_cap_traj is None:
        raise RuntimeError(
            "residual×mass×policy patience×cap traj unexpectedly None under dry_run×matrix×reconnect cfg"
        )
    ...
    gate_defaults = GateConfig()
    dual_defaults = DualFlowConfig()
    return ResidualMassPolicyPatienceCapTrajFailClosedDryRunMatrixReconnectProbe(
        probe_flag_default_off=not DualFlowConfig().enable_residual_mass_policy_patience_cap_traj_fail_closed_dry_run_matrix_reconnect_probe,
        residual_patience_cap_traj=residual_patience_cap_traj,
        fail_closed_dry_run_all_matched=bool(dry.all_matched),
        fail_closed_dry_run_n_cases=int(dry.n_cases),
        fail_closed_dry_run_n_matched=int(dry.n_matched),
...
@dataclass(frozen=True)
class ResidualSpectrumDualPathPatienceCapFailClosedDryRunMatrixReconnectComposeProbe:
    """Residual×spectrum dual-path × fail_closed dry_run×matrix×reconnect (A5-T111).

    Harness only — packages :func:`probe_residual_mass_policy_patience_cap`
    and :func:`probe_spectrum_safe_policy_mass_patience_cap` under shared
    :func:`proteus.evidence.gate.probe_fail_closed_dry_run_evidence_gate`,
    :func:`proteus.evidence.gate.probe_fail_closed_evidence_gate_matrix`, and
    :func:`proteus.evidence.gate.probe_fail_closed_dry_run_reconnect_bridge`.
    Non-traj mega-compose complementing A5-T109. Does **not** flip
    mass/density ``@awaiting`` or GateConfig defaults.
    """
...
def probe_residual_spectrum_dual_path_patience_cap_fail_closed_dry_run_matrix_reconnect_compose(
    samples: Sequence[np.ndarray],
    simplex_positions: Mapping[Hashable, np.ndarray],
    simplices: Sequence[Sequence[Hashable]]
    | Mapping[Hashable, Sequence[Hashable]],
    ...
    *,
    ...
    complex_path: object | None = None,
    config: DualFlowConfig | None = None,
) -> ResidualSpectrumDualPathPatienceCapFailClosedDryRunMatrixReconnectComposeProbe | None:
    """Compose residual + spectrum patience×cap with dry_run×matrix×reconnect (A5-T111).

    When ``enable_residual_spectrum_dual_path_patience_cap_fail_closed_dry_run_matrix_reconnect_compose_probe``
    is off, returns ``None``. When on:
    ...
    cfg = config or DualFlowConfig()
    if not cfg.enable_residual_spectrum_dual_path_patience_cap_fail_closed_dry_run_matrix_reconnect_compose_probe:
        return None
    if not samples:
        raise ValueError("samples must be non-empty")
    ...
    if residual_patience_cap is None:
        raise RuntimeError(
            "residual×mass×policy patience×cap unexpectedly None under dual-path mega-compose cfg"
        )
    spectrum_mass_patience_cap = probe_spectrum_safe_policy_mass_patience_cap(
        samples,
        simplex_positions,
    ...
    if spectrum_mass_patience_cap is None:
        raise RuntimeError(
            "spectrum×policy×mass patience×cap unexpectedly None under dual-path mega-compose cfg"
        )
    ...
    gate_defaults = GateConfig()
    dual_defaults = DualFlowConfig()
    return ResidualSpectrumDualPathPatienceCapFailClosedDryRunMatrixReconnectComposeProbe(
        probe_flag_default_off=not DualFlowConfig().enable_residual_spectrum_dual_path_patience_cap_fail_closed_dry_run_matrix_reconnect_compose_probe,
        residual_patience_cap=residual_patience_cap,
        spectrum_mass_patience_cap=spectrum_mass_patience_cap,
        fail_closed_dry_run_all_matched=bool(dry.all_matched),
        fail_closed_dry_run_n_cases=int(dry.n_cases),
