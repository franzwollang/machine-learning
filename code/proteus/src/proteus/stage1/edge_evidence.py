...
class HollowEdgeConfig:
    ...
    *both* cross-shell and intra-shell lifted edges (non-discriminative).
    ...
    see :data:`DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_*`. Denser
    mid-band soft fracs {0.03,0.08,0.1,0.12,0.18,0.25} across seeds 0..2:
    seed0 soft≤0.12 → tori K=2 chance-ARI≈0.14–0.18 (gabriel conj kills;
    soft≥0.18 collapses); seeds 1–2 all ≤1 at majors — keep window is
    **not** seed-stable. Lean e2e non-persist: only seed0 youden nested
    K=2 chance-ARI≈0.01; soft/soft×conj all ≤1. Defaults off.

    A2-T72: denser soft keep×gabriel×persist majors-pin multi-seed
    confirm — see
    :data:`DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_*`.
    Extends T69 seed0 majors pin across seeds 0..2 with lean keep fracs
    {0.05,0.12,0.15}: seed0 soft≤0.12 → tori K=2 (gabriel kills;
    ...
    T73 non-persist — see
    :data:`DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_*`.
    Seed0 T73 keep+inflate fracs {0.05,0.12,0.25}: majors soft≤0.12 →
    tori K=2 (gabriel kills; soft=0.25 collapses); e2e non-persist matches
    T73 (youden nested K=2≈0.01; soft all ≤1); soft×persist /
    soft×conj×persist e2e also all ≤1 — persist does **not** rescue T73
    soft e2e collapse. Defaults off.

    A2-T76: denser soft keep×gabriel×persist majors-pin seed0-only ARI
    probe — see
    :data:`DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_*`.
    ...
    collapses); soft×persist e2e all ≤1 across the fine grid; only seed0
    youden nested K=2≈0.01. Defaults off.
    """
...
# ---------------------------------------------------------------------------
# Denser soft×gabriel majors seed-stable window (non-persist) (A2-T71)
# ---------------------------------------------------------------------------
# denser n=160/240, max_nodes=128, Youden h0≈0.73, mid=0.5 gabriel=False,
# betweenness soft_capacity, seeds 0..2, denser mid-band soft fracs
# {0.03,0.08,0.1,0.12,0.18,0.25}. Fixed-tau majors (0.27/0.5): T55/T64/T66
# seed0 keep soft_frac≤0.12 → tori K=2 chance-ARI≈0.14–0.18 survives denser
# mid-band pin; gabriel conj kills seed0 keep; soft≥0.18 collapses. Seeds
# 1–2 stay ≤1 under youden/soft/conj at every denser frac — majors keep
# window is NOT seed-stable (extends T66 lean-frac seed0-only). Lean tau*
# e2e (max_grid_points=12, scale_seed=42+seed, non-persist): only seed0
# youden nested K=2 chance-ARI≈0.01; soft/soft×conj collapse nested+tori
# to ≤1 across denser keep/collapse and seeds 1–2. Soft≠recovery.

DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_SEEDS: tuple[int, ...] = (
    0, 1, 2,
)
DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_FRACS: tuple[float, ...] = (
    0.03, 0.08, 0.1, 0.12, 0.18, 0.25,
)
DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_NESTED_N: int = (
    DENSER_PROPOSED_H0_NESTED_N
)
DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_TORI_N: int = (
    DENSER_PROPOSED_H0_TORI_N
)
DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_MAX_NODES: int = (
    DENSER_PROPOSED_H0_MAX_NODES
)
DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_NESTED_TAU: float = 0.27
DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_TORI_TAU: float = 0.5
DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_H0: float = PROPOSED_H0_YOUDEN
DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_METHOD: str = "betweenness"
DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_MAX_GRID_POINTS: int = 12
DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_SCALE_SEED_BASE: int = 42
DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_KEEP_MAX_FRAC: float = (
    DENSER_SOFT_SEED0_TORI_ARI_WINDOW_KEEP_MAX_FRAC
)
DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_COLLAPSE_MIN_FRAC: float = (
    DENSER_SOFT_SEED0_TORI_ARI_WINDOW_COLLAPSE_MIN_FRAC
)

# seed → mode → (nested_majors, nested_ari, tori_majors, tori_ari)
DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_TABLE: dict[
    int, dict[str, tuple[int, float | None, int, float | None]]
] = {
    0: {
        "youden": (1, None, 2, 0.14),
        "conj": (1, None, 1, None),
        "soft_0.03": (1, None, 2, 0.14),
        "soft_x_conj_0.03": (1, None, 1, None),
        "soft_0.08": (1, None, 2, 0.16),
        "soft_x_conj_0.08": (1, None, 1, None),
        "soft_0.1": (1, None, 2, 0.18),
        "soft_x_conj_0.1": (1, None, 1, None),
        ...
        "soft_x_conj_0.12": (1, None, 1, None),
        "soft_0.18": (1, None, 1, None),
        "soft_x_conj_0.18": (1, None, 1, None),
        "soft_0.25": (1, None, 1, None),
        "soft_x_conj_0.25": (1, None, 1, None),
    },
    1: {
...
# seed → mode → (nested_leaves, nested_ari, tori_leaves, tori_ari)
DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_E2E_TABLE: dict[
    int, dict[str, tuple[int, float | None, int, float | None]]
] = {
    0: {
        "youden": (2, 0.01, 1, None),
...
DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_UNIFORMS: dict[
    str, dict[str, int]
] = {
    "circle": {
        "youden": 1,
...
DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_SI_NOTE: str = (
    "A2-T71 denser soft×require_gabriel_and_h majors seed-stable window "
    "(non-persist; n=160/240, max_nodes=128, Youden h0≈0.73, mid=0.5 "
    "gabriel=False, betweenness, seeds 0..2, denser mid-band soft fracs "
    "{0.03,0.08,0.1,0.12,0.18,0.25}, fixed-tau majors 0.27/0.5 + lean "
    "tau* e2e max_grid_points=12 scale_seed=42+seed): T55/T64/T66 seed0 "
    "majors keep soft_frac≤0.12 → tori K=2 chance-ARI≈0.14–0.18 survives "
    "denser mid-band; gabriel conj kills seed0 keep; soft≥0.18 collapses. "
    "Seeds 1–2 stay ≤1 under youden/soft/conj at every denser frac — "
    "majors keep window is NOT seed-stable (extends T66 lean-frac "
    "seed0-only). Lean e2e non-persist: only seed0 youden nested K=2 "
    "chance-ARI≈0.01; soft/soft×conj collapse nested+tori to ≤1 across "
    "denser keep/collapse and seeds 1–2. Chance-ARI ≠ sample-ARI "
    "recovery; defaults off; no awaiting flip."
)


def format_denser_soft_x_gabriel_majors_seed_stable_window_table() -> str:
    """TSV export of denser soft×gabriel majors seed-stable window (A2-T71)."""

    lines = [
        "# denser soft×gabriel majors seed-stable window (non-persist)",
        f"# nested_n="
        f"{DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_NESTED_N} "
        f"tori_n={DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_TORI_N} "
        f"max_nodes="
        f"{DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_MAX_NODES} "
        f"h0={DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_H0:g} "
        f"method={DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_METHOD} "
        f"max_grid_points="
        f"{DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_MAX_GRID_POINTS} "
        f"scale_seed_base="
        f"{DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_SCALE_SEED_BASE} "
        f"seeds="
        f"{list(DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_SEEDS)} "
        f"fracs="
        f"{list(DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_FRACS)} "
        f"keep_max="
        f"{DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_KEEP_MAX_FRAC:g} "
        f"collapse_min="
        f"{DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_COLLAPSE_MIN_FRAC:g}",
        "surface\tseed\tmode\tdataset\ttau_or_e2e\tmajors_or_leaves\tsample_ari",
    ]
    for seed in DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_SEEDS:
        for mode, (nm, na, tm, ta) in (
            DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_TABLE[seed].items()
        ):
            na_s = "" if na is None else f"{na:.2f}"
            ta_s = "" if ta is None else f"{ta:.2f}"
            lines.append(
                f"majors\t{seed}\t{mode}\tnested\t"
                f"{DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_NESTED_TAU:g}\t"
                f"{nm}\t{na_s}"
            )
            lines.append(
                f"majors\t{seed}\t{mode}\ttori\t"
                f"{DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_TORI_TAU:g}\t"
                f"{tm}\t{ta_s}"
            )
        for mode, (nl, na, tl, ta) in (
    ...
    lines.append("dataset\tmode\tleaves")
    for dataset, mode_table in (
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_UNIFORMS.items()
    ):
        for mode, leaves in mode_table.items():
            lines.append(f"{dataset}\t{mode}\t{leaves}")
    lines.append(
        f"# {DENSER_SOFT_X_GABRIEL_MAJORS_SEED_STABLE_WINDOW_SI_NOTE}"
    )
    return "\n".join(lines)


# ---------------------------------------------------------------------------
# Denser soft keep×gabriel×persist majors-pin multi-seed confirm (A2-T72)
# ---------------------------------------------------------------------------
# denser n=160/240, max_nodes=128, Youden h0≈0.73, mid=0.5 gabriel=False,
# betweenness soft_capacity, seeds 0..2, lean keep fracs {0.05,0.12,0.15}.
# Fixed-tau majors (0.27/0.5): T69 seed0 denser majors keep soft≤0.12 →
# tori K=2 chance-ARI≈0.16–0.18; gabriel conj kills seed0 keep; soft≥0.15
# collapses. Seeds 1–2 stay ≤1 under youden/soft/conj — majors pin is NOT
# seed-stable (extends T69 seed0-only + T71 non-persist multi-seed). Lean
# tau* e2e (max_grid_points=12, scale_seed=42+seed): soft×persist /
# soft×conj×persist collapse nested+tori to ≤1 across keep/collapse and
# seeds 0..2; only seed0 youden nested K=2 chance-ARI≈0.01. Soft≠recovery.

DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_SEEDS: tuple[
    int, ...
] = (
    0, 1, 2,
)
DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_FRACS: tuple[
    float, ...
] = (
    0.05, 0.12, 0.15,
)
DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_NESTED_N: int = (
    DENSER_PROPOSED_H0_NESTED_N
)
DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_TORI_N: int = (
    DENSER_PROPOSED_H0_TORI_N
)
DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_MAX_NODES: int = (
    DENSER_PROPOSED_H0_MAX_NODES
)
DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_NESTED_TAU: float = (
    0.27
)
DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_TORI_TAU: float = (
    0.5
)
DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_H0: float = (
    PROPOSED_H0_YOUDEN
)
DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_METHOD: str = (
    "betweenness"
)
DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_MAX_GRID_POINTS: int = (
    12
)
DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_SCALE_SEED_BASE: int = (
    42
)
DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_KEEP_MAX_FRAC: float = (
    DENSER_SOFT_SEED0_TORI_ARI_WINDOW_KEEP_MAX_FRAC
)
DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_COLLAPSE_MIN_FRAC: float = (
    DENSER_SOFT_SEED0_TORI_ARI_WINDOW_COLLAPSE_MIN_FRAC
)

# seed → mode → (nested_majors, nested_ari, tori_majors, tori_ari)
DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_TABLE: dict[
    int, dict[str, tuple[int, float | None, int, float | None]]
] = {
    0: {
        "youden": (1, None, 2, 0.14),
...
# seed → mode → (nested_leaves, nested_ari, tori_leaves, tori_ari)
DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_E2E_TABLE: dict[
    int, dict[str, tuple[int, float | None, int, float | None]]
] = {
    0: {
        "youden": (2, 0.01, 1, None),
...
DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_UNIFORMS: dict[
    str, dict[str, int]
] = {
    "circle": {
        "youden": 1,
...
DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_SI_NOTE: str = (
    "A2-T72 denser soft keep-band × require_gabriel_and_h × persist "
    "majors-pin multi-seed confirm (n=160/240, max_nodes=128, Youden "
    "h0≈0.73, mid=0.5 gabriel=False, betweenness, seeds 0..2, lean keep "
    "fracs {0.05,0.12,0.15}, fixed-tau majors 0.27/0.5 + lean tau* e2e "
    "max_grid_points=12 scale_seed=42+seed): T69 seed0 denser majors keep "
    "soft_frac≤0.12 → tori K=2 chance-ARI≈0.16–0.18; gabriel conj kills "
    "seed0 keep; soft≥0.15 collapses. Seeds 1–2 stay ≤1 under "
    "youden/soft/conj — majors pin is NOT seed-stable (extends T69 "
    "seed0-only + T71 non-persist multi-seed). Soft×persist / "
    "soft×conj×persist e2e collapse nested+tori to ≤1 across "
    "keep/collapse and seeds 0..2; only seed0 youden nested K=2 "
    "chance-ARI≈0.01. Circle/swiss stay 1. Chance-ARI ≠ sample-ARI "
    "recovery; defaults off; no awaiting flip."
)


def format_denser_soft_keep_band_x_gabriel_x_persist_majors_pin_multiseed_table() -> str:
    """TSV export of denser soft keep×gabriel×persist majors-pin multi-seed (A2-T72)."""

    lines = [
        "# denser soft keep×gabriel×persist majors-pin multi-seed confirm",
        f"# nested_n="
        f"{DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_NESTED_N} "
        f"tori_n="
        f"{DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_TORI_N} "
        f"max_nodes="
        f"{DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_MAX_NODES} "
        f"h0="
        f"{DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_H0:g} "
        f"method="
        f"{DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_METHOD} "
        f"max_grid_points="
        f"{DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_MAX_GRID_POINTS} "
        f"scale_seed_base="
        f"{DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_SCALE_SEED_BASE} "
        f"seeds="
        f"{list(DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_SEEDS)} "
        f"fracs="
        f"{list(DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_FRACS)} "
        f"keep_max="
        f"{DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_KEEP_MAX_FRAC:g} "
        f"collapse_min="
        f"{DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_COLLAPSE_MIN_FRAC:g}",
        "surface\tseed\tmode\tdataset\ttau_or_e2e\tmajors_or_leaves\tsample_ari",
    ]
    for seed in DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_SEEDS:
        for mode, (nm, na, tm, ta) in (
            ...
            lines.append(
                f"majors\t{seed}\t{mode}\tnested\t"
                f"{DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_NESTED_TAU:g}\t"
                f"{nm}\t{na_s}"
            )
            lines.append(
                f"majors\t{seed}\t{mode}\ttori\t"
                f"{DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_TORI_TAU:g}\t"
                f"{tm}\t{ta_s}"
            )
        for mode, (nl, na, tl, ta) in (
    ...
    lines.append("dataset\tmode\tleaves")
    for dataset, mode_table in (
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_MULTISEED_UNIFORMS.items()
    ):
        for mode, leaves in mode_table.items():
            lines.append(f"{dataset}\t{mode}\t{leaves}")
    lines.append(
...
# ---------------------------------------------------------------------------
# Denser soft×gabriel majors seed0 keep×persist compose vs T73 non-persist
# (A2-T75)
# ---------------------------------------------------------------------------
# denser n=160/240, max_nodes=128, Youden h0≈0.73, mid=0.5 gabriel=False,
# betweenness soft_capacity, seed0 only, T73 keep+inflate fracs
# {0.05,0.12,0.25}. Contrast T73 non-persist e2e soft collapse against
# soft×persist compose: fixed-tau majors soft≤0.12 → tori K=2
# chance-ARI≈0.16–0.18 (gabriel conj kills; soft=0.25 collapses); lean
# tau* e2e non-persist matches T73 (youden nested K=2≈0.01; soft/soft×conj
# all ≤1); soft×persist / soft×conj×persist at the same fracs also all ≤1
# — persist does **not** rescue T73 soft e2e collapse. Soft≠recovery.

DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_SEED: int = (
    0
)
DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_FRACS: tuple[
    float, ...
] = (
    0.05, 0.12, 0.25,
)
DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_NESTED_N: int = (
    DENSER_PROPOSED_H0_NESTED_N
)
DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_TORI_N: int = (
    DENSER_PROPOSED_H0_TORI_N
)
DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_MAX_NODES: int = (
    DENSER_PROPOSED_H0_MAX_NODES
)
DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_NESTED_TAU: float = (
    0.27
)
DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_TORI_TAU: float = (
    0.5
)
DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_H0: float = (
    PROPOSED_H0_YOUDEN
)
DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_METHOD: str = (
    "betweenness"
)
DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_MAX_GRID_POINTS: int = (
    12
)
DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_SCALE_SEED_BASE: int = (
    42
)
DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_KEEP_MAX_FRAC: float = (
    DENSER_SOFT_SEED0_TORI_ARI_WINDOW_KEEP_MAX_FRAC
)
DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_INFLATE_FRAC: float = (
    0.25
)

# Fixed-tau majors (match T73 seed0): mode → (n_maj, n_ari, t_maj, t_ari)
DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_TABLE: dict[
    str, tuple[int, float | None, int, float | None]
] = {
    "youden": (1, None, 2, 0.14),
    "conj": (1, None, 1, None),
...
# Lean tau* e2e non-persist (match T73 seed0)
DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_E2E_NONPERSIST_TABLE: dict[
    str, tuple[int, float | None, int, float | None]
] = {
    "youden": (2, 0.01, 1, None),
    "soft_0.05": (1, None, 1, None),
...
# Lean tau* e2e persist compose — persist does not rescue soft collapse
DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_E2E_PERSIST_TABLE: dict[
    str, tuple[int, float | None, int, float | None]
] = {
    "youden": (2, 0.01, 1, None),
    "persist": (1, None, 1, None),
...
DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_UNIFORMS: dict[
    str, dict[str, int]
] = {
    "circle": {
        "youden": 1,
...
DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_SI_NOTE: str = (
    "A2-T75 denser soft×require_gabriel_and_h majors seed0 keep×persist "
    "compose vs T73 non-persist (n=160/240, max_nodes=128, Youden "
    "h0≈0.73, mid=0.5 gabriel=False, betweenness, seed0, T73 keep+inflate "
    "fracs {0.05,0.12,0.25}, fixed-tau majors 0.27/0.5 + lean tau* "
    "max_grid_points=12 scale_seed=42): majors soft≤0.12 → tori K=2 "
    "chance-ARI≈0.16–0.18 (gabriel conj kills; soft=0.25 collapses); "
    "e2e non-persist matches T73 (youden nested K=2≈0.01; soft all ≤1); "
    "soft×persist / soft×conj×persist e2e also all ≤1 — persist does "
    "not rescue T73 soft e2e collapse. Circle/swiss stay 1. Chance-ARI ≠ "
    "sample-ARI recovery; defaults off; no awaiting flip."
)


def format_denser_soft_x_gabriel_majors_seed0_keep_x_persist_compose_vs_t73_nonpersist_table() -> str:
    """TSV export of denser soft×gabriel seed0 keep×persist vs T73 (A2-T75)."""

    lines = [
        "# denser soft×gabriel majors seed0 keep×persist compose vs "
        "T73 non-persist",
        f"# nested_n="
        f"{DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_NESTED_N} "
        f"tori_n="
        f"{DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_TORI_N} "
        f"max_nodes="
        f"{DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_MAX_NODES} "
        f"h0="
        f"{DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_H0:g} "
        f"method="
        f"{DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_METHOD} "
        f"max_grid_points="
        f"{DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_MAX_GRID_POINTS} "
        f"scale_seed_base="
        f"{DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_SCALE_SEED_BASE} "
        f"seed="
        f"{DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_SEED} "
        f"fracs="
        f"{list(DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_FRACS)} "
        f"keep_max="
        f"{DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_KEEP_MAX_FRAC:g} "
        f"inflate_frac="
        f"{DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_INFLATE_FRAC:g}",
        "surface\tseed\tmode\tdataset\ttau_or_e2e\tmajors_or_leaves\tsample_ari",
    ]
    seed = (
    ...
    for mode, (nm, na, tm, ta) in (
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_TABLE.items()
    ):
        na_s = "" if na is None else f"{na:.2f}"
        ta_s = "" if ta is None else f"{ta:.2f}"
        lines.append(
            f"majors\t{seed}\t{mode}\tnested\t"
            f"{DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_NESTED_TAU:g}\t"
            f"{nm}\t{na_s}"
        )
        lines.append(
            f"majors\t{seed}\t{mode}\ttori\t"
            f"{DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_TORI_TAU:g}\t"
            f"{tm}\t{ta_s}"
        )
    for mode, (nl, na, tl, ta) in (
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_E2E_NONPERSIST_TABLE.items()
    ):
        na_s = "" if na is None else f"{na:.2f}"
        ta_s = "" if ta is None else f"{ta:.2f}"
        lines.append(
            f"e2e_nonpersist\t{seed}\t{mode}\tnested\ttau*\t{nl}\t{na_s}"
        )
        lines.append(
            f"e2e_nonpersist\t{seed}\t{mode}\ttori\ttau*\t{tl}\t{ta_s}"
        )
    for mode, (nl, na, tl, ta) in (
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_E2E_PERSIST_TABLE.items()
    ):
        na_s = "" if na is None else f"{na:.2f}"
        ta_s = "" if ta is None else f"{ta:.2f}"
        lines.append(
            f"e2e_persist\t{seed}\t{mode}\tnested\ttau*\t{nl}\t{na_s}"
        )
        lines.append(
            f"e2e_persist\t{seed}\t{mode}\ttori\ttau*\t{tl}\t{ta_s}"
        )
    lines.append("dataset\tmode\tleaves")
    for dataset, mode_table in (
        DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_UNIFORMS.items()
    ):
        for mode, leaves in mode_table.items():
            lines.append(f"{dataset}\t{mode}\t{leaves}")
    lines.append(
        f"# {DENSER_SOFT_X_GABRIEL_MAJORS_SEED0_KEEP_X_PERSIST_COMPOSE_VS_T73_NONPERSIST_SI_NOTE}"
    )
    return "\n".join(lines)


# ---------------------------------------------------------------------------
# Denser soft keep×gabriel×persist majors-pin seed0-only ARI probe (A2-T76)
# ---------------------------------------------------------------------------
# denser n=160/240, max_nodes=128, Youden h0≈0.73, mid=0.5 gabriel=False,
# betweenness soft_capacity, seed0 only, fine T55 soft_frac grid
# {0.05,0.08,0.10,0.12,0.15,0.18,0.20,0.25} under T69 soft×gabriel×persist
# majors-pin compose. Fixed-tau majors (0.27/0.5): soft_frac≤0.12 → tori
# K=2 chance-ARI≈0.16–0.18; gabriel conj kills keep; soft≥0.15 collapses
# (matches T55/T69 keep band). Lean tau* e2e (max_grid_points=12,
# scale_seed=42): soft×persist / soft×conj×persist collapse nested+tori
# to ≤1 across the entire fine grid; only bare youden nested K=2
# chance-ARI≈0.01. Circle/swiss stay 1. Soft≠recovery.

DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_SEED: int = 0
DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_FRACS: tuple[
    float, ...
] = DENSER_SOFT_SEED0_TORI_ARI_WINDOW_FRACS
DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_NESTED_N: int = (
    DENSER_PROPOSED_H0_NESTED_N
)
DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_TORI_N: int = (
    DENSER_PROPOSED_H0_TORI_N
)
DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_MAX_NODES: int = (
    DENSER_PROPOSED_H0_MAX_NODES
)
DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_NESTED_TAU: float = (
    0.27
)
DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_TORI_TAU: float = (
    0.5
)
DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_H0: float = (
    PROPOSED_H0_YOUDEN
)
DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_METHOD: str = (
    "betweenness"
)
DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_MAX_GRID_POINTS: int = (
    12
)
DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_SCALE_SEED_BASE: int = (
    42
)
DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_KEEP_MAX_FRAC: float = (
    DENSER_SOFT_SEED0_TORI_ARI_WINDOW_KEEP_MAX_FRAC
)
DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_COLLAPSE_MIN_FRAC: float = (
    DENSER_SOFT_SEED0_TORI_ARI_WINDOW_COLLAPSE_MIN_FRAC
)

# Fixed-tau majors: mode → (nested_majors, nested_ari, tori_majors, tori_ari)
DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_TABLE: dict[
    str, tuple[int, float | None, int, float | None]
] = {
    "youden": (1, None, 2, 0.14),
    "conj": (1, None, 1, None),
    ...
    "soft_x_conj_0.18": (1, None, 1, None),
    "soft_0.2": (1, None, 1, None),
    "soft_x_conj_0.2": (1, None, 1, None),
    "soft_0.25": (1, None, 1, None),
    "soft_x_conj_0.25": (1, None, 1, None),
}

# Lean tau* e2e under soft×gabriel×persist compose across fine ARI grid
DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_E2E_TABLE: dict[
    str, tuple[int, float | None, int, float | None]
] = {
    "youden": (2, 0.01, 1, None),
    "persist": (1, None, 1, None),
    ...
    "soft_x_persist_0.18": (1, None, 1, None),
    "soft_x_conj_x_persist_0.18": (1, None, 1, None),
    "soft_x_persist_0.2": (1, None, 1, None),
    "soft_x_conj_x_persist_0.2": (1, None, 1, None),
    "soft_x_persist_0.25": (1, None, 1, None),
    "soft_x_conj_x_persist_0.25": (1, None, 1, None),
}

DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_UNIFORMS: dict[
    str, dict[str, int]
] = {
    "circle": {
        "youden": 1,
...
DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_SI_NOTE: str = (
    "A2-T76 denser soft keep-band × require_gabriel_and_h × persist "
    "majors-pin seed0-only ARI probe (n=160/240, max_nodes=128, Youden "
    "h0≈0.73, mid=0.5 gabriel=False, betweenness, seed0, fine T55 "
    "soft_frac grid {0.05,0.08,0.10,0.12,0.15,0.18,0.20,0.25}, fixed-tau "
    "majors 0.27/0.5 + lean tau* max_grid_points=12 scale_seed=42): "
    "soft_frac≤0.12 → tori K=2 chance-ARI≈0.16–0.18 (gabriel conj kills; "
    "soft≥0.15 collapses) — T55 keep band survives under T69 "
    "soft×gabriel×persist compose; soft×persist / soft×conj×persist e2e "
    "all ≤1 across the fine grid; only bare youden nested K=2 "
    "chance-ARI≈0.01. Circle/swiss stay 1. Chance-ARI ≠ sample-ARI "
    "recovery; defaults off; no awaiting flip."
)


def format_denser_soft_keep_band_x_gabriel_x_persist_majors_pin_seed0_ari_probe_table() -> str:
    """TSV export of denser soft×gabriel×persist majors-pin seed0 ARI probe (A2-T76)."""

    lines = [
        "# denser soft keep×gabriel×persist majors-pin seed0-only ARI probe",
        f"# nested_n="
        f"{DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_NESTED_N} "
        f"tori_n="
        f"{DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_TORI_N} "
        f"max_nodes="
        f"{DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_MAX_NODES} "
        f"h0="
        f"{DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_H0:g} "
        f"method="
        f"{DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_METHOD} "
        f"max_grid_points="
        f"{DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_MAX_GRID_POINTS} "
        f"scale_seed_base="
        f"{DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_SCALE_SEED_BASE} "
        f"seed="
        f"{DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_SEED} "
        f"fracs="
        f"{list(DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_FRACS)} "
        f"keep_max="
        f"{DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_KEEP_MAX_FRAC:g} "
        f"collapse_min="
        f"{DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_COLLAPSE_MIN_FRAC:g}",
        "surface\tseed\tmode\tdataset\ttau_or_e2e\tmajors_or_leaves\tsample_ari",
    ]
    seed = DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_SEED
    for mode, (nm, na, tm, ta) in (
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_TABLE.items()
    ):
        na_s = "" if na is None else f"{na:.2f}"
        ta_s = "" if ta is None else f"{ta:.2f}"
        lines.append(
            f"majors\t{seed}\t{mode}\tnested\t"
            f"{DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_NESTED_TAU:g}\t"
            f"{nm}\t{na_s}"
        )
        lines.append(
            f"majors\t{seed}\t{mode}\ttori\t"
            f"{DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_TORI_TAU:g}\t"
            f"{tm}\t{ta_s}"
        )
    for mode, (nl, na, tl, ta) in (
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_E2E_TABLE.items()
    ):
        na_s = "" if na is None else f"{na:.2f}"
        ta_s = "" if ta is None else f"{ta:.2f}"
        lines.append(f"e2e\t{seed}\t{mode}\tnested\ttau*\t{nl}\t{na_s}")
    ...
    lines.append("dataset\tmode\tleaves")
    for dataset, mode_table in (
        DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_UNIFORMS.items()
    ):
        for mode, leaves in mode_table.items():
            lines.append(f"{dataset}\t{mode}\t{leaves}")
    lines.append(
        f"# {DENSER_SOFT_KEEP_BAND_X_GABRIEL_X_PERSIST_MAJORS_PIN_SEED0_ARI_PROBE_SI_NOTE}"
    )
    return "\n".join(lines)
