...
| M1 — Canonical clustering objective | blocked — #44 flag stays off. D6 keeps `require_separation_evidence` default off and kills cut persistence (null/composite Jaccard overlap); fixed-ceiling, curvature, and cap-rule families remain dead. `φ=0.25` stays unchanged/provisional, and cut-local density contrast is the only live #48 floor candidate this burn. #45 remains tissue context: weak shatter is tissue-sufficient, while bimodal shatter is not reproducible on the current tip. Hierarchy seed-0 `N`/`φ` are cap-path-dependent, not thresholds. No default or ceiling change is authorized. |
