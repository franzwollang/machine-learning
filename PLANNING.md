...
| M1 — Canonical clustering objective | blocked — #44 flag stays off. D4 declined the `require_separation_evidence` flip: the guard is ON≡OFF on all 60 metric lines and null-clean but cannot see live positive-`φ` sheet/strip/child accepts. Fixed-ceiling and curvature families remain killed; `φ=0.25` stays unchanged/provisional. Next #48 oracles are cut-local density contrast and cut persistence, not a new `φ` number. #45 is tissue/sibling context only; descent A/B are dead and interim C is adopted. Hierarchy seed 0 is a cap×ceiling interaction, not ceiling recalibration. No default or ceiling change is authorized. |
