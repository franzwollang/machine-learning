...
| M1 — Canonical clustering objective | blocked — #44 flag stays off. D7 records that every tested #48 floor family this burn is killed, including density contrast; closeout is documentation-only. `φ=0.25` stays unchanged/provisional and `require_separation_evidence` stays default off. #45 remains weak tissue-sufficient plus the current-tip shatter census; nested s0 is not over-splitting and tissue-in-child is not the residual. No default or ceiling change is authorized. |
