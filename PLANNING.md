...
| M4 — Stage 2 core (complex, evidence gate, dual flow) | in progress (step 1a flag-complex construction landed — `stage2/flag_complex.py`, S4.1/S4.2/S4.5/S13.4. step 2 DM EVIDENCE GATE landed — `evidence/dm_score.py`+`star_matrix.py`+`gate.py`, S3.4/S3.5/S3.6/S10.4; #42 resolved; #43 stub + DualAdjacency Protocol + SI anchors landed, still needs S6 dual_flow producer; #41 PH harness + `topology_from_accepted_regions` + diagnostics landed; SI lifetime_frac logged; recovery flips still need fitted-region evidence. Remaining: wire gate into runtime scaffold loop, T3 count transfer, dual-connectivity #43, topology-recovery #41, S6 dual flow) |
