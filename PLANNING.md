...
| M1 — Canonical clustering objective | blocked — #44 flag stays off. D9: A3-T16 killed scan-length monotonicity after the longest-aspect minimum `φ` rose `0.123→0.163`; A2-T16 matched-scan remains the sole live statistic-definition falsifier. `φ=0.25`, `require_separation_evidence`, and `use_level_set_clustering` remain unchanged. Convene D10 when A2-T16 reports. |
