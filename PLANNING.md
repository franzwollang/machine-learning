...
| M1 — Canonical clustering objective | in progress — #44 flag stays off. #48: `track_tau` walk + min-cut φ landed; φ ceiling 0.25 calibrated on 359 null reads (min 0.288). A2-T1 rules out nearest-saddle/Poisson support for the φ=0 false accept; only `core_hits_min` clears the 3x contrast, narrowly. #45: explicit `tissue_mass` generator path landed with legacy default unchanged; mass sweep and halo descent semantics remain. |
