...
| M1 — Canonical clustering objective | in progress — #44 flag stays off. #48: `track_tau` walk + min-cut φ landed; the reproducible null harness confirms min φ≈0.288 and the lone-Gaussian φ=0 false accept, with small count/percentile drift from the 359-read SI table pending reconciliation. A2-T1 rules out nearest-saddle/Poisson support; only `core_hits_min` clears the 3x contrast, narrowly. #45: explicit `tissue_mass` generator path landed with legacy default unchanged; mass sweep and halo descent semantics remain. |
