...
| M4 — Stage 2 core (complex, evidence gate, dual flow) | in progress (step 1a flag-complex construction landed — `stage2/flag_complex.py`, S4.1/S4.2/S4.5/S13.4. step 2 DM EVIDENCE GATE landed — `evidence/dm_score.py`+`star_matrix.py`+`gate.py`, S3.4/S3.5/S3.6/S10.4; #42 resolved (S10.4 blesses incidence+rank-guard operational runtime); #43 stub `affected_dual_subgraph_connected` landed, still needs S6 adjacency; Remaining: wire gate into runtime scaffold loop (unblocks M1 Part B), T3 count transfer (S4.3/S4.4), dual-connectivity #43, topology-recovery #41, S6 dual flow) |
