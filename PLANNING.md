...
| M1 — Canonical clustering objective | blocked — #44 flag stays off. #48: the corrected injective, area-uniform S-curve null accepts 4/20 seeds with `φ` as low as 0.096, overlapping the composite-accept band through 0.244; D3 therefore killed fixed-ceiling calibration as a sufficient evidence floor and escalated to statistic mechanism controls. `φ=0.25` stays unchanged and calibrated-provisional; the zero-cross guard and level-set path remain default off. #45: pure nested/bimodal children and an off-scope larger weak-child run are clean, but the exact `n≈194`, `N≤24` weak-child regime remains pending. No default or ceiling change is authorized. |
