...
| M1 — Canonical clustering objective | in progress (#27: coarse-anchoring fixes circle/hierarchy heuristic-free; cold-start recheck (ii') REFUTED. Part B: DM cluster-acceptance reduction now IMPLEMENTED behind a flag (`stage1/dm_cluster.py`, `RecursionConfig.require_dm_split`, proposed SI S2.6.3) + validated + cross-family audited (gpt-5.4-high, fixes applied). FINDING: single-scale DM alone over-fragments at every level (circle 56 / hierarchy 32 / swiss 76), but `persist+dm` recovers circle 1 / hierarchy 6 / swiss 1 heuristic-free. Remaining: fuller scenario suite before deleting stand-ins / flipping default) |
