...
| M1 — Canonical clustering objective | blocked on #44 (#27: coarse-anchoring fixes circle/hierarchy heuristic-free; cold-start recheck (ii') REFUTED. Part B: DM cluster-acceptance reduction IMPLEMENTED behind a flag (`stage1/dm_cluster.py`, `RecursionConfig.require_dm_split`, proposed SI S2.6.3) + validated + cross-family audited. `persist+dm` recovers circle 1 / hierarchy 6 / swiss 1 heuristic-free. FULLER-SUITE FINDING: recursion terminates on K=1-at-tau* before the gate sees disconnected structure — filed #44. Prototype landed flag-gated `allow_finer_research` + radial/band/noncentroid/signal-density/PCA-axis prepasses (defaults off). **Nested unit recovery via signal-density**; **A2-T21 PCA prepass**: unit offset rings ok, interlocking linked_tori still 1 leaf; swiss safe only at steps≤4 — do not flip awaiting. Next: tube/spectral tori cues. Stand-in deletion / default flip still gated on #44 fuller confirmation) |
| M2 — Characteristic-scale selection rebuild | in progress (load-crossover default; persistence signal + tau/heat bridge landed; c_{d,k} calibrated + shipped, SI S2.5.5; #31 hierarchy-recovery harness now uses per-frame tau + gates tightened vs canonical tau*I (S2.5.4), RESOLVED; paper S2.6.2 cluster-count pointer; **load_band selector deleted**; SI DEPRECATED cleaned; `persistence.py` docstring rewritten; `resolve_within_interval` hybrid prototyped default-off; A6 diagnose: ~9× is category mismatch (fine-leaf expected vs coarse persist) — keep coarse-end default; paper L=1/coarse-end synced; **A3-T31 SI flag prose landed**; remaining #28: experimental within-interval strategies / accept coarse-end) |
...
| M4 — Stage 2 core (complex, evidence gate, dual flow) | in progress (step 1a flag-complex construction landed — `stage2/flag_complex.py`, S4.1/S4.2/S4.5/S13.4. step 2 DM EVIDENCE GATE landed — `evidence/dm_score.py`+`star_matrix.py`+`gate.py`, S3.4/S3.5/S3.6/S10.4; #42 resolved; #43 stub + DualAdjacency Protocol + SI anchors landed; A5 assigned experimental `dual_flow.py` stub (throughput). #41: A4-T12 reading path = prefer SI 1.5σ* via denser coverage (not mult=6); recovery flips still need coverage evidence. Remaining: wire gate into runtime scaffold loop, T3 count transfer, dual-connectivity #43, topology-recovery #41, S6 dual flow) |
