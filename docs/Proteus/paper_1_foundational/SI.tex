...
\subsection*{S3.4 Evidence-Based Structure Learning (Local Free Energy)}
Purely geometric triggers (e.g., $\sigma_i^2>\tau_{\mathrm{local},i}$ or high torsion ratio $R_S$) are fast indicators of local misfit, but they cannot distinguish unresolved structure from noise under weak evidence. Proteus therefore treats geometry as a \emph{proposal} mechanism and uses a local evidence test to \emph{ratify} discrete edits (splits, pruning/merges, and optional warp attachments). The evidence is computed from how well the current topology explains the observed local transition counts $\{n_{i\to j}\}$ under the geometry-induced router $q(j\mid i; m)$ (see S13).
...
\paragraph{Local likelihood and prior.}
Restrict the transition log-likelihood to affected sources:
\[
\mathcal L_{\mathcal R}(m;M)\;=\;\sum_{i\in\mathcal V_{\mathrm{aff}}}\ \sum_{j\in\mathcal N(i)} n_{i\to j}\,\log q(j\mid i; m,M)\, ,
\]
...
\paragraph{Local settle (MAP).}
For each candidate topology $M$, warm-start the affected masses from the current solution and settle to a local MAP configuration:
\[
\hat m_M\;=\;\arg\max_{m\in\Delta}\ \mathcal L_{\mathcal R}(m;M)\;+\;\log \psi(m\mid M)\, ,
\]
...
\paragraph{BIC/MDL evidence proxy and acceptance.}
We score each candidate by a local free-energy proxy (negative evidence) with an explicit complexity penalty:
\[
F(\mathcal R;M)\;=\;-\mathcal L_{\mathcal R}(\hat m_M;M)\;-\;\log \psi(\hat m_M\mid M)\;+\;\tfrac{1}{2}\,k_M\,\log N_{\mathrm{eff},\mathcal R}\, ,
\]
where $N_{\mathrm{eff},\mathcal R}=\sum_{i\in\mathcal V_{\mathrm{aff}}}\sum_{j} n_{i\to j}$ and $k_M$ is the number of free mass parameters in the affected region (typically $k_M=|\mathcal S_{\mathrm{aff}}|-1$ under the simplex constraint, counted consistently across candidates). The edit is accepted iff it decreases local free energy by a margin:
\[
F(\mathcal R;M_{\mathrm{edit}})\;<\;F(\mathcal R;M_{\mathrm{keep}})\;-\;\log\tau\, ,
\]
with $\tau>1$ controlling conservatism (e.g., $\log\tau\in[1,3]$ for splits; merges/prunes may use $\tau=1$).
...
\textbf{Torsion Ladder:} \emph{Ignore} if $R_S{<}0.05$ (numerical noise). \emph{Keep as-is} if $0.05\le R_S{<}0.30$ (tolerable curvature). \emph{Geometric fix} if $0.30\le R_S{<}0.60$: propose a torsion-aligned split with explicit placement below, and accept it only if it passes the local free-energy/evidence test in S3.4 (in addition to the numerical safeguards in this section). \emph{Non-linear upgrade} if $R_S\ge 0.60$: attach a local mini-NSF (see S7) to the affected patch. If the fitted mini-NSF fails to reduce $R_S$ below $0.60$ on the patch, propose a torsion-aligned split as a geometric fallback, again gated by S3.4.
...
This reconstruction is conditioned on the accepted topology and its settled geometry/mass field; it does not act as an additional likelihood term for the same simplex masses.
...
\end{itemize}
Proteus uses geometric signals to propose discrete edits to $\mathcal S$, and a local evidence proxy (S3.4) to accept edits only when they reduce free energy after accounting for model complexity.
