...
The raw feature set from Proteus---a high-dimensional, sparse fuzzy membership vector---is not optimal for direct use in complex analytical models. Moreover, the foundational Proteus model provides not only memberships, but also structure-sensitive diagnostics derived from its evidence objective (e.g., local residual and misfit signals that indicate where the learned manifold is currently under-explaining incoming data). To address this, we introduce a foundational representation learning layer: a self-scaling autoencoder that produces compact, dense, and semantically rich latent vectors. This layer acts as a stable interface between the sparse, structured vocabulary produced by the geometric generative model and the dense representations expected by downstream analytics.
...
\begin{enumerate}
    ...
    \item \textbf{Drift Detection:} Concept drift is detected by comparing the statistical profile of a recent time window to that of a more stable, longer-term window. A significant divergence indicates that the underlying data distribution has changed. In addition, we monitor a model-level signal: the local evidence (equivalently, free energy) of the underlying Proteus generative model on recent data. A sustained rise in free energy can flag drift even when latent-vector moments remain stable, because it detects failures of the current manifold structure to predict local transition statistics.
